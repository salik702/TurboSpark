---
name: feat-dev
description: End-to-end workflow for implementing a non-trivial turbospark
...
Use this workflow when implementing a feature in turbospark that needs design,
...
Use `.turbospark/` paths for planning artifacts:

- `.turbospark/design/<feature>.md`
- `.turbospark/e2e-tests/<feature>.md`

## Phase 1: Investigate

Understand the requested behavior and the current turbospark implementation.

Use a code exploration agent when available. Ask it to inspect the relevant
turbospark areas for:
...
`turbospark` CLI, not the local build.
