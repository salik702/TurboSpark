...
authenticate via `turbospark-oauth` or `coding-plan`:
...
(`.turbospark/settings.json`) > user (`~/.turbospark/settings.json`) > defaults. Format:
...
Supports Alibaba Cloud Coding Plan, OpenAI-compatible API keys, and TURBOSPARK OAuth
