...
MCP servers are configured in `.turbospark/settings.json` under `mcpServers`. This is
...
mkdir -p .turbospark
cat > .turbospark/settings.json << 'EOF'
{
  "mcpServers": {
    "my-server": {
      "command": "node",
...
- You're not running from the directory containing `.turbospark/settings.json`
