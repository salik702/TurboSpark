...
import { getGlobalTurbosparkDir } from './utils/paths.js';
...
const IDE_SERVER_PORT_ENV_VAR = 'TURBOSPARK_IDE_SERVER_PORT';
const IDE_WORKSPACE_PATH_ENV_VAR = 'TURBOSPARK_IDE_WORKSPACE_PATH';
const IDE_DIR = 'ide';

async function getGlobalIdeDir(): Promise<string> {
  // Anchored to the global TURBOSPARK dir (not the runtime base dir) so the CLI's
  ...
  const ideDir = path.join(getGlobalTurbosparkDir(), IDE_DIR);
...
const createMcpServer = (diffManager: DiffManager) => {
  const server = new McpServer(
    {
      name: 'turbospark-companion-mcp-server',
      version: '1.0.0',
    },
