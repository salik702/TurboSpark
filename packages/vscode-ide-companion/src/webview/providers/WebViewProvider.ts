...
import { TurbosparkAgentManager } from '../../services/turbosparkAgentManager.js';
...
import {
  ...
  readTurbosparkSettingsForVSCode,
  clearPersistedAuth,
...
const AUTH_RELATED_TURBOSPARK_SETTINGS = [
  'turbospark.provider',
  'turbospark.apiKey',
  'turbospark.codingPlanRegion',
] as const;

export function resolveTurbosparkCliEntryPath(
  ...
  return vscode.Uri.joinPath(extensionUri, 'dist', 'turbospark-cli', 'cli.js').fsPath;
}
...
export class WebViewProvider {
  ...
  private agentManager: TurbosparkAgentManager;
  ...
  constructor(
    ...
    this.agentManager = new TurbosparkAgentManager();
    ...
    const configChangeDisposable = vscode.workspace.onDidChangeConfiguration(
      async (e) => {
        const authSettingsChanged = AUTH_RELATED_TURBOSPARK_SETTINGS.some((setting) =>
          e.affectsConfiguration(setting),
        );

        if (authSettingsChanged && !this.isSyncingToVSCode) {
          console.log(
            '[WebViewProvider] Auth-related turbospark settings changed by user, syncing...',
          );
          ...
          if (synced && this.agentInitialized) {
            ...
            e.affectsConfiguration('turbospark.apiKey')
          ) {
            // Only de-auth when turbospark.apiKey itself was cleared.
    ...
    // Note: Tool call updates are handled in handleSessionUpdate within TurbosparkAgentManager
      ...
      async (request: RequestPermissionRequest) => {
        ...
        return new Promise((resolve) => {
          ...
          const handler = (message: PermissionResponseMessage) => {
            ...
            // Always close open turbospark-diff editors after any permission decision
            void vscode.commands.executeCommand('turbospark.diff.closeAll');
            ...
            if (isCancel) {
              ...
              void vscode.commands.executeCommand('turbospark.diff.suppressBriefly');
            }
          };
    ...
    newPanel.webview.onDidReceiveMessage(
      async (message: { type: string; data?: unknown }) => {
        ...
        if (message.type === 'updatePanelTitle') {
          ...
          if (panelRef) {
            panelRef.title = title ? truncatePanelTitle(title) : 'TURBO SPARK';
          }
          return;
        }
        if (this.handleNewChatByContext(message)) {
  ...
  /**
   * Sync VSCode extension settings (turbospark.*) to ~/.turbospark/settings.json
  ...
  private async syncVSCodeSettingsToQwenConfig(): Promise<boolean> {
    const config = vscode.workspace.getConfiguration('turbospark');
    ...
    try {
      ...
      console.log(
        `[WebViewProvider] Synced VSCode settings → ~/.turbospark/settings.json (provider=${provider})`,
      );
      return true;
    } catch (error) {
  ...
  /**
   * Sync ~/.turbospark/settings.json values back to VSCode Settings UI.
  ...
  private async syncQwenConfigToVSCodeSettings(): Promise<void> {
    try {
      const qwenSettings = readTurbosparkSettingsForVSCode();
      if (!qwenSettings) {
        return;
      }

      console.log(
        '[WebViewProvider] Syncing ~/.turbospark/settings.json → VSCode settings',
      );
      ...
      if (updates.length === 0) {
        console.log(
          '[WebViewProvider] VSCode settings already match ~/.turbospark/settings.json',
        );
        return;
      }
  ...
  /**
   * Attempt to restore authentication state and initialize connection.
   * On startup, sync ~/.turbospark/settings.json → VSCode settings so the Settings UI
   * reflects existing non-secret CLI config, then attempt a connection.
   * Writing back to ~/.turbospark/settings.json happens through the auth flow and
  ...
  private async doInitializeAgentConnection(options?: {
    ...
    const run = async () => {
      ...
      const cliEntry = resolveTurbosparkCliEntryPath(
        this.extensionUri,
   ...
   * Writes provider config to ~/.turbospark/settings.json and reconnects.
   * Mirrors the CLI's `qwen auth coding-plan` / `qwen auth` flow.
   */
  private async handleAuthInteractive(
    providerConfig: import('@turbospark/turbospark-core').ProviderConfig,
    inputs: import('@turbospark/turbospark-core').ProviderSetupInputs,
  ): Promise<void> {
    if (!inputs.apiKey) {
      this.sendMessageToWebView({
        type: 'authError',
   ...
   *   routing to the newTurbosparkSession handler (includes auth checks and UI clearing).
  ...
  private handleNewChatByContext(message: {
    ...
    void this.messageHandler.route({ type: 'newTurbosparkSession', data: {} });
    return true;
  }
   ...
   * the TURBO SPARK panel (or sidebar view) when clicked.
   */
  private notifyUser(message: string): void {
    void vscode.window
      .showInformationMessage(`TURBO SPARK: ${message}`, 'Show')
      .then((action) => {
        if (action === 'Show') {
          const panel = this.panelManager.getPanel();
          if (panel) {
            ...
            void vscode.commands.executeCommand('turbospark.focusChat');
          }
        }
      });
  ...
  /**
   * Whether the user can currently see the TURBO SPARK panel.
  ...
  /** Whether the turbospark.notifications setting is enabled. */
