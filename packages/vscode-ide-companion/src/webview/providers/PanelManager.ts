...
export class PanelManager {
  ...
  async createPanel(): Promise<boolean> {
    ...
    // First, check if there's an existing TURBO SPARK group
    const existingGroup = this.findExistingQwenCodeGroup();

    if (existingGroup) {
      // If TURBO SPARK webview already exists in a locked group, create the new panel in that same group
      console.log(
        '[PanelManager] Found existing TURBO SPARK group, creating panel in same group',
      );
      ...
      // If no existing TURBO SPARK group, create a new group to the right of the active editor group
      try {
  ...
  /**
   * Find the group and view column where the existing TURBO SPARK webview is located
    ...
    for (const group of vscode.window.tabGroups.all) {
      for (const tab of group.tabs) {
        ...
        if (
          ...
          input.viewType === 'mainThreadWebview-turbosparkCode.chat'
        ) {
          // Found an existing TURBO SPARK tab
          console.log('[PanelManager] Found existing TURBO SPARK group:', {
  ...
  captureTab(): void {
    ...
    setTimeout(() => {
      ...
      const match = allTabs.find((t) => {
        ...
        const sameViewType = isWebview && input.viewType === 'turbosparkCode.chat';
