...
export const useSessionManagement = (vscode: VSCodeAPI) => {
  const [qwenSessions, setTurbosparkSessions] = useState<
   ...
   * Load session list
   */
  const handleLoadTurbosparkSessions = useCallback(() => {
    // Reset pagination state and load first page
    setTurbosparkSessions([]);
    setNextCursor(undefined);
    setHasMore(true);
    setIsLoading(true);
    vscode.postMessage({ type: 'getTurbosparkSessions', data: { size: PAGE_SIZE } });
   ...
   * Create new session
   */
  const handleNewTurbosparkSession = useCallback(
    (modelId?: string | null) => {
  ...
  return {
    ...
    // Operations
    handleLoadTurbosparkSessions,
    handleNewTurbosparkSession,
    handleSwitchSession,
