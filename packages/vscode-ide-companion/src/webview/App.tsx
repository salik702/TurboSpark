...
import type { PermissionOption, PermissionToolCall } from '@turbospark/webui';
...
function findMessageIndex(
  ...
  // .turbospark-message class (e.g. InterruptedMessage does not).
  ...
  useEffect(() => {
    const clearEditingOnRestoreOrFailure = (event: MessageEvent) => {
      ...
      if (
        ...
        message?.type === 'turbosparkSessionSwitched' ||
        ...
        for (const item of allMessages) {
          if (item.type === 'message') {
            ...
            if (msg.role === 'user') {
              ...
              parts.push(`**TURBO SPARK:** ${content}`);
            }
          } else if (
            item.type === 'completed-tool-call' ||
            item.type === 'in-progress-tool-call'
  ...
  return (
    <div className="chat-container relative">
      ...
      <ChatHeader
        ...
        onLoadSessions={sessionManagement.handleLoadTurbosparkSessions}
        onNewSession={() =>
          sessionManagement.handleNewTurbosparkSession(modelInfo?.modelId ?? null)
        }
      />
