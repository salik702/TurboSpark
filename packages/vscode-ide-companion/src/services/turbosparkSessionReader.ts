...
import { getGitBranch, getProjectHash } from '@turbospark/turbospark-core';
import { getRuntimeBaseDir } from '../utils/paths.js';
import { truncatePanelTitle } from '../webview/utils/panelTitleUtils.js';

export interface TurbosparkMessage {
  id: string;
  timestamp: string;
  type: 'user' | 'turbospark';
  content: string;
  thoughts?: unknown[];
  tokens?: {
    input: number;
...
export interface TurbosparkSession {
  sessionId: string;
  projectHash: string;
  startTime: string;
  lastUpdated: string;
  messages: TurbosparkMessage[];
  filePath?: string;
  messageCount?: number;
...
export class TurbosparkSessionReader {
    ...
    try {
      ...
      if (!allProjects && workingDir) {
        ...
        if (!fs.existsSync(tmpDir)) {
          console.log('[TurbosparkSessionReader] Tmp directory not found:', tmpDir);
          return [];
        }
      ...
      console.error('[TurbosparkSessionReader] Failed to get sessions:', error);
      return [];
    }
  }
   ...
   */
  private async readSessionsFromDir(chatsDir: string): Promise<TurbosparkSession[]> {
    const sessions: TurbosparkSession[] = [];
    ...
    for (const file of jsonSessionFiles) {
      ...
      try {
        ...
        console.error(
          '[TurbosparkSessionReader] Failed to read session file:',
          filePath,
          error,
        );
      }
    ...
    for (const file of jsonlSessionFiles) {
      ...
      try {
        ...
        console.error(
          '[TurbosparkSessionReader] Failed to read JSONL session file:',
          filePath,
          error,
        );
      }
   ...
   */
  getSessionTitle(session: TurbosparkSession): string {
  ...
  private async readJsonlSession(
    ...
    try {
      ...
      const messages: TurbosparkMessage[] = [];
        ...
        if (type === 'user' || type === 'assistant') {
          ...
          if (includeMessages) {
            messages.push({
              ...
              type: type === 'user' ? 'user' : 'turbospark',
              content: text,
            });
          }
      ...
      console.error(
        '[TurbosparkSessionReader] Failed to parse JSONL session:',
        error,
      );
      return null;
    }
  ...
  async deleteSession(sessionId: string, workingDir: string): Promise<boolean> {
    try {
      ...
      console.error('[TurbosparkSessionReader] Failed to delete session:', error);
      return false;
    }
  }
  ...
  async renameSession(
    ...
    try {
      ...
      console.error('[TurbosparkSessionReader] Failed to rename session:', error);
      return false;
    }
  }
}
