 ...
 * Handles bidirectional sync between VSCode Settings and ~/.turbospark/settings.json.
 */
...
/**
 * Mode for ~/.turbospark/settings.json — owner-only read/write. The unified install
...
/** Directory mode for ~/.turbospark — owner-only. */
...
/**
 * Values extracted from ~/.turbospark/settings.json for populating VSCode Settings.
 */
 ...
 * so that `JSON.parse` can handle ~/.turbospark/settings.json files that
...
/**
 * Read ~/.turbospark/settings.json. Returns {} if the file is missing.
...
function readSettings(): Record<string, unknown> {
  ...
  try {
    ...
    throw new Error(
      `Cannot parse ~/.turbospark/settings.json (${message}). ` +
...
/**
 * Write ~/.turbospark/settings.json atomically (temp file + rename), creating the
...
function writeSettings(settings: Record<string, unknown>): void {
  ...
  try {
    ...
    // target (EPERM/EBUSY). The temp file otherwise lingers in ~/.turbospark
...
// ---------------------------------------------------------------------------
// Write: VSCode Settings → ~/.turbospark/settings.json
// ---------------------------------------------------------------------------

/**
 * Write Coding Plan configuration to ~/.turbospark/settings.json.
...
/**
 * Write model providers from VSCode Settings (key-value map) to ~/.turbospark/settings.json.
...
/**
 * Create a ProviderSettingsAdapter backed by ~/.turbospark/settings.json.
...
/**
 * Apply a ProviderInstallPlan to ~/.turbospark/settings.json.
...
// ---------------------------------------------------------------------------
// Read: ~/.turbospark/settings.json → VSCode Settings
// ---------------------------------------------------------------------------

/**
 * Read ~/.turbospark/settings.json and extract values for VSCode Settings UI.
 ...
 */
export function readTurbosparkSettingsForVSCode(): QwenSettingsForVSCode | null {
  let settings: Record<string, unknown>;
  try {
    settings = readSettings();
  } catch (error) {
    console.error(
      '[settingsWriter] readTurbosparkSettingsForVSCode failed; returning null:',
      error,
    );
    return null;
  }
...
/**
 * Clear persisted auth credentials from ~/.turbospark/settings.json.
