...
import { TurbosparkSessionReader, type TurbosparkSession } from './turbosparkSessionReader.js';
import { TurbosparkSessionManager } from './turbosparkSessionManager.js';
import type {
  ChatMessage,
  PlanEntry,
  ToolCallUpdateData,
  TurbosparkAgentCallbacks,
  UsageStatsPayload,
} from '../types/chatTypes.js';
import {
  TurbosparkConnectionHandler,
  type QwenConnectionResult,
} from '../services/turbosparkConnectionHandler.js';
import { TurbosparkSessionUpdateHandler } from './turbosparkSessionUpdateHandler.js';
...
export class TurbosparkAgentManager {
  private connection: AcpConnection;
  private sessionReader: TurbosparkSessionReader;
  private sessionManager: TurbosparkSessionManager;
  private connectionHandler: TurbosparkConnectionHandler;
  private sessionUpdateHandler: TurbosparkSessionUpdateHandler;
  ...
  // Callback storage
  private callbacks: TurbosparkAgentCallbacks = {};
  ...
  constructor() {
    ...
    this.sessionReader = new TurbosparkSessionReader();
    this.sessionManager = new TurbosparkSessionManager();
    this.connectionHandler = new TurbosparkConnectionHandler();
    this.sessionUpdateHandler = new TurbosparkSessionUpdateHandler({});
    ...
    this.connection.onSessionUpdate = (data: SessionNotification) => {
      ...
      try {
        ...
        console.warn('[TurbosparkAgentManager] Rehydration routing failed:', err);
      }
    ...
    this.connection.onEndTurn = (reason?: string, source?: string) => {
      try {
        ...
        console.warn('[TurbosparkAgentManager] onEndTurn callback error:', err);
      }
    };
    ...
    this.connection.onAuthenticateUpdate = (
      ...
      try {
        ...
        console.warn(
          '[TurbosparkAgentManager] onAuthenticateUpdate callback error:',
          err,
        );
      }
    };
    ...
    this.connection.onInitialized = (init: unknown) => {
      try {
        ...
        console.warn('[TurbosparkAgentManager] onInitialized parse error:', err);
      }
    };
    ...
    this.connection.onDisconnected = (
      ...
      console.log(
        `[TurbosparkAgentManager] Process disconnected (code: ${code}, signal: ${signal})`,
      );
  ...
  async connect(
    ...
    if (res.availableModels && res.availableModels.length > 0) {
      ...
      console.log(
        '[TurbosparkAgentManager] Emitting availableModels from connect():',
  ...
  async reconnect(
    ...
    console.log('[TurbosparkAgentManager] Attempting reconnection...');
    try {
  ...
  async setApprovalModeFromUi(
    ...
    try {
      ...
      console.error('[TurbosparkAgentManager] Failed to set mode:', err);
      throw err;
    }
  }
  ...
  async setModelFromUi(modelId: string): Promise<ModelInfo | null> {
    try {
      ...
      console.error('[TurbosparkAgentManager] Failed to set model:', err);
      throw err;
    }
  }
  ...
  async validateCurrentSession(): Promise<boolean> {
    try {
      ...
      console.warn('[TurbosparkAgentManager] Session validation failed:', error);
  ...
  async getSessionList(): Promise<Array<Record<string, unknown>>> {
    console.log(
      '[TurbosparkAgentManager] Getting session list with version-aware strategy',
    );

    try {
      console.log(
        '[TurbosparkAgentManager] Attempting to get session list via ACP method',
      );
      const response = await this.connection.listSessions();
      console.log('[TurbosparkAgentManager] ACP session list response:', response);

      const res: unknown = response;
      const items = extractSessionListItems(res);

      console.log(
        '[TurbosparkAgentManager] Sessions retrieved via ACP:',
        res,
        items.length,
      );
      if (items.length > 0) {
      ...
      console.warn(
        '[TurbosparkAgentManager] ACP session list failed, falling back to file system method:',
        error,
      );
    }
    ...
    try {
      console.log('[TurbosparkAgentManager] Getting session list from file system');
      const sessions = await this.sessionReader.getAllSessions(undefined, true);
      console.log(
        '[TurbosparkAgentManager] Session list from file system (all projects):',
        sessions.length,
      );

      const result = sessions.map(
        (session: TurbosparkSession): Record<string, unknown> => ({
      ...
      console.log(
        '[TurbosparkAgentManager] Sessions retrieved from file system:',
        result.length,
      );
      return result;
    } catch (error) {
      console.error(
        '[TurbosparkAgentManager] Failed to get session list from file system:',
        error,
      );
      return [];
    }
  ...
  async getSessionListPaged(params?: {
    ...
    try {
      ...
      console.warn('[TurbosparkAgentManager] Paged ACP session list failed:', error);
    ...
    try {
      ...
      console.error('[TurbosparkAgentManager] File system paged list failed:', error);
  ...
  async getSessionMessages(sessionId: string): Promise<ChatMessage[]> {
    try {
      try {
        ...
        console.log(
          '[TurbosparkAgentManager] Session list item for filePath lookup:',
          item,
        );
        if (
          typeof item === 'object' &&
        ...
        console.warn('[TurbosparkAgentManager] JSONL read path lookup failed:', e);
      }
      ...
      console.error(
        '[TurbosparkAgentManager] Failed to get session messages:',
        error,
      );
      return [];
    }
  ...
  async deleteSession(sessionId: string): Promise<boolean> {
    try {
      ...
      console.error('[TurbosparkAgentManager] Failed to delete session:', error);
      return false;
    }
  }
  ...
  async renameSession(sessionId: string, title: string): Promise<boolean> {
    try {
      ...
      console.error('[TurbosparkAgentManager] Failed to rename session:', error);
      return false;
    }
  }
  ...
  private async readJsonlMessages(filePath: string): Promise<ChatMessage[]> {
    ...
    try {
      ...
      console.log(
        '[TurbosparkAgentManager] JSONL records read:',
        records.length,
        filePath,
      );
      ...
      console.log(
        '[TurbosparkAgentManager] JSONL messages reconstructed:',
        msgs.length,
      );
      return msgs;
    } catch (err) {
      console.warn('[TurbosparkAgentManager] Failed to read JSONL messages:', err);
      return [];
    }
  }
  ...
  async loadSessionViaAcp(
    ...
    try {
      ...
      console.log(
        '[TurbosparkAgentManager] Rehydration start for session:',
        sessionId,
      );
      console.log(
        '[TurbosparkAgentManager] Attempting session/load via ACP for session:',
        sessionId,
      );
      ...
      console.log(
        '[TurbosparkAgentManager] Session load succeeded. Response:',
      ...
      console.error(
        '[TurbosparkAgentManager] Session load via ACP failed for session:',
        sessionId,
      );
      console.error('[TurbosparkAgentManager] Error type:', error?.constructor?.name);
      console.error('[TurbosparkAgentManager] Error message:', errorMessage);
      ...
      if (error && typeof error === 'object') {
        ...
        if ('error' in error) {
          ...
          if (acpError.error) {
            console.error(
              '[TurbosparkAgentManager] ACP error code:',
              acpError.error.code,
            );
            console.error(
              '[TurbosparkAgentManager] ACP error message:',
              acpError.error.message,
            );
          }
        } else {
          console.error('[TurbosparkAgentManager] Non-ACPIf error details:', error);
        }
      }
      ...
      console.log('[TurbosparkAgentManager] Rehydration end for session:', sessionId);
  ...
  async loadSession(sessionId: string): Promise<ChatMessage[] | null> {
    console.log(
      '[TurbosparkAgentManager] Loading session with version-aware strategy:',
      sessionId,
    );

    try {
      console.log(
        '[TurbosparkAgentManager] Attempting to load session via ACP method',
      );
      await this.loadSessionViaAcp(sessionId);
      console.log('[TurbosparkAgentManager] Session loaded successfully via ACP');
      ...
      console.warn(
        '[TurbosparkAgentManager] ACP session load failed, falling back to file system method:',
        error,
      );
    }
    ...
    try {
      console.log(
        '[TurbosparkAgentManager] Loading session messages from file system',
      );
      const messages = await this.loadSessionMessagesFromFile(sessionId);
      console.log(
        '[TurbosparkAgentManager] Session messages loaded successfully from file system',
      );
      return messages;
    } catch (error) {
      console.error(
        '[TurbosparkAgentManager] Failed to load session messages from file system:',
        error,
      );
      return null;
    }
  ...
  private async loadSessionMessagesFromFile(
    ...
    try {
      console.log(
        '[TurbosparkAgentManager] Loading session from file system:',
        sessionId,
      );
      ...
      if (!session) {
        console.log(
          '[TurbosparkAgentManager] Session not found in file system:',
          sessionId,
        );
        return null;
      }
      ...
      console.error(
        '[TurbosparkAgentManager] Session load from file system failed:',
        error,
      );
      throw error;
    }
  ...
  async createNewSession(
    ...
    if (!forceNew && this.connection.currentSessionId) {
      console.log(
        '[TurbosparkAgentManager] createNewSession: reusing existing session',
    ...
    if (this.sessionCreateInFlight) {
      console.log(
        '[TurbosparkAgentManager] createNewSession: session creation already in flight',
      );
      if (!forceNew) {
    ...
    console.log('[TurbosparkAgentManager] Creating new session...');
    ...
    this.sessionCreateInFlight = (async () => {
      try {
        ...
        try {
          ...
          console.log(
            '[TurbosparkAgentManager] newSession returned:',
          ...
          if (requiresAuth) {
            if (!autoAuthenticate) {
              console.warn(
                '[TurbosparkAgentManager] session/new requires authentication but auto-auth is disabled. Deferring until user logs in.',
              );
              throw err;
            }
            console.warn(
              '[TurbosparkAgentManager] session/new requires authentication. Retrying with authenticate...',
            );
            try {
              // Let CLI handle authentication - it's the single source of truth
              await this.connection.authenticate(authMethod);
              console.log(
                '[TurbosparkAgentManager] createNewSession Authentication successful. Retrying session/new...',
              );
              ...
              console.error(
                '[TurbosparkAgentManager] Re-authentication failed:',
                reauthErr,
              );
              throw reauthErr;
            }
        ...
        console.log(
          '[TurbosparkAgentManager] New session created with ID:',
          newSessionId,
        );
        return newSessionId;
      } finally {
  ...
  async cancelCurrentPrompt(): Promise<void> {
    console.log('[TurbosparkAgentManager] Cancelling current prompt');
