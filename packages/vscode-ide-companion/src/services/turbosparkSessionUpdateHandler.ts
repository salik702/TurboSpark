...
/**
 * TURBOSPARK Session Update Handler
 *
...
/**
 * TURBOSPARK Session Update Handler class
 * Processes various session update events and calls appropriate callbacks
 */
export class TurbosparkSessionUpdateHandler {
  private callbacks: TurbosparkAgentCallbacks;

  constructor(callbacks: TurbosparkAgentCallbacks) {
   ...
   */
  updateCallbacks(callbacks: TurbosparkAgentCallbacks): void {
    this.callbacks = callbacks;
  }
  ...
  handleSessionUpdate(data: SessionNotification): void {
    ...
    switch (sessionUpdate) {
      ...
      case 'agent_message_chunk': {
        ...
        // both carry turbosparkDiscreteMessage. Persist only the original here so the
        ...
        const isDiscreteMessage =
          (meta?.turbosparkDiscreteMessage === true ||
      ...
      default:
        console.log('[TurbosparkAgentManager] Unhandled session update type');
        break;
    }
  }
