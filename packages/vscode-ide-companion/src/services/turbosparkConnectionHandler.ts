 ...
 *
 * Handles TURBOSPARK Agent connection establishment, authentication, and session creation
 */
 ...
 */
export class TurbosparkConnectionHandler {
  /**
  ...
  async connect(
    ...
    console.log(`[TurbosparkAgentManager] 🚀 CONNECT() CALLED - ID: ${connectId}`);
    ...
    if (proxyUrl) {
      ...
      console.log(
        '[TurbosparkAgentManager] Using proxy from VSCode settings:',
        proxyUrl,
      );
    }
    ...
    for (let attempt = 1; attempt <= maxConnectAttempts; attempt++) {
      try {
        console.log(
          `[TurbosparkAgentManager] Connecting to ACP process (attempt ${attempt}/${maxConnectAttempts})...`,
        );
        await connection.connect(cliEntryPath!, workingDir, extraArgs);
        console.log('[TurbosparkAgentManager] ACP process connected successfully');
        break;
      } catch (connectError) {
        console.error(
          `[TurbosparkAgentManager] Connect attempt ${attempt} failed:`,
        ...
        console.log(`[TurbosparkAgentManager] Retrying connect in ${delay}ms...`);
    ...
    if (!sessionRestored) {
      console.log(
        '[TurbosparkAgentManager] no sessionRestored, Creating new session...',
      );

      try {
        console.log(
          '[TurbosparkAgentManager] Creating new session (letting CLI handle authentication)...',
        );
        ...
        if (
          ...
          console.log(
            '[TurbosparkAgentManager] Extracted availableModels from session/new:',
        ...
        console.log('[TurbosparkAgentManager] New session created successfully');
        sessionCreated = true;
      } catch (sessionError) {
        const needsAuth =
        ...
        if (needsAuth) {
          requiresAuth = true;
          console.log(
            '[TurbosparkAgentManager] Session creation requires authentication; waiting for user-triggered login.',
          );
        } else {
          console.log(
            `\n⚠️ [SESSION FAILED] newSessionWithRetry threw error\n`,
          );
          console.log(`[TurbosparkAgentManager] Error details:`, sessionError);
          throw sessionError;
        }
      }
    } else {
    ...
    console.log(`\n========================================`);
    console.log(`[TurbosparkAgentManager] ✅ CONNECT() COMPLETED SUCCESSFULLY`);
    console.log(`========================================\n`);
    return {
      sessionCreated,
      requiresAuth,
  ...
  private async newSessionWithRetry(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(
          `[TurbosparkAgentManager] Creating session (attempt ${attempt}/${maxRetries})...`,
        );
        const res = await connection.newSession(workingDir);
        console.log('[TurbosparkAgentManager] Session created successfully');
        return res;
      } catch (error) {
        lastError = error;
        const errorMessage = getErrorMessage(error);
        console.error(
          `[TurbosparkAgentManager] Session creation attempt ${attempt} failed:`,
          errorMessage,
        );
        ...
        if (requiresAuth) {
          if (!autoAuthenticate) {
            console.log(
              '[TurbosparkAgentManager] Authentication required but auto-authentication is disabled. Propagating error.',
            );
            throw error;
          }
          console.log(
            '[TurbosparkAgentManager] Qwen requires authentication. Authenticating and retrying session/new...',
          );
          try {
            ...
            console.log(
              '[TurbosparkAgentManager] newSessionWithRetry Authentication successful',
            );
            // Retry immediately after successful auth
            const res = await connection.newSession(workingDir);
            console.log(
              '[TurbosparkAgentManager] Session created successfully after auth',
            );
            return res;
          } catch (authErr) {
            console.error(
        ...
        console.log(`[TurbosparkAgentManager] Retrying in ${delay}ms...`);
        await new Promise((resolve) => setTimeout(resolve, delay));
      }
    }
