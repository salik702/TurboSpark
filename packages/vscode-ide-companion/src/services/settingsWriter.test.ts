...
import { AuthType, type ProviderInstallPlan } from '@turbospark/turbospark-core';
...
describe('settingsWriter', () => {
  ...
  beforeEach(() => {
    ...
    settingsPath = path.join(tempDir, '.turbospark', 'settings.json');
  ...
  it('reads an api-key configuration after switching away from coding plan', () => {
    ...
    expect(readTurbosparkSettingsForVSCode()).toEqual({
      provider: 'api-key',
      apiKey: 'manual-key',
