...
const CLI_IDE_COMPANION_IDENTIFIER = 'turbospark.turbospark-vscode-ide-companion';
const INFO_MESSAGE_SHOWN_KEY = 'turbosparkInfoMessageShown';
export const DIFF_SCHEME = 'turbospark-diff';
  ...
  try {
    ...
    if (latestVersion && semver.gt(latestVersion, currentVersion)) {
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the TURBO SPARK Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('TURBO SPARK Companion');
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer('turbosparkCode.chat', {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('turbospark.copyMessage', () =>
      sendCopyToActive('copyMessage'),
    ),
    vscode.commands.registerCommand('turbospark.copyAllMessages', () =>
      sendCopyToActive('copyAllMessages'),
    ),
    vscode.commands.registerCommand('turbospark.copyLastReply', () =>
  ...
  context.subscriptions.push(
    ...
    (vscode.commands.registerCommand('turbospark.diff.accept', (uri?: vscode.Uri) => {
    ...
    vscode.commands.registerCommand('turbospark.diff.cancel', (uri?: vscode.Uri) => {
      const docUri = uri ?? vscode.window.activeTextEditor?.document.uri;
      if (docUri && docUri.scheme === DIFF_SCHEME) {
    ...
    vscode.commands.registerCommand('turbospark.diff.closeAll', async () => {
      try {
        await diffManager.closeAll();
      } catch (err) {
        console.warn('[Extension] turbospark.diff.closeAll failed:', err);
      }
    }),
    vscode.commands.registerCommand('turbospark.diff.suppressBriefly', async () => {
      try {
        diffManager.suppressFor(1200);
      } catch (err) {
        console.warn('[Extension] turbospark.diff.suppressBriefly failed:', err);
      }
    }),
  );
  ...
  if (!context.globalState.get(INFO_MESSAGE_SHOWN_KEY) && infoMessageEnabled) {
    void vscode.window.showInformationMessage(
      'TURBO SPARK Companion extension successfully installed.',
    );
      ...
      async (
        ...
        if (!workspaceFolders || workspaceFolders.length === 0) {
          vscode.window.showInformationMessage(
            'No folder open. Please open a folder to run TURBO SPARK.',
          );
          return;
        }
        ...
        if (workspaceFolders.length === 1) {
          ...
          selectedFolder = await vscode.window.showWorkspaceFolderPick({
            placeHolder: 'Select a folder to run TURBO SPARK in',
          });
        }
        ...
        if (selectedFolder) {
          ...
          const terminalOptions: vscode.TerminalOptions = {
            name: `TURBO SPARK (${selectedFolder.name})`,
            cwd: selectedFolder.uri.fsPath,
            location,
          };

          let turbosparkCmd: string;
          ...
          if (isWindows) {
            ...
            turbosparkCmd = `ELECTRON_RUN_AS_NODE=1 ${baseCmd}`;
          }

          const terminal = vscode.window.createTerminal(terminalOptions);
          terminal.show();
          terminal.sendText(turbosparkCmd);
        }
      },
    ),
    vscode.commands.registerCommand('turbospark.showNotices', async () => {
