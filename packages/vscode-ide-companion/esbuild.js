...
const publicCliExportPlugin = {
  ...
  setup(build) {
    build.onResolve({ filter: /^@turbospark\/turbospark\/export$/ }, () => ({
...
async function main() {
  ...
  const webviewCtx = await esbuild.context({
    ...
    // @turbospark/turbospark-core is a peer dependency of @turbospark/webui.
    // Since @turbospark/webui marks it as external in its own Vite build, the
    ...
    // '@turbospark/turbospark-core/src/core/tokenLimits.js') are also excluded;
    ...
    external: ['@turbospark/turbospark-core', '@turbospark/turbospark-core/*'],
    logLevel: 'silent',
