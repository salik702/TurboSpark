...
1. Extract components from `vscode-ide-companion/src/webview/` into an independent `@turbospark/webui` package
...
```typescript
// @turbospark/webui/src/context/PlatformContext.ts
...
```javascript
// @turbospark/webui/tailwind.preset.js
module.exports = {
  theme: {
    extend: {
      colors: {
...
module.exports = {
  presets: [require('@turbospark/webui/tailwind.preset')],
  content: [
    './src/**/*.{ts,tsx}',
    './node_modules/@turbospark/webui/dist/**/*.js',
  ],
};
```
...
```
Developer ──► @turbospark/webui ──► vscode-ide-companion
  │              │                      │
...
// After: Update import
import { FileIcon } from '@turbospark/webui';
// or import { FileIcon } from '@turbospark/webui/icons';
```
...
1. **Tailwind Class Name Tree Shaking**
   ...
   - Solution: Consumer's `content` config needs to include `node_modules/@turbospark/webui`
...
│     └── Edit files in @turbospark/webui/src/                     │
│                                                                  │
...
│     └── import { Component } from '@turbospark/webui'            │
...
1. **Single Source of Truth**: All UI components live in `@turbospark/webui`
