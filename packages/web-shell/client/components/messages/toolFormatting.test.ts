    ...
    expect(
      getToolDescription(
        tool({
          args: {
            file_path:
              '/Users/ytahdn/Documents/Codes/alishu/turbospark/README.md',
          },
        }),
        '/Users/ytahdn/Documents/Codes/turbospark/turbospark',
      ),
    ).toBe('README.md');
  });
  ...
  it('matches CLI-style glob fallback descriptions', () => {
    expect(
      getToolDescription(
        tool({
          toolName: 'glob',
          args: { pattern: '**/*.ts', path: '/Users/ytahdn/.turbospark' },
        }),
        '/workspace/project',
      ),
    ).toBe("'**/*.ts' in path '/Users/ytahdn/.turbospark'");
  });
  ...
  it('matches CLI-style glob result summaries', () => {
    expect(
      getToolResultSummary(
        tool({
          toolName: 'glob',
          rawOutput: '/Users/ytahdn/.turbospark/settings.json\n',
        }),
      ),
      ...
      getToolDescription(
        tool({
          ...
          args: {
            ...
            description: '查看 ~/.turbospark/settings.json 文件内容',
          },
        }),
      ),
    ).toBe('cat ~/.turbospark/settings.json (查看 ~/.turbospark/settings.json 文件内容)');
  });
