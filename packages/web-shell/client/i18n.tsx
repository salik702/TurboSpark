...
const EN: Messages = {
  ...
  'about.platform': 'OS',
  'about.proxy': 'Proxy',
  'about.turbosparkCode': 'TURBO SPARK',
  ...
  'agent.create.method.turbospark': 'Generate with TURBO SPARK',
  'agent.create.method.turbospark.recommended':
    'Generate with TURBO SPARK (Recommended)',
  'agent.create.method.turbospark.desc':
  ...
  'agent.create.project.cli': 'Project-level (.turbospark/agents/)',
  ...
  'agent.create.turbosparkHint':
    'Describe what this subagent should do and when it should be used. (Be comprehensive for best results)',
  'agent.create.turbosparkPlaceholder':
  ...
  'agent.create.user.cli': 'User-level (~/.turbospark/agents/)',
...
const ZH: Messages = {
  ...
  'agent.create.method.turbospark': '使用 TURBO SPARK 生成',
  'agent.create.method.turbospark.recommended': '使用 TURBO SPARK 生成（推荐）',
  'agent.create.method.turbospark.desc': 'LLM 生成名称、提示词和描述',
  'agent.create.name': '名称',
  'agent.create.namePlaceholder': 'my-agent',
  'agent.create.nameHelp': '输入此子智能体清晰且唯一的名称。',
  'agent.create.project': '项目',
  'agent.create.project.cli': '项目级 (.turbospark/agents/)',
  ...
  'agent.create.user.cli': '用户级 (~/.turbospark/agents/)',
  ...
  'help.intro':
    'TURBO SPARK 可以理解代码库，在你的许可下修改文件，并直接执行命令。',
  ...
  'settings.description.general.cleanupPeriodDays':
    '~/.turbospark/file-history/ 中用于 /rewind 的会话备份保留天数。后台清理最多每天运行一次。设为 0 表示最小保留（约 1 小时），仍会保护最近一小时触碰过的会话和当前活动会话。',
  'settings.label.general.gitCoAuthor.commit': '归因：commit',
  'settings.description.general.gitCoAuthor.commit':
    '通过 TURBO SPARK 创建 commit 时，添加 Co-authored-by trailer，并写入逐文件 AI 归因 git note。关闭后两者都会跳过。',
  'settings.label.general.gitCoAuthor.pr': '归因：PR',
  'settings.description.general.gitCoAuthor.pr':
    '运行 gh pr create 时，在 PR 描述中追加 TURBO SPARK 归因行。',
  'settings.label.general.language': '语言：界面',
  'settings.description.general.language':
    '用户界面的语言。使用 auto 可根据系统设置自动检测；也可以在 ~/.turbospark/locales/ 中放置 JS 语言文件来使用自定义语言代码。',
  ...
  'settings.description.general.preventSystemSleep':
    '当 TURBO SPARK 正在流式生成模型回复或执行工具时防止系统睡眠。空闲输入状态和权限确认状态不会阻止睡眠。',
  ...
  'settings.description.context.fileFiltering.respectGitIgnore':
    '搜索时遵守 .gitignore 文件。',
  'settings.label.context.fileFiltering.respectTurbosparkIgnore': '遵守 .turbosparkignore',
  'settings.description.context.fileFiltering.respectTurbosparkIgnore':
    '搜索时遵守 .turbosparkignore 文件。',
  ...
  'settings.description.experimental.enableCronTools':
    '启用会话内 cron/loop 工具（实验性）。启用后，模型可以用 cron_create、cron_list 和 cron_delete 创建周期性提示。也可通过 TURBOSPARK_ENABLE_CRON=1 环境变量启用。',
