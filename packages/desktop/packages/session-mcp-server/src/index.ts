...
// ============================================================
// TURBO SPARK Docs Upstream Proxy
// ============================================================
...
async function connectDocsUpstream(): Promise<void> {
  try {
    ...
    console.error(`TURBO SPARK Docs proxy connected: ${docsTools.length} tools`);
  } catch (err) {
    console.error(`TURBO SPARK Docs proxy connection failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
    docsClient = null;
    docsTools = [];
  }
}
...
async function callDocsUpstream(
  ...
  if (!docsClient) {
    return errorResponse(`TURBO SPARK Docs server is not connected. Tool '${name}' unavailable.`);
  }
