...
const ERROR_DEFINITIONS: Record<ErrorCode, Omit<AgentError, 'code' | 'originalError' | 'details'>> = {
  ...
  model_no_tool_support: {
    ...
    message: 'The selected model does not support tool/function calling, which is required for TURBO SPARK. Please choose a Qwen model with tool support.',
    actions: [
...
export function parseError(
  ...
  const providerInfo = providerContext
    ? getProviderMetadata(
        providerContext.providerType ?? 'turbospark',
      ) ?? undefined
    : undefined;
  ...
  if (code === 'model_no_tool_support') {
    ...
    if (modelMatch?.[1]) {
      return {
        code,
        ...definition,
        message: `Model "${modelMatch[1]}" does not support tool/function calling, which is required for TURBO SPARK. Please choose a different model with tool support in Settings.`,
        originalError: errorMessage,
        providerInfo,
      };
    }
