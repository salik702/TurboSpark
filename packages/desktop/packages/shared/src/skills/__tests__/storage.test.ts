 ...
 * 1. Global skills: ~/.turbospark/skills/ and ~/.agents/skills/ (lowest priority)
 * 2. Workspace skills: {workspaceRoot}/skills/ (medium priority)
 * 3. Project skills: {projectRoot}/.turbospark/skills/ and .agents/skills/ (highest priority)
 *
 * Uses real temp directories to test actual filesystem operations.
 *
 * Note: The global skills directories (~/.turbospark/skills/ and ~/.agents/skills/) are module-level constants
...
describe('loadAllSkills', () => {
  ...
  const getProjectQwenSkillsDir = () => join(projectRoot, '.turbospark', 'skills');
  ...
  it('should load Qwen project skills without requiring a Craft workspace skills directory', () => {
    ...
    createSkill(qwenProjDir, `${TEST_PREFIX}qwen_proj`, {
      ...
      description: 'From .turbospark skills',
    });
  ...
  it('should prefer .turbospark project skills over .agents project skills with the same slug', () => {
    const baselineGlobal = getExistingGlobalSlugs();
    const qwenProjDir = getProjectQwenSkillsDir();
