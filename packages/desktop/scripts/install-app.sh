...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_NAME="TURBO SPARK Desktop.app"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    if is_app_running; then
        ...
        if is_app_running; then
            ...
            pkill -9 -f "TURBO SPARK Desktop.app" 2>/dev/null || true
            pkill -9 -f "TURBO SPARK.app" 2>/dev/null || true
    ...
    if [ -d "$INSTALL_DIR/TURBO SPARK.app" ]; then
        info "Removing previous TURBO SPARK installation..."
        rm -rf "$INSTALL_DIR/TURBO SPARK.app"
    fi
    ...
    echo ""
    printf "%b\n" "  TURBO SPARK Desktop has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'TURBO SPARK Desktop'${NC}"
    echo ""
...
#!/bin/bash
# TURBO SPARK Desktop launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.craft-agent/app/Qwen-Code-Desktop-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@turbospark"
ELECTRON_CACHE_ALT="$HOME/.cache/@turbospark"

# Verify AppImage exists
if [ ! -f "$APPIMAGE_PATH" ]; then
    echo "Error: TURBO SPARK Desktop not found at $APPIMAGE_PATH"
