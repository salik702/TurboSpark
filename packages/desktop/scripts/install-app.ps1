# TURBO SPARK Desktop Windows Installer
...
$VERSIONS_URL = ""
$DOWNLOAD_DIR = "$env:TEMP\turbospark-install"
$APP_NAME = "TURBO SPARK Desktop"
...
$process = Get-Process -Name "TURBO SPARK Desktop", "TURBO SPARK" -ErrorAction SilentlyContinue
if ($process) {
    Write-Info "Closing TURBO SPARK Desktop..."
...
Write-Info "Adding 'turbospark' command to PATH..."

$binDir = "$env:LOCALAPPDATA\TURBO SPARK\bin"
$cmdFile = "$binDir\turbospark.cmd"
$exePath = "$env:LOCALAPPDATA\Programs\TURBO SPARK Desktop\TURBO SPARK Desktop.exe"
...
if ($userPath -notlike "*$binDir*") {
    ...
    Write-Success "Added to PATH (restart terminal to use 'turbospark' command)"
} else {
    Write-Success "Command 'turbospark' is ready"
}
...
Write-Host ""
Write-Host "  TURBO SPARK Desktop has been installed."
Write-Host ""
Write-Host "  Launch from:"
Write-Host "    - Start Menu or desktop shortcut"
Write-Host "    - Command line: turbospark (restart terminal first)"
Write-Host ""
}
