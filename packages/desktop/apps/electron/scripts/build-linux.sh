...
Environment variables (from .env or environment):
  TURBOSPARK_CODE_VERSION         - Optional npm version for bundled TURBO SPARK
...
echo "=== Building TURBO SPARK Desktop AppImage (${ARCH}) using electron-builder ==="
if [ "$UPLOAD" = true ]; then
...
# 4. Vendor TURBO SPARK CLI package
echo "Vendoring TURBO SPARK CLI..."
cd "$ROOT_DIR"
bun run scripts/vendor-turbospark.ts
