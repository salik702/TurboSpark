...
export default [
  ...
  // TypeScript/React files
  {
    ...
    plugins: {
      ...
      // Custom plugin for TURBO SPARK rules
      'craft-agent': {
        rules: {
  ...
  // Enforce backend abstraction boundary in Electron main process.
  {
    ...
    rules: {
      'no-restricted-imports': ['error', {
        paths: [
          { name: '@craft-agent/shared/agent/turbospark-agent', message: 'Use backend factory APIs from @craft-agent/shared/agent/backend instead.' },
        ],
      }],
    },
  },
