...
This guide explains how to configure automations in TURBO SPARK to automate workflows based on events.
...
Automations allow you to trigger actions automatically when specific events occur in TURBO SPARK. You can:
...
### App Events (triggered by TURBO SPARK)
...
Send a prompt to TURBO SPARK (creates a new session for scheduled prompts).
...
The `llmConnection` value is the slug of an LLM connection configured in AI Settings. The `model` value is a model ID supported by TURBO SPARK. If either is invalid or not found, it gracefully falls back to the workspace default. Both can be used independently or together.
...
```json
{
  ...
  "method": "POST",
  "body": { "text": "Hello from TURBO SPARK!" }
}
```
...
Ask TURBO SPARK to validate your automations configuration:
...
2. **Check env vars** — Ensure `CRAFT_WH_*` variables are set in your shell profile and TURBO SPARK was restarted after adding them. URLs using `$VAR` templates are validated after variable expansion — if the variable is empty or unset, the URL will be invalid.
