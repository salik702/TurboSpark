    ...
    variants: [
      ...
      },
      {
        ...
        props: {
          ...
          mode: 'read',
          error: "Bash command `python3 -c import re; f=open('./node_modules/@turbospark/turbospark/dist/cli.js','r'); content=f.read(); f.close(); [print(m.group(),'---') for m in list(re.finditer(r'.{0,150}AbortError.{0,150}', content))[:5]]` is not in the read-only allowlist.\n\nMatched: `python3 -` (9 chars)\nFailed at: `c` (position 9)\n\nPattern: Python 3 version",
        },
      },
    ],
    mockData: () => ({
