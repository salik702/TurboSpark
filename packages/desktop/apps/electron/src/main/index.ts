...
// Supports multi-instance dev: CRAFT_APP_NAME env var (e.g., "TURBO SPARK [1]")
...
if (!gotTheLock) {
  mainLog.warn('Another TURBO SPARK instance already owns the Electron single-instance lock; quitting.', {
    ...
    if (!isClientOnly) {
      ...
      if (process.platform === 'win32') {
        ...
        if (gitBashPath) {
          ...
          if (validation.valid) {
            process.env.TURBOSPARK_GIT_BASH_PATH = validation.path
          } else {
            clearGitBashPath()
            delete process.env.TURBOSPARK_GIT_BASH_PATH
