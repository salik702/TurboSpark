#!/usr/bin/env bun
/**
 * craft-cli — Terminal client for TURBO SPARK server.
 *
 * Connects over WebSocket (ws:// or wss://) to a running TURBO SPARK server
...
export function parseArgs(argv: string[]): CliArgs {
  ...
  if (!provider) provider = process.env.LLM_PROVIDER ?? 'turbospark'
...
function getProviderDisplayName(provider: string): string {
  return provider === 'turbospark' ? 'TURBO SPARK' : provider.charAt(0).toUpperCase() + provider.slice(1)
}
...
async function setupLlmConnection(
  ...
  const provider = 'turbospark'
  const connectionSlug = 'turbospark'
  ...
  process.stderr.write('LLM connection configured: TURBO SPARK\n')
...
async function cmdRun(args: CliArgs): Promise<void> {
  ...
  try {
    ...
    // Auto-setup the TURBO SPARK connection when no connection exists yet.
...
export interface ValidateContext {
  ...
  apiKey?: string
  /** Provider hint (from --provider, default 'turbospark') */
  provider?: string
  workspaceId?: string
  ...
  return [
    ...
    },
    {
      ...
      fn: async (client, ctx) => {
        ...
        return '0 found -> created TURBO SPARK connection'
      },
    },
    {
      name: 'sources:get',
...
function printHelp(): void {
  process.stdout.write(`craft-cli — Terminal client for TURBO SPARK server
...
LLM Configuration (for 'run' command):
  ...
  --api-key <key>        Ignored for TURBO SPARK; kept for script compatibility
  --base-url <url>       Ignored for TURBO SPARK; kept for script compatibility
