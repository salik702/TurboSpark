...
describe('resolveApiKey', () => {
  it('keeps explicit key passthrough for script compatibility', () => {
    expect(resolveApiKey('turbospark', 'unused-key')).toBe('unused-key')
  })
})

describe('shouldSetupLlmConnection', () => {
  it('sets up Qwen when no connection exists', () => {
    expect(shouldSetupLlmConnection(0, { provider: 'turbospark', baseUrl: '' })).toBe(true)
  })

  it('skips setup when a connection already exists', () => {
    expect(shouldSetupLlmConnection(2, { provider: 'turbospark', baseUrl: '' })).toBe(false)
  })
})
