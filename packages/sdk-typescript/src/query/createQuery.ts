...
export function query({
  ...
  /**
   * The prompt to send to the TURBO SPARK CLI process.
  ...
  const pathToTurbosparkExecutable = options.pathToTurbosparkExecutable;
...
function validateOptions(options: QueryOptions): SpawnInfo | undefined {
  ...
  try {
    return prepareSpawnInfo(options.pathToTurbosparkExecutable);
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    throw new Error(`Invalid pathToTurbosparkExecutable: ${errorMessage}`);
  }
}
