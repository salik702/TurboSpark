...
/**
 * Wire types for the `turbospark serve` daemon HTTP API.
 *
...
export interface DaemonCapabilities {
  ...
  /**
   * TURBO SPARK CLI/SDK version served by this daemon. Optional because this is
   * additive to v=1; older v=1 daemons omit it.
   */
   ...
   * (see `turbospark-serve-protocol.md`) makes additive optionality the
...
export interface DaemonMcpBudgetStatusCell extends DaemonStatusCell {
   ...
   * scope values, drop don't fail." See `turbospark-serve-protocol.md`.
   */
 ...
 *
 * `project` / `user` / `builtin` are the levels the `turbospark serve`
...
export type DaemonAuthProviderId = 'turbospark-oauth' | (string & {});
