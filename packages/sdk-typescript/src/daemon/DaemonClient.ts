...
} from '@turbospark/acp-bridge/mcpTimeouts';
...
/**
 * SDK-side HTTP client for the `turbospark serve` daemon. Sibling to
 ...
 * `--token` CLI fallback to `TURBOSPARK_SERVER_TOKEN` so a developer with
 * `export TURBOSPARK_SERVER_TOKEN=...` in their shell never has to thread the
 ...
 *      imported by `@turbospark/webui`; a literal
 ...
 *      `export TURBOSPARK_SERVER_TOKEN=""` would otherwise let the
...
function readTokenFromEnv(): string | undefined {
  try {
    ...
    const raw = proc?.env?.['TURBOSPARK_SERVER_TOKEN'];
...
export class DaemonClient {
  ...
  constructor(opts: DaemonClientOptions) {
    ...
    // TURBOSPARK_SERVER_TOKEN env var so clients with
    // `export TURBOSPARK_SERVER_TOKEN=...` in their shell don't have to
  ...
  /**
   * Fetch the daemon's `TURBOSPARK.md` / `AGENTS.md` snapshot. Read-only;
   ...
   * workspace root or `~/.turbospark`.
   *
   * v1 discovers files at the bound workspace ROOT only, plus the
   * user's global `~/.turbospark` directory — it does NOT walk parent
  ...
  /**
   * Append to or replace `TURBOSPARK.md` at workspace or global scope.
  ...
  /**
   * Scaffold a `TURBOSPARK.md` at the daemon's bound
