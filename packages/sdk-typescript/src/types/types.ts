...
export type TransportOptions = {
  pathToTurbosparkExecutable?: string;
  spawnInfo?: SpawnInfo;
  cwd?: string;
  model?: string;
 ...
 * - WebSocket: tcp
 *
 * This interface aligns with MCPServerConfig in @turbospark/turbospark-core.
 */
...
export interface QueryOptions {
   ...
   * - Command name (no path separators): `'turbospark'` -> executes from PATH
   ...
   *   uses TURBO SPARK's built-in prompt as the base and optionally appends extra
   ...
   * - 'turbospark-oauth': Use TURBOSPARK OAuth authentication
   *
   * Though we support 'turbospark-oauth', it's not recommended to use it in the SDK.
   * Because the credentials are stored in `~/.turbospark` and may need to refresh periodically.
   */
  authType?: AuthType;
