...
A minimum experimental TypeScript SDK for programmatic access to TURBO SPARK.
...
```typescript
import { query } from '@turbospark/sdk';
...
Creates a new query session with the TURBO SPARK.
...
| `pathToTurbosparkExecutable`   | `string`                                       | Auto-detected    | Path to the TURBO SPARK executable. Supports multiple formats: `'turbospark'` (native binary from PATH), `'/path/to/qwen'` (explicit path), `'/path/to/cli.js'` (Node.js bundle), `'node:/path/to/cli.js'` (force Node.js runtime), `'bun:/path/to/cli.js'` (force Bun runtime). If not provided, auto-detects from: `TURBOSPARK_CLI_PATH` env var, `~/.volta/bin/qwen`, `~/.npm-global/bin/qwen`, `/usr/local/bin/qwen`, `~/.local/bin/qwen`, `~/node_modules/.bin/qwen`, `~/.yarn/bin/qwen`. |
...
| `env`                    | `Record<string, string>`                       | -                | Environment variables to pass to the TURBO SPARK process. Merged with the current process environment.                                                                                                                                                                                                                                                                                                                                                                                  |
| `systemPrompt`           | `string \| QuerySystemPromptPreset`            | -                | System prompt configuration for the main session. Use a string to fully override the built-in TURBO SPARK system prompt, or a preset object to keep the built-in prompt and append extra instructions.                                                                                                                                                                                                                                                                                  |
...
| `authType`               | `'openai' \| 'turbospark-oauth'`                     | `'openai'`       | Authentication type for the AI service. Using `'turbospark-oauth'` in SDK is not recommended as credentials are stored in `~/.turbospark` and may need periodic refresh.                                                                                                                                                                                                                                                                                                                          |
...
running `turbospark serve` daemon over HTTP + SSE. It binds one daemon session so TUI,
...
```typescript
import { query, type SDKUserMessage } from '@turbospark/sdk';
...
```typescript
import { query, type CanUseTool } from '@turbospark/sdk';
...
```typescript
import { z } from 'zod';
import { query, tool, createSdkMcpServer } from '@turbospark/sdk';
...
```typescript
import { query, isAbortError } from '@turbospark/sdk';
...
```typescript
import { AbortError, isAbortError } from '@turbospark/sdk';
...
#### TURBO SPARK Installation Required

Version 0.1.0 requires [TURBO SPARK](https://github.com/turbospark/turbospark) **>= 0.4.0** to be installed separately and accessible in your PATH.

```bash
# Install TURBO SPARK globally
npm install -g @turbospark/turbospark@latest
```

**Note**: From version **0.1.1** onwards, the CLI is bundled with the SDK, so no separate TURBO SPARK installation is needed.
