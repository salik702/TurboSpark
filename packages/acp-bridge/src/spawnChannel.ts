...
export function createSpawnChannelFactory(
  ...
  return async (workspaceCwd, childEnvOverrides) => {
    ...
    childEnv['TURBOSPARK_NO_RELAUNCH'] = 'true';
 ...
 *
 * Lifted from `cli/src/serve/httpAcpBridge.ts` to `@turbospark/acp-bridge`
 ...
 *
 * `TURBOSPARK_SERVER_TOKEN`: the daemon's own bearer token, which the agent
 ...
 *
 * `TURBOSPARK_SIMPLE`: an invocation-level bare-mode override. Letting a
 ...
 *      `{ TURBOSPARK_SERVER_TOKEN: 'x' }` in overrides can't smuggle the
...
function killChild(child: ChildProcess): Promise<void> {
  return new Promise<void>((resolve) => {
    ...
    // `runTurbosparkServe` only covers `server.close()`, not the bridge.
    ...
    setTimeout(() => {
      if (!resolved) {
        process.stderr.write(
          `turbospark serve: killChild hard deadline (${KILL_HARD_DEADLINE_MS}ms) ` +
