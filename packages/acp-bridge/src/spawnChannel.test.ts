 ...
 * export of `@turbospark/acp-bridge`, channels (`packages/channels/base/
 ...
 *
 *   - `TURBOSPARK_SERVER_TOKEN` (the daemon's own bearer token) leaking into
 ...
 *   - `TURBOSPARK_SIMPLE` leaking from the daemon/IDE process into
 ...
 *     the `runTurbosparkServe.ts:216` use case).
 *
...
describe('createSpawnChannelFactory env policy', () => {
  ...
  beforeEach(() => {
    mockSpawn.mockReset();
    originalSimple = process.env['TURBOSPARK_SIMPLE'];
    originalServerToken = process.env['TURBOSPARK_SERVER_TOKEN'];
    process.argv[1] = '/tmp/qwen.js';
    process.env['TURBOSPARK_SIMPLE'] = '1';
    process.env['TURBOSPARK_SERVER_TOKEN'] = 'secret';
  });
  ...
  afterEach(() => {
    ...
    if (originalSimple === undefined) {
      ...
      process.env['TURBOSPARK_SIMPLE'] = originalSimple;
    }
    if (originalServerToken === undefined) {
      delete process.env['TURBOSPARK_SERVER_TOKEN'];
    } else {
      process.env['TURBOSPARK_SERVER_TOKEN'] = originalServerToken;
    }
  });
  ...
  it('scrubs daemon-only env vars from the spawned ACP child', async () => {
    ...
    await factory('/tmp/project', {
      TURBOSPARK_SIMPLE: '1',
      TURBOSPARK_SERVER_TOKEN: 'override-secret',
    });

    const spawnOptions = mockSpawn.mock.calls[0]?.[2] as
      | { env?: NodeJS.ProcessEnv }
      | undefined;
    expect(spawnOptions?.env).not.toHaveProperty('TURBOSPARK_SIMPLE');
    expect(spawnOptions?.env).not.toHaveProperty('TURBOSPARK_SERVER_TOKEN');
    expect(spawnOptions?.env?.['TURBOSPARK_NO_RELAUNCH']).toBe('true');
  });
});
...
const SCRUBBED = new Set<string>(['TURBOSPARK_SERVER_TOKEN', 'TURBOSPARK_SIMPLE']);
...
describe('scrubChildEnv (defaultSpawnChannelFactory env policy)', () => {
  ...
  it('strips TURBOSPARK_SERVER_TOKEN from the child env', () => {
    const source = { TURBOSPARK_SERVER_TOKEN: 'super-secret', PATH: '/usr/bin' };
    const result = scrubChildEnv(source, SCRUBBED);
    expect(result).not.toHaveProperty('TURBOSPARK_SERVER_TOKEN');
    expect(result['PATH']).toBe('/usr/bin');
  });

  it('strips TURBOSPARK_SIMPLE from the child env', () => {
    const source = { TURBOSPARK_SIMPLE: '1', PATH: '/usr/bin' };
    const result = scrubChildEnv(source, SCRUBBED);
    expect(result).not.toHaveProperty('TURBOSPARK_SIMPLE');
    expect(result['PATH']).toBe('/usr/bin');
  });
  ...
  it('overrides CANNOT re-introduce a scrubbed key (defense in depth)', () => {
    const source = { PATH: '/usr/bin' };
    const result = scrubChildEnv(source, SCRUBBED, {
      TURBOSPARK_SERVER_TOKEN: 'sneaky-attempt-via-override',
    });
    expect(result).not.toHaveProperty('TURBOSPARK_SERVER_TOKEN');
  });

  it('overrides CANNOT re-introduce TURBOSPARK_SIMPLE', () => {
    const source = { PATH: '/usr/bin' };
    const result = scrubChildEnv(source, SCRUBBED, {
      TURBOSPARK_SIMPLE: '1',
    });
  ...
  it('overrides CANNOT undo the scrub by setting undefined for a scrubbed key', () => {
    ...
    const source = { TURBOSPARK_SERVER_TOKEN: 'secret', PATH: '/usr/bin' };
    const result = scrubChildEnv(source, SCRUBBED, {
      TURBOSPARK_SERVER_TOKEN: undefined,
    });
    expect(result).not.toHaveProperty('TURBOSPARK_SERVER_TOKEN');
  });
  ...
  it('overrides are applied AFTER scrub — the denylist always wins', () => {
    ...
    const source = { TURBOSPARK_SERVER_TOKEN: 'from-process-env' };
    const result = scrubChildEnv(source, SCRUBBED, {
      TURBOSPARK_SERVER_TOKEN: 'from-override',
    });
    expect(result).not.toHaveProperty('TURBOSPARK_SERVER_TOKEN');
  });
  ...
  it('multi-key scrub set strips every listed key', () => {
    ...
    const source = {
      TURBOSPARK_SERVER_TOKEN: 't1',
      TURBOSPARK_SIMPLE: 't2',
