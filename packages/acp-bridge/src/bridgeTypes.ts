...
export interface BridgeClientRequestContext {
   ...
   * reverse-proxy `turbospark serve` should not rely on `local-only` (use a
 ...
 * method from its own in-memory queue; in `turbospark serve` the daemon answers it
...
export interface AcpSessionBridge {
   ...
   * the value `runTurbosparkServe` resolved from
