...
import type { ApprovalMode } from '@turbospark/turbospark-core';
import {
...
function writeServeDebugLine(message: string): void {
  ...
  writeStderrLine(`turbospark serve debug: ${message}`);
}
...
export function createAcpSessionBridge(opts: BridgeOptions): AcpSessionBridge {
  ...
  // `runTurbosparkServe()` handle might have mutated. Frozen to make
  ...
  // canonical value (via `canonicalizeWorkspace`). `runTurbosparkServe`
  ...
  async function killChannelWithLog(
    ...
    await ci.channel.kill().catch((err) => {
      writeStderrLine(
        `turbospark serve: channel kill failed${context ? ` (${context})` : ''}: ${String(err)}`,
      );
    });
  }
  ...
  async function startIdleTimer(
    ...
    idleTimer = setTimeout(() => {
      ...
      if (ci.sessionIds.size === 0 && ci.pendingRestoreIds.size === 0) {
        writeStderrLine(
          `turbospark serve: idle timeout (${timeoutMs}ms) expired, killing channel`,
        );
  ...
  function startSessionReaper(): void {
    if (sessionReapIntervalMs <= 0 || sessionIdleTimeoutMs <= 0) {
      writeStderrLine('turbospark serve: session reaper disabled');
      return;
    }
    writeStderrLine(
      `turbospark serve: session reaper started ` +
    ...
    sessionReaper = setInterval(() => {
      ...
      for (const [id, entry] of byId) {
        ...
        void closeSessionImpl(id, undefined, { reason: 'idle_timeout' }).catch(
          (err) => {
            writeStderrLine(
              `turbospark serve: session reaper failed to close ` +
  ...
  // (cli/serve/runTurbosparkServe.ts wraps a real `PermissionAuditRing`
  ...
  // server.listen → bridge.shutdown ordering in `runTurbosparkServe` leaves
  ...
  async function ensureChannel(): Promise<ChannelInfo> {
    ...
    const promise = (async () => {
      const channel = await telemetry.withSpan(
        'channel.spawn',
        {
          'turbospark.daemon.bridge.operation': 'channel.spawn',
          'turbospark.daemon.channel.reused': false,
        },
      ...
      const client = new BridgeClient(
        ...
        // production `turbospark serve` can wire the `WorkspaceFileSystem`
      ...
      if (aliveChannels.size > 2) {
        writeStderrLine(
          `turbospark serve: WARNING aliveChannels.size=${aliveChannels.size} ` +
      ...
      void channel.exited.then((exitInfo) => {
        ...
        // can't tell from `turbospark serve`'s own output that the agent
        // process is gone.
        //
        // Suppressed during `shuttingDown` because the operator
        // already saw "received SIGINT, draining..." from
        // `runTurbosparkServe`'s signal handler. The standalone
        ...
        if (!shuttingDown) {
          telemetry.event('channel.exited', {
            'turbospark.daemon.channel.exit_code': exitInfo?.exitCode ?? -1,
            'turbospark.daemon.channel.session_count': sessions.length,
            ...(exitInfo?.signalCode
              ? { 'turbospark.daemon.channel.signal': exitInfo.signalCode }
              : {}),
          });
          writeStderrLine(
            `turbospark serve: channel exited (code=${exitInfo?.exitCode ?? 'none'}, signal=${exitInfo?.signalCode ?? 'none'}, ${sessions.length} session(s) torn down)`,
          );
        }
      ...
      try {
        await telemetry.withSpan(
          'channel.initialize',
          {
            'turbospark.daemon.bridge.operation': 'channel.initialize',
          },
          async () =>
            await withTimeout(
  ...
  async function doSpawn(
    ...
    try {
      newSessionResp = await telemetry.withSpan(
        'session.new',
        {
          'turbospark.daemon.bridge.operation': 'session.new',
          'turbospark.daemon.session_scope': effectiveScope,
        },
        async () =>
          await withTimeout(
  ...
  const notifyAgentSessionClose = async (
    ...
    try {
      ...
      writeStderrLine(
        `turbospark serve: ${label} ACP session close notification failed ` +
  ...
  const broadcastWorkspaceEvent = (
    ...
    for (const entry of sessions) {
      ...
      try {
        ...
        if (shuttingDown) {
          ...
          writeStderrLine(`turbospark serve: ${detail}`);
        }
      }
    }
    ...
    if (eligible > 0 && successCount === 0 && !shuttingDown) {
      writeStderrLine(
        `turbospark serve: broadcastWorkspaceEvent type=${envelope.type} dropped on ALL ${failureCount} session bus(es); SSE subscribers will miss this event (GET fallback still authoritative)`,
      );
    }
  };
  ...
  async function closeSessionImpl(
    ...
    writeStderrLine(
      `turbospark serve: closing session ${JSON.stringify(sessionId)}` +
        ` (reason: ${reason})` +
        (originatorClientId
    ...
    telemetry.event('session.close', {
      'turbospark.daemon.bridge.operation': 'session.close',
    ...
    if (!ci) {
      writeStderrLine(
        `turbospark serve: closeSession channelInfoForEntry returned undefined ` +
    ...
    try {
      await telemetry.withSpan(
        ...
        {
          'turbospark.daemon.bridge.operation':
            'session.close.cancel_active_prompt',
          'session.id': sessionId,
        },
  ...
  return {
    ...
    async spawnOrAttach(req) {
      if (shuttingDown) {
        // `runTurbosparkServe.close()` calls `bridge.shutdown()` BEFORE
    ...
    sendPrompt(sessionId, req, signal, context) {
      opts.onDiagnosticLine?.(
        `turbospark serve: bridge sendPrompt for session=${sessionId}`,
        'info',
      );
        ...
        telemetry.runWithContext(capturedContext, async () => {
          ...
          try {
            return await telemetry.withSpan(
              'prompt.dispatch',
              {
                'turbospark.daemon.bridge.operation': 'prompt.dispatch',
                'session.id': sessionId,
                'turbospark.daemon.prompt.queue_wait_ms': queueWaitMs,
                ...(context?.clientId
                  ? { 'turbospark.client_id': context.clientId }
                  : {}),
              },
              async () => {
                  ...
                  .finally(() => {
                    ...
                    if (
                      ...
                      void closeSessionImpl(sessionId, undefined, {
                        ...
                        writeStderrLine(
                          `turbospark serve: deferred close-on-prompt-complete failed for ` +
    ...
    async cancelSession(sessionId, req, context) {
      opts.onDiagnosticLine?.(
        `turbospark serve: bridge cancelSession for session=${sessionId}`,
        'info',
      );
      const entry = byId.get(sessionId);
      if (!entry) throw new SessionNotFoundError(sessionId);
      ...
      await telemetry.withSpan(
        'session.cancel',
        {
          'turbospark.daemon.bridge.operation': 'session.cancel',
          'session.id': sessionId,
        },
        async () => {
          try {
    ...
    respondToPermission(requestId, response, context) {
      ...
      if (sessionId === undefined || !byId.has(sessionId)) {
        ...
        writeStderrLine(
          `turbospark serve: legacy permission vote ${JSON.stringify(requestId)} ` +
    ...
    respondToSessionPermission(sessionId, requestId, response, context) {
      ...
      if (actualSessionId === undefined) {
        writeStderrLine(
          `turbospark serve: rejected permission vote ${JSON.stringify(requestId)} ` +
    ...
    async branchSession(sessionId, req, context) {
      ...
      const branchResult = entry.promptQueue.then(async () => {
        ...
        try {
          ...
          writeStderrLine(
            `turbospark serve: branchSession resume failed for ${result.newSessionId}, attempting cleanup...`,
          );
          try {
            ...
            writeStderrLine(
              `turbospark serve: branchSession cleanup of ${result.newSessionId} failed: ${cleanupErr instanceof Error ? cleanupErr.message : cleanupErr}`,
            );
          }
          throw restoreErr;
        }
    ...
    updateSessionMetadata(sessionId, metadata, context) {
      ...
      if (metadata.displayName !== undefined) {
        ...
        if (entry.displayName !== nextDisplayName) {
          ...
          writeStderrLine(
            `turbospark serve: updated session metadata ${JSON.stringify(sessionId)} ` +
            ...
            entry.connection
              ...
              .then((res: unknown) => {
                ...
                if (r && r.persisted === false) {
                  writeStderrLine(
                    `turbospark serve: displayName for ${sessionId} was not persisted (recording service unavailable)`,
                  );
                }
              })
              .catch((err: unknown) => {
                writeStderrLine(
                  `turbospark serve: failed to persist displayName for ${sessionId}: ${
                    err instanceof Error ? err.message : String(err)
                  }`,
                );
              });
    ...
    publishWorkspaceEvent(event) {
      ...
      if (sessions.length > 0 && successCount === 0 && !shuttingDown) {
        writeStderrLine(
          `turbospark serve: publishWorkspaceEvent type=${event.type} dropped on ALL ${failureCount} session bus(es); SSE subscribers will miss this event (GET fallback still authoritative)`,
        );
      }
    },
      ...
      if (opts.persist && !persistApprovalMode) {
        throw new Error(
          'setSessionApprovalMode called with `persist: true` but no ' +
            ...
            'runTurbosparkServe wires the production callback; direct embeds ' +
    ...
    async generateSessionRecap(sessionId, _context) {
      ...
      if (!info || info.isDying) throw new SessionNotFoundError(sessionId);
      opts.onDiagnosticLine?.(
        `turbospark serve: bridge generateSessionRecap dispatching ext-method for session=${sessionId}`,
        'info',
      );
      const response = (await Promise.race([
        withTimeout(
      ...
      opts.onDiagnosticLine?.(
        `turbospark serve: bridge generateSessionRecap completed for session=${sessionId} recap=${response.recap ? `len=${response.recap.length}` : 'null'}`,
        'info',
      );
      return {
        sessionId: entry.sessionId,
    ...
    async executeShellCommand(
      ...
      opts.onDiagnosticLine?.(
        `turbospark serve: bridge executeShellCommand for session=${sessionId}`,
        'info',
      );
    ...
    async killSession(sessionId, opts) {
      ...
      if (!ci) {
        ...
        writeStderrLine(
          `turbospark serve: killSession channelInfoForEntry returned undefined ` +
            `for session ${JSON.stringify(sessionId)} — channel cleanup skipped (entry's channel already torn down)`,
        );
      }
      if (ci && ci.channel === entry.channel) {
    ...
    async detachClient(sessionId, clientId) {
      ...
      if (
        ...
        await closeSessionImpl(sessionId, undefined, {
          ...
          writeStderrLine(
            `turbospark serve: close-on-last-detach failed for ` +
              `${JSON.stringify(sessionId)}: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`,
          );
        });
      }
    ...
    async shutdown() {
      ...
      // by an in-flight HTTP connection after `runTurbosparkServe.close()`
