...
import type { HookEventName } from '@turbospark/turbospark-core';
import { SkillError } from '@turbospark/turbospark-core';
...
export interface ServeMcpBudgetStatusCell extends ServeStatusCell {
   ...
   * cap independently. See `turbospark-serve-protocol.md` "The budget feature v1
...
export interface ServeWorkspaceMemoryFile {
   ...
   * `~/.turbospark/TURBOSPARK.md` style entries. Helps adapters render scope chips.
   */
...
export interface ServeWorkspaceMemoryStatus {
  ...
  totalBytes: number;
  /**
   * Number of merged TURBOSPARK.md / AGENTS.md files the loader pulled in.
   * Mirrors `LoadServerHierarchicalMemoryResponse.fileCount`.
   */
  fileCount: number;
  /** Baseline path-rule count from `.turbospark/rules/`. */
  ruleCount: number;
  errors?: ServeStatusCell[];
}
 ...
 * Mirrors `DaemonAgentLevel` in `@turbospark/sdk` so route + SDK
...
export const IDLE_HOOK_EVENTS: Record<HookEventName, ServeHookEventMeta> = {
  ...
  Stop: { description: 'Right before TURBO SPARK concludes its response' },
  SubagentStart: {
...
export type ServeExtensionOriginSource = 'QwenCode' | 'TurbosparkCode' | 'Claude' | 'Gemini';
...
export function mapDomainErrorToErrorKind(
  ...
  // `SkillError` is defined in `@turbospark/turbospark-core/skills`; same
  ...
  // the one resolved here through acp-bridge's `@turbospark/turbospark-core`
  ...
  // `TrustGateError` is defined in `@turbospark/turbospark-core/config`; we
