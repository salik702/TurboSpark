...
/**
 * Maximum output length (10,000 characters as per TURBO SPARK spec)
 */
...
export class HttpHookRunner {
  ...
  async execute(
    ...
    try {
      ...
      try {
        ...
        // Per TURBO SPARK spec: Non-2xx status is a non-blocking error
        ...
        if (
          ...
          // Timeout or abort is a non-blocking error per TURBO SPARK spec
          debugLogger.warn(
        ...
        // Connection failure is a non-blocking error per TURBO SPARK spec
        debugLogger.warn(
   ...
   * Per TURBO SPARK spec: output is capped at 10,000 characters
   */
  ...
  private normalizeOutput(
    ...
    if ('systemMessage' in json && typeof json['systemMessage'] === 'string') {
      // Apply output length limit per TURBO SPARK spec
