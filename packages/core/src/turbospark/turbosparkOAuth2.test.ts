...
interface MockSharedTokenManager {
  getValidCredentials(qwenClient: TurbosparkOAuth2Client): Promise<QwenCredentials>;
...
vi.mock('./sharedTokenManager.js', () => ({
  SharedTokenManager: class {
    ...
    async getValidCredentials(
      qwenClient: TurbosparkOAuth2Client,
  ...
  describe('isDeviceAuthorizationSuccess', () => {
    it('should return true for successful authorization response', () => {
      const expectedBaseUrl = process.env['DEBUG']
        ? 'https://pre4-chat.turbospark.ai'
        : 'https://chat.turbospark.ai';
  ...
  describe('isErrorResponse', () => {
    ...
    it('should return false for successful responses', () => {
      const successResponse: DeviceAuthorizationResponse = {
        device_code: 'test-device-code',
        user_code: 'TEST123',
        verification_uri: 'https://chat.turbospark.ai/device',
        verification_uri_complete: 'https://chat.turbospark.ai/device?code=TEST123',
        expires_in: 1800,
      };
...
describe('TurbosparkOAuth2Client', () => {
  let client: TurbosparkOAuth2Client;
  let originalFetch: typeof global.fetch;

  beforeEach(() => {
    // Create client instance
    client = new TurbosparkOAuth2Client();
  ...
  describe('refreshAccessToken error handling', () => {
    ...
    it('should preserve non-timeout network errors and show the token endpoint', async () => {
      ...
      expect((error as Error).message).toContain(
        'https://chat.turbospark.ai/api/v1/oauth2/token',
      );
    });

    it('should NOT clear credentials on malformed 200 response (e.g. proxy HTML)', async () => {
      const { CredentialsClearRequiredError } = await import('./turbosparkOAuth2.js');
      ...
      await expect(client.refreshAccessToken()).rejects.toThrow(
        'TURBOSPARK OAuth refresh returned invalid JSON:',
      );
    });
...
describe('getQwenOAuthClient', () => {
  ...
  it('should load cached credentials if available', async () => {
    ...
    const client = await import('./turbosparkOAuth2.js').then((module) =>
  ...
  it('should handle cached credentials refresh failure', async () => {
    ...
    await expect(
      import('./turbosparkOAuth2.js').then((module) =>
        module.getQwenOAuthClient(mockConfig),
      ),
  ...
  it('should include troubleshooting hints when device auth fetch fails', async () => {
    ...
    try {
      const { getQwenOAuthClient } = await import('./turbosparkOAuth2.js');
...
describe('clearQwenCredentials', () => {
  it('should successfully clear credentials file', async () => {
    ...
    const { clearQwenCredentials } = await import('./turbosparkOAuth2.js');
...
describe('TurbosparkOAuth2Client - Additional Error Scenarios', () => {
  let client: TurbosparkOAuth2Client;
  let originalFetch: typeof global.fetch;
...
describe('Browser Launch and Error Handling', () => {
  ...
  it('should handle browser launch failure gracefully', async () => {
    ...
    expect(mockOpenBrowserSecurely).toHaveBeenCalledWith(
      'https://chat.turbospark.ai/device?code=TEST123',
    );
...
describe('Event Emitter Integration', () => {
  it('should export qwenOAuth2Events as EventEmitter', async () => {
    const { qwenOAuth2Events } = await import('./turbosparkOAuth2.js');
    expect(qwenOAuth2Events).toBeInstanceOf(EventEmitter);
  });

  it('should define correct event enum values', async () => {
    const { TurbosparkOAuth2Event } = await import('./turbosparkOAuth2.js');
    expect(TurbosparkOAuth2Event.AuthUri).toBe('auth-uri');
    expect(TurbosparkOAuth2Event.AuthProgress).toBe('auth-progress');
    expect(TurbosparkOAuth2Event.AuthCancel).toBe('auth-cancel');
  });
});
...
describe('Utility Functions', () => {
  ...
  describe('getQwenCachedCredentialPath', () => {
    it('should return correct path to cached credentials', async () => {
      ...
      const expectedPath = path.join(os.homedir(), '.turbospark', 'oauth_creds.json');
...
describe('Enhanced Error Handling and Edge Cases', () => {
  ...
  describe('TurbosparkOAuth2Client getAccessToken enhanced scenarios', () => {
...
describe('SharedTokenManager Integration in TurbosparkOAuth2Client', () => {
  let client: TurbosparkOAuth2Client;
  ...
  it('should handle TokenManagerError types correctly in getQwenOAuthClient', async () => {
    ...
    for (const errorInfo of tokenErrors) {
      ...
      try {
        await import('./turbosparkOAuth2.js').then((module) =>
          module.getQwenOAuthClient(mockConfig),
        );
      } catch {
...
describe('Constants and Configuration', () => {
  it('should have correct OAuth endpoints', async () => {
    ...
    expect(url).toBe('https://chat.turbospark.ai/api/v1/oauth2/device/code');
  });
