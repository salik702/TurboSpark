...
import type { ITurbosparkOAuth2Client } from './turbosparkOAuth2.js';
import { type QwenCredentials, type ErrorData } from './turbosparkOAuth2.js';
import type {
...
import { TurbosparkContentGenerator } from './turbosparkContentGenerator.js';
...
vi.mock('./sharedTokenManager.js', () => ({
  SharedTokenManager: class {
    ...
    async getValidCredentials(
      qwenClient: ITurbosparkOAuth2Client,
...
describe('TurbosparkContentGenerator', () => {
  let mockQwenClient: ITurbosparkOAuth2Client;
  let qwenContentGenerator: TurbosparkContentGenerator;
  ...
  beforeEach(() => {
    ...
    // Create TurbosparkContentGenerator instance
    ...
    qwenContentGenerator = new TurbosparkContentGenerator(
      mockQwenClient,
  ...
  describe('Endpoint URL Normalization', () => {
    it('should use default endpoint when no custom endpoint provided', async () => {
      ...
      parentPrototype.generateContent = vi.fn().mockImplementation(function (
        this: TurbosparkContentGenerator,
      ) {
        capturedBaseURL = (
  ...
  describe('SharedTokenManager Integration', () => {
    it('should use SharedTokenManager to get valid credentials', async () => {
      ...
      const newGenerator = new TurbosparkContentGenerator(
        mockQwenClient,
  ...
  describe('Constructor and Initialization', () => {
    it('should initialize with configured base URL when provided', () => {
      const generator = new TurbosparkContentGenerator(
        mockQwenClient,
        {
          model: 'qwen-turbo',
          authType: AuthType.QWEN_OAUTH,
