...
/**
 * Qwen Content Generator that uses TURBOSPARK OAuth tokens with automatic refresh
 */
export class TurbosparkContentGenerator extends OpenAIContentGenerator {
  private readonly debugLogger = createDebugLogger('QWEN');
  private readonly turbosparkClient: ITurbosparkOAuth2Client;
  ...
  constructor(
    ...
    this.turbosparkClient = qwenClient;
  ...
  private async executeWithCredentialManagement<T>(
    ...
    try {
      ...
      if (this.isAuthError(error)) {
        ...
        await this.sharedManager.getValidCredentials(this.turbosparkClient, true);
        return await attemptOperation();
      }
      throw error;
    }
