...
export class SharedTokenManager {
  ...
  private async performFileCheck(
    qwenClient: ITurbosparkOAuth2Client | undefined,
    checkTime: number,
  ): Promise<void> {
   ...
   */
  private async forceFileCheck(qwenClient?: ITurbosparkOAuth2Client): Promise<void> {
    try {
      const filePath = this.getCredentialFilePath();
  ...
  private getCredentialFilePath(): string {
    return path.join(Storage.getGlobalTurbosparkDir(), QWEN_CREDENTIAL_FILENAME);
  }
  ...
  private getLockFilePath(): string {
    return path.join(Storage.getGlobalTurbosparkDir(), QWEN_LOCK_FILENAME);
  }
