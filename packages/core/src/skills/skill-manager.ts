...
export class SkillManager {
  ...
  getSkillsBaseDirs(level: SkillLevel): string[] {
    switch (level) {
      ...
      case 'user':
        return SKILL_PROVIDER_CONFIG_DIRS.map((v) =>
          v === QWEN_DIR
            ? path.join(Storage.getGlobalTurbosparkDir(), SKILLS_CONFIG_DIR)
  ...
  private async listSkillsAtLevel(level: SkillLevel): Promise<SkillConfig[]> {
    ...
    // so the order defines implicit precedence (.turbospark > .agent > .cursor > ...).
  ...
  private async ensureUserSkillsDir(): Promise<void> {
    const baseDir = path.join(Storage.getGlobalTurbosparkDir(), SKILLS_CONFIG_DIR);
    try {
