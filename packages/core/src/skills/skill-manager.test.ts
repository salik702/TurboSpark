...
describe('SkillManager', () => {
  ...
  beforeEach(() => {
    ...
    // calls Storage.getGlobalTurbosparkDir() which needs os.homedir()
  ...
  const validSkillConfig: SkillConfig = {
    ...
    description: 'A test skill',
    level: 'project',
    filePath: '/test/project/.turbospark/skills/test-skill/SKILL.md',
...
`;
    ...
    it('should determine level from file path', () => {
      const projectPath = '/test/project/.turbospark/skills/test-skill/SKILL.md';
      const userPath = '/home/user/.turbospark/skills/test-skill/SKILL.md';
...
`;
  ...
  describe('listSkills', () => {
    beforeEach(() => {
      ...
      const userQwenSkillsDir = path.join('/home/user', '.turbospark', 'skills');
...
Body`);
    ...
    it('should deduplicate same-name skills across provider dirs within a level', async () => {
      // Override readdir to return the same skill name from both .turbospark and .agents dirs
      vi.mocked(fs.readdir).mockReset();
      const projectQwenDir = path.join('/test/project', '.turbospark', 'skills');
      ...
      vi.mocked(fs.readFile).mockImplementation((filePath) => {
        const pathStr = String(filePath);
        if (pathStr.includes('.turbospark') && pathStr.includes('shared-skill')) {
          return Promise.resolve(
      ...
      // Only one instance should remain, from .turbospark (first in PROVIDER_CONFIG_DIRS)
  ...
  describe('getSkillsBaseDirs', () => {
    it('should return all project-level base dirs', () => {
      ...
      expect(baseDirs).toContain(path.join('/test/project', '.turbospark', 'skills'));
    ...
    it('should return all user-level base dirs', () => {
      ...
      expect(baseDirs).toHaveLength(2);
      expect(baseDirs).toContain(path.join('/home/user', '.turbospark', 'skills'));
  ...
  describe('bundled skills', () => {
    ...
    const projectDirSegment = path.join('.turbospark', 'skills');
    const userDirSegment = path.join('.turbospark', 'skills');
...
Symlinked skill content`);
  ...
  describe('file watchers', () => {
    it('should pass ignored function and shallow depth to chokidar', async () => {
      const projectSkillsDir = path.join('/test/project', '.turbospark', 'skills');
