...
3. If **no remote matches**, use **lightweight mode**: run `gh pr diff <url>` to get the diff directly. Skip Steps 2 (no local rules), 3 (no local linter), 8 (no local files to fix), 10 (no local cache). In Step 11, skip worktree removal (none was created) but still clean up temp files (`.turbospark/tmp/qwen-review-{target}-*`). Also fetch existing PR comments using the URL's owner/repo (`gh api repos/{owner}/{repo}/pulls/{number}/comments`) to avoid duplicating human feedback. In Step 9, use the owner/repo from the URL. Inform the user: "Cross-repo review: running in lightweight mode (no build/test, no linter, no autofix)."
...
- **PR number or same-repo URL** (e.g., `123` or a URL whose owner/repo matches the current repo — cross-repo URLs are handled by the lightweight mode above):
  ...
  - **Run `qwen review fetch-pr`** to set up the working state in one pass — it cleans any stale worktree, fetches the PR HEAD into `qwen-review/pr-<n>`, queries `gh pr view` for metadata, and creates an ephemeral worktree at `.turbospark/tmp/review-pr-<n>`:

    ```bash
    qwen review fetch-pr <pr_number> <owner>/<repo> \
      --remote <remote> \
      --out .turbospark/tmp/qwen-review-pr-<pr_number>-fetch.json
    ```

    `<remote>` is the matched remote from the URL-based detection above (e.g. `upstream` for fork workflows), or `origin` by default for pure integer PR numbers. Read `.turbospark/tmp/qwen-review-pr-<n>-fetch.json` for: `worktreePath`, `baseRefName`, `headRefName`, `fetchedSha` (use as the **pre-autofix HEAD commit SHA** for Step 9), `isCrossRepository`, `diffStat` (files / additions / deletions). If the command fails (auth, network, PR not found), inform the user and stop.
  ...
  - **Incremental review check**: if `.turbospark/review-cache/pr-<n>.json` exists, read `lastCommitSha` and `lastModelId`. Compare to `fetchedSha` from the fetch report and the current model ID (`{{model}}`):
  ...
  - **Fetch PR context** (metadata + already-discussed issues) in one pass:
    ...
    qwen review pr-context <pr_number> <owner>/<repo> \
      --out .turbospark/tmp/qwen-review-pr-<pr_number>-context.md
    ```
...
qwen review load-rules <resolved_base_ref> \
  --out .turbospark/tmp/qwen-review-<target>-rules.md
```
...
The subcommand reads (in order, all sources combined): `.turbospark/review-rules.md`, then either `.github/copilot-instructions.md` or root-level `copilot-instructions.md` (only one — preferred wins), then the `## Code Review` section of `AGENTS.md`, then the `## Code Review` section of `TURBOSPARK.md`. Missing files are silently skipped. The output file is empty when no rules are found — the subcommand reports `No review rules found on <ref>` to stdout in that case; skip rule injection in Step 4.
...
1. **Bundled deterministic checks** (covers TypeScript/JavaScript, Python, Rust, Go in one call): the subcommand auto-detects each language's config files (`tsconfig.json` / eslint config / `pyproject.toml [tool.ruff]` / `Cargo.toml` / `go.mod`), runs the applicable tool on changed files (or whole project filtered to changed files for whole-project tools), parses each tool's structured output (JSON or line-based), and emits a single findings JSON:
   ...
   echo '<json array of changed files relative to worktree>' \
     > .turbospark/tmp/qwen-review-<target>-changed.json
   qwen review deterministic <worktree> \
     --changed-files .turbospark/tmp/qwen-review-<target>-changed.json \
     --out .turbospark/tmp/qwen-review-<target>-deterministic.json
   ```
...
```bash
echo '[{"path":"src/foo.ts","line":42}, ...]' > .turbospark/tmp/qwen-review-{target}-findings.json
```
...
qwen review presubmit \
  ...
  .turbospark/tmp/qwen-review-{target}-presubmit.json \
  [--new-findings .turbospark/tmp/qwen-review-{target}-findings.json]
```

Read `.turbospark/tmp/qwen-review-{target}-presubmit.json`. Schema:
...
**Build the review JSON** with `write_file` to create `.turbospark/tmp/qwen-review-{target}-review.json`. Every high-confidence Critical/Suggestion finding that can be mapped to a diff line MUST be an entry in the `comments` array:
  ...
  "comments": [
    {
      "path": "src/file.ts",
      "line": 42,
      "body": "**[Critical]** issue description\n\n```suggestion\nfix code\n```\n\n_— YOUR_MODEL_ID via TURBO SPARK /review_"
    }
  ]
}
````
...
gh api repos/{owner}/{repo}/pulls/{pr_number}/reviews \
  --input .turbospark/tmp/qwen-review-{target}-review.json
```
...
- Local changes review → `.turbospark/reviews/<YYYY-MM-DD>-<HHMMSS>-local.md`
- PR review → `.turbospark/reviews/<YYYY-MM-DD>-<HHMMSS>-pr-<number>.md`
- File review → `.turbospark/reviews/<YYYY-MM-DD>-<HHMMSS>-<filename>.md`

Include hours/minutes/seconds in the filename to avoid overwriting on same-day re-reviews.

Create the `.turbospark/reviews/` directory if it doesn't exist. **For PR worktree mode, use absolute paths to the main project directory** (not the worktree) — e.g., `mkdir -p /absolute/path/to/project/.turbospark/reviews/`. Relative paths would land inside the worktree and be deleted in Step 11.
...
1. Create `.turbospark/review-cache/` directory if it doesn't exist
2. Write `.turbospark/review-cache/pr-<number>.json` with:
...
3. Ensure `.turbospark/reviews/` and `.turbospark/review-cache/` are ignored by `.gitignore` — a broader rule like `.turbospark/*` also satisfies this. Only warn the user if those paths are not ignored at all.
...
`<target>` is the same suffix used throughout (`pr-<n>`, `local`, or filename). The command removes the worktree at `.turbospark/tmp/review-pr-<n>` (PR targets only), deletes the local branch ref `qwen-review/pr-<n>`, and clears any `.turbospark/tmp/qwen-review-<target>-*` side files (review JSON, PR context, presubmit / findings reports). It is idempotent — missing files are silent OK.

**If Step 8 flagged the worktree for preservation** (autofix commit/push failure), skip Step 11 entirely. The user needs the worktree intact to recover the autofix commit. Inform the user the worktree is preserved at `.turbospark/tmp/review-pr-<n>` and they should run `qwen review cleanup pr-<n>` manually after recovering the commit.
