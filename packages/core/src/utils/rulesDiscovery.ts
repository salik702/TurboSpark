...
//
// Discovers .turbospark/rules/ files (recursively) with optional YAML frontmatter.
...
/**
 * Discover and load rule files from a single `.turbospark/rules/` directory.
 ...
 * consistent with the `--- Context from: ... ---` format used for TURBOSPARK.md.
 */
export function formatRules(rules: RuleFile[], projectRoot: string): string {
  return rules
    .map((rule) => {
      ...
      // display paths should match — otherwise Windows shows `.turbospark\rules\foo.md`
      // and Linux shows `.turbospark/rules/foo.md`, which is confusing in diffs/tests.
...
/**
 * Load rules from both global (`~/.turbospark/rules/`) and project-level
 * (`.turbospark/rules/`) directories.
 *
...
export async function loadRules(
  ...
  // 1. Global rules: <TURBOSPARK_HOME or ~/.turbospark>/rules/
  const globalRulesDir = path.join(Storage.getGlobalTurbosparkDir(), 'rules');
  ...
  // 2. Project-level rules: <projectRoot>/.turbospark/rules/  (trusted only)
