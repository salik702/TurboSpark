...
describe('loadIgnoreRules', () => {
  ...
  it('should load rules from .turbosparkignore', async () => {
    tmpDir = await createTmpDir({
      '.turbosparkignore': '*.log',
    });
    const ignore = loadIgnoreRules({
      projectRoot: tmpDir,
      useGitignore: false,
  ...
  it('should combine rules from .gitignore and .turbosparkignore', async () => {
    tmpDir = await createTmpDir({
      '.gitignore': '*.log',
      '.turbosparkignore': '*.txt',
    });
    const ignore = loadIgnoreRules({
      projectRoot: tmpDir,
      useGitignore: true,
