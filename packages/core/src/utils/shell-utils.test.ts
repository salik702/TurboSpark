...
describe('getCommandRoots', () => {
  ...
  it('should skip leading env var assignments', async () => {
    expect(
      getCommandRoots(
        'PYTHONPATH=/Users/jinjing/.turbospark/skills/scripts python3 -c "print(1)"',
      ),
    ).toEqual(['python3']);
  });
...
describe('detectSelfKillCommand', () => {
  it('detects broad Windows taskkill patterns that target turbospark hosts', () => {
    expect(detectSelfKillCommand('taskkill /F /IM node.exe 2>nul')).toBe(true);
    expect(
      detectSelfKillCommand('taskkill /FI "IMAGENAME eq turbospark.exe" /F'),
    ).toBe(true);
  });
  ...
  it('detects broad Unix killall and pkill patterns', () => {
    ...
    expect(detectSelfKillCommand('pkill -f turbospark')).toBe(true);
  ...
  it('detects self-kill commands in chains and execution prefixes', () => {
    ...
    expect(detectSelfKillCommand('env FOO=bar pkill -f turbospark')).toBe(true);
  ...
  it('allows targeted process kills and unrelated process patterns', () => {
    ...
    expect(detectSelfKillCommand('pkill -F turbospark.pid vite')).toBe(false);
