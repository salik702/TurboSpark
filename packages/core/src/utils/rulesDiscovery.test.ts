...
Use hooks.`,
    ...
    it('reads global rules from TURBOSPARK_HOME when set', async () => {
      const customQwenHome = path.join(testRootDir, 'custom-qwen-home');
      const originalQwenHome = process.env['TURBOSPARK_HOME'];
      process.env['TURBOSPARK_HOME'] = customQwenHome;
      try {
        await createTestFile(
          path.join(customQwenHome, 'rules', 'fromCustomHome.md'),
          'CustomHome rule.',
        );
        // A stale rule in the legacy ~/.turbospark/rules location should NOT be
        // loaded once TURBOSPARK_HOME points elsewhere.
        await createTestFile(
