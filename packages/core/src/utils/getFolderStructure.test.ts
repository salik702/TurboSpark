...
`.trim();
  ...
  describe('with turbosparkignore', () => {
    it('should ignore turbosparkignore files by default', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.turbosparkignore'),
        'ignored.txt\nnode_modules/\n.gemini/\n!/.gemini/config.yaml',
      );
      await createTestFile('file1.txt');
      await createTestFile('node_modules', 'some-package', 'index.js');
    ...
    it('should not ignore files if respectTurbosparkIgnore is false', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.turbosparkignore'),
        'ignored.txt\nnode_modules/\n.gemini/\n!/.gemini/config.yaml',
      );
      ...
      const structure = await getFolderStructure(testRootDir, {
        ...
        fileFilteringOptions: {
          respectTurbosparkIgnore: false,
          respectGitIgnore: true, // Explicitly disable turbospark ignore only
        },
      });
      expect(structure).toContain('ignored.txt');
