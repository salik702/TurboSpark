...
export function estimatePartChars(
  ...
  // Tool results in turbospark carry media on `functionResponse.parts`
 ...
 * turbospark attaches media here (see
...
function transformPart(part: Part, stats: SlimStats): Part {
  ...
  // Walk into functionResponse.parts (turbospark's nested-media carrier
...
function isNonImageMime(mime: string): boolean {
  ...
  // placeholder. audio/video are rare on turbospark's tool surface and
