...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .turbosparkignore patterns even when not in a git repo', async () => {
      await createTestFile('.turbosparkignore', 'secrets.txt');
      const service = new FileDiscoveryService(projectRoot);

      expect(service.shouldTurbosparkIgnoreFile('secrets.txt')).toBe(true);
      expect(service.shouldTurbosparkIgnoreFile('src/index.js')).toBe(false);
    });
  });
  ...
  describe('filterFiles', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
      await createTestFile('.turbosparkignore', 'logs/');
    });
    ...
    it('should not filter files when respectGitIgnore is false', () => {
      ...
      const filtered = service.filterFiles(files, {
        ...
        respectTurbosparkIgnore: true, // still respect this one
      });
    ...
    it('should not filter files when respectTurbosparkIgnore is false', () => {
      const files = [
        'src/index.ts',
        'node_modules/package/index.js',
        'logs/latest.log',
  ...
  describe('shouldGitIgnoreFile & shouldTurbosparkIgnoreFile', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile('.turbosparkignore', '*.log');
    });
    ...
    it('should return true for turbospark-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldTurbosparkIgnoreFile(path.join(projectRoot, 'debug.log')),
      ).toBe(true);
    });

    it('should return false for non-turbospark-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldTurbosparkIgnoreFile(path.join(projectRoot, 'src/index.ts')),
      ).toBe(false);
    });
  });
