...
vi.mock('../turbospark/turbosparkOAuth2.js', () => ({
...
vi.mock('../turbospark/turbosparkContentGenerator.js', () => ({
  TurbosparkContentGenerator: class {},
}));
...
describe('createContentGenerator - ERR_MODULE_NOT_FOUND handling', () => {
  ...
  it('should preserve module-not-found errors from QWEN OAuth setup', async () => {
    const moduleError = new Error("Cannot find module '../turbospark/stale.js'");
    (moduleError as NodeJS.ErrnoException).code = 'ERR_MODULE_NOT_FOUND';
    ...
    try {
      ...
      expect(err.message).toMatch(/turbospark-oauth/);
      expect(err.cause).toBe(moduleError);
    }
  });
});
