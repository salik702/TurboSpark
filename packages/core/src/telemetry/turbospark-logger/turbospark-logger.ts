...
export class TurbosparkLogger {
  private static instance: TurbosparkLogger;
  ...
  static getInstance(config?: Config): TurbosparkLogger | undefined {
    if (config === undefined || !config?.getUsageStatisticsEnabled())
      return undefined;
    if (!TurbosparkLogger.instance) {
      TurbosparkLogger.instance = new TurbosparkLogger(config);
    }

    return TurbosparkLogger.instance;
  }
    ...
    try {
      ...
      if (wasAtCapacity) {
        this.debugLogger.debug(
          `TurbosparkLogger: Dropped old event to prevent memory leak (queue size: ${this.events.size})`,
        );
      }
    } catch (error) {
      this.debugLogger.error('TurbosparkLogger: Failed to enqueue log event.', error);
    }
  }
  ...
  readSourceInfo(): string {
    try {
      ...
      // Also check legacy ~/.turbospark/source.json when TURBOSPARK_HOME is set,
      // since the installer writes to ~/.turbospark/ regardless of the env var.
      const legacyPath = path.join(os.homedir(), '.turbospark', 'source.json');
      const candidates =
  ...
  async flushToRum(): Promise<LogResponse> {
    if (this.isFlushInProgress) {
      this.debugLogger.debug(
        'TurbosparkLogger: Flush already in progress, marking pending flush.',
      );
  ...
  private requeueFailedEvents(eventsToSend: RumEvent[]): void {
    ...
    this.debugLogger.debug(
      `TurbosparkLogger: Re-queued ${numEventsToRequeue} events for retry (queue size: ${this.events.size})`,
    );
  }
}
