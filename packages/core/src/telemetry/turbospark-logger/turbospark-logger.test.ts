...
import { TurbosparkLogger, TEST_ONLY } from './turbospark-logger.js';
...
describe('TurbosparkLogger', () => {
  ...
  beforeEach(() => {
    ...
    (TurbosparkLogger as any).instance = undefined;
  });
  ...
  describe('getInstance', () => {
    it('returns undefined when usage statistics are disabled', () => {
      ...
      const logger = TurbosparkLogger.getInstance(config);
      expect(logger).toBeUndefined();
    });

    it('returns an instance when usage statistics are enabled', () => {
      const logger = TurbosparkLogger.getInstance(mockConfig);
      expect(logger).toBeInstanceOf(TurbosparkLogger);
    });

    it('is a singleton', () => {
      const logger1 = TurbosparkLogger.getInstance(mockConfig);
      const logger2 = TurbosparkLogger.getInstance(mockConfig);
  ...
  describe('logHookCallEvent', () => {
    ...
    it('should log a failed hook call event with error when telemetry log prompts enabled', () => {
      ...
      const logger = TurbosparkLogger.getInstance(configWithLogPrompts)!;
      const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
    ...
    it('should not include error when telemetry log prompts disabled', () => {
      ...
      (TurbosparkLogger as unknown as { instance: undefined }).instance = undefined;
      const logger = TurbosparkLogger.getInstance(configWithoutLogPrompts)!;
      const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
    ...
    it('should sanitize hook name to remove sensitive information', () => {
      ...
      const event = new HookCallEvent(
        'PreToolUse',
        'command',
        '/home/user/.turbospark/hooks/check-secrets.sh --api-key=secret123',
        { tool_name: 'read_file' },
        100,
        true,
      );
