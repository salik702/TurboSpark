...
describe('loggers', () => {
  ...
  describe('logChatCompression', () => {
    beforeEach(() => {
      ...
      vi.spyOn(TurbosparkLogger.prototype, 'logChatCompressionEvent');
    });

    it('logs the chat compression event to TurbosparkLogger', () => {
      ...
      expect(TurbosparkLogger.prototype.logChatCompressionEvent).toHaveBeenCalledWith(
        event,
      );
    });
  ...
  describe('logRipgrepFallback', () => {
    ...
    beforeEach(() => {
      vi.spyOn(TurbosparkLogger.prototype, 'logRipgrepFallbackEvent');
    });
    ...
    it('should log ripgrep fallback event', () => {
      ...
      expect(TurbosparkLogger.prototype.logRipgrepFallbackEvent).toHaveBeenCalled();
  ...
  describe('logSkillLaunch', () => {
    ...
    beforeEach(() => {
      vi.spyOn(TurbosparkLogger.prototype, 'logSkillLaunchEvent');
    });

    it('forwards the event to TurbosparkLogger and emits an OTLP record', () => {
      const event = new SkillLaunchEvent('test-skill', true, 'prompt-id-42');

      logSkillLaunch(mockConfig, event);

      expect(TurbosparkLogger.prototype.logSkillLaunchEvent).toHaveBeenCalledWith(
        event,
      );
    ...
    it('forwards to TurbosparkLogger even when OTLP SDK is not initialized', () => {
  ...
  describe('logMalformedJsonResponse', () => {
    beforeEach(() => {
      vi.spyOn(TurbosparkLogger.prototype, 'logMalformedJsonResponseEvent');
    });
    ...
    it('logs the event to Clearcut and OTEL', () => {
      ...
      expect(
        TurbosparkLogger.prototype.logMalformedJsonResponseEvent,
  ...
  describe('logExtensionInstall', () => {
    ...
    beforeEach(() => {
      vi.spyOn(TurbosparkLogger.prototype, 'logExtensionInstallEvent');
    });
    ...
    it('should log extension install event', () => {
      ...
      expect(
        TurbosparkLogger.prototype.logExtensionInstallEvent,
      ).toHaveBeenCalledWith(event);
  ...
  describe('logExtensionUninstall', () => {
    ...
    beforeEach(() => {
      vi.spyOn(TurbosparkLogger.prototype, 'logExtensionUninstallEvent');
    });
    ...
    it('should log extension uninstall event', () => {
      ...
      expect(
        TurbosparkLogger.prototype.logExtensionUninstallEvent,
      ).toHaveBeenCalledWith(event);
  ...
  describe('logExtensionEnable', () => {
    ...
    beforeEach(() => {
      vi.spyOn(TurbosparkLogger.prototype, 'logExtensionEnableEvent');
    });
    ...
    it('should log extension enable event', () => {
      ...
      expect(TurbosparkLogger.prototype.logExtensionEnableEvent).toHaveBeenCalledWith(
        event,
      );
  ...
  describe('logExtensionDisable', () => {
    ...
    beforeEach(() => {
      vi.spyOn(TurbosparkLogger.prototype, 'logExtensionDisableEvent');
    });
    ...
    it('should log extension disable event', () => {
      ...
      expect(
        TurbosparkLogger.prototype.logExtensionDisableEvent,
      ).toHaveBeenCalledWith(event);
  ...
  describe('logHookCall', () => {
    ...
    const mockTurbosparkLogger = {
      logHookCallEvent: vi.fn(),
    };

    beforeEach(() => {
      vi.spyOn(TurbosparkLogger, 'getInstance').mockReturnValue(
        mockTurbosparkLogger as unknown as TurbosparkLogger,
      );
      mockTurbosparkLogger.logHookCallEvent.mockClear();
    });

    it('should log a successful hook call to TurbosparkLogger', () => {
      ...
      // Should call TurbosparkLogger
      expect(mockTurbosparkLogger.logHookCallEvent).toHaveBeenCalledWith(event);
    });
    ...
    it('should handle when TurbosparkLogger is not available', () => {
      vi.spyOn(TurbosparkLogger, 'getInstance').mockReturnValue(undefined);
      ...
      // Should not throw when TurbosparkLogger is not available
    ...
    it('should pass the exact event object to TurbosparkLogger', () => {
      const event = new HookCallEvent(
        'PreToolUse',
        'command',
        'test-hook.sh',
      ...
      expect(mockTurbosparkLogger.logHookCallEvent).toHaveBeenCalledTimes(1);
      const passedEvent = mockTurbosparkLogger.logHookCallEvent.mock.calls[0][0];
  ...
  describe('logApiRetry (Phase 4b)', () => {
    ...
    beforeEach(() => {
      ...
      mockTurbosparkLogger.logApiRetryEvent.mockClear();
    ...
    it('fans out to all 3 sinks: TurbosparkLogger, OTel log, and metric counter', () => {
      const mockConfig = makeFakeConfig({ sessionId: 'test-session-id' });
      const event = buildEvent();
      logApiRetry(mockConfig, event);

      // 1. TurbosparkLogger RUM
      expect(mockTurbosparkLogger.logApiRetryEvent).toHaveBeenCalledWith(event);
      ...
      expect(logRecord.attributes['event.name']).toBe('turbospark.api_retry');
    ...
    it('skips logger.emit and metric counter when SDK is not initialized (TurbosparkLogger still called)', () => {
      vi.spyOn(sdk, 'isTelemetrySdkInitialized').mockReturnValue(false);
      const mockConfig = makeFakeConfig({ sessionId: 'test-session-id' });
      const event = buildEvent();
      logApiRetry(mockConfig, event);
