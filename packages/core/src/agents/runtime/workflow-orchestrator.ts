...
export const MAX_WORKFLOW_AGENTS_ENV = 'TURBOSPARK_MAX_WORKFLOW_AGENTS';
/**
 * Absolute upper bound on the env-override agent cap. Even an operator who
 * sets `TURBOSPARK_MAX_WORKFLOW_AGENTS=999999999` cannot exceed this — the
...
/**
 * Resolve the per-run agent cap, honoring `TURBOSPARK_MAX_WORKFLOW_AGENTS`.
...
export const MAX_WORKFLOW_CONCURRENCY_ENV =
  'TURBOSPARK_MAX_WORKFLOW_CONCURRENCY';
/**
 ...
 * produce a deadlocking limit. `TURBOSPARK_MAX_WORKFLOW_CONCURRENCY` overrides
...
async function runOverridePath(
  ...
  // the just-provisioned worktree on disk under .turbospark/worktrees/. Move
 ...
 *    `<projectRoot>/.turbospark/worktrees/`; the subagent's rebound cwd.
...
export class WorkflowOrchestrator {
  ...
  async run(req: WorkflowRunRequest): Promise<WorkflowRunOutcome> {
    ...
    const countedDispatch: WorkflowAgentDispatch = (prompt, opts) => {
      ...
      // (TURBOSPARK_MAX_TOKENS_PER_WORKFLOW), fail-fast at fire time if the
