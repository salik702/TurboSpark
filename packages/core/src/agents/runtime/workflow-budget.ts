 ...
 *  - The env override `TURBOSPARK_MAX_TOKENS_PER_WORKFLOW` lets operators
 ...
 *    `TURBOSPARK_MAX_WORKFLOW_CONCURRENCY`). Operators sizing the cap
...
export const MAX_TOKENS_PER_WORKFLOW_ENV = 'TURBOSPARK_MAX_TOKENS_PER_WORKFLOW';

/**
 * Absolute upper bound on the env-override token cap. Even an operator
 * who sets `TURBOSPARK_MAX_TOKENS_PER_WORKFLOW=999999999` cannot exceed
 ...
 * `TURBOSPARK_MAX_TOKENS_PER_WORKFLOW`. Returns `null` when the env is
 ...
 * and means `TURBOSPARK_MAX_TOKENS_PER_WORKFLOW=0` does NOT disable
 * workflows; operators wanting "no agents may run" should disable the
 * tool entirely via `TURBOSPARK_DISABLE_WORKFLOWS=1` instead. An
...
export class WorkflowBudgetExceededError extends Error {
  ...
  constructor(runId: string, budgetTotal: number, spent: number) {
    ...
    // The previous "Increase TURBOSPARK_MAX_TOKENS_PER_WORKFLOW or unset
