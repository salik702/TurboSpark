  ...
  const makeStub = () => ({
    ...
    createUserWorktree: vi.fn(
      async (
        ...
        success: true,
        worktree: {
          path: `/fake/repo/.turbospark/worktrees/${slug}`,
...
describe('WorkflowOrchestrator P2 — parallel() / pipeline() / caps', () => {
  ...
  describe('pipeline()', () => {
    ...
    it('is staggered with no inter-stage barrier (item A reaches stage 2 while item B is still in stage 1)', async () => {
      const envPrev = process.env['TURBOSPARK_MAX_WORKFLOW_CONCURRENCY'];
      process.env['TURBOSPARK_MAX_WORKFLOW_CONCURRENCY'] = '2';
      try {
        ...
        if (envPrev === undefined)
          delete process.env['TURBOSPARK_MAX_WORKFLOW_CONCURRENCY'];
        else process.env['TURBOSPARK_MAX_WORKFLOW_CONCURRENCY'] = envPrev;
      }
    }, 10_000);
  });
  ...
  describe('nested fan-out (shared-window re-entrancy)', () => {
    ...
    it('a parallel() inside a pipeline() stage does not deadlock at concurrency=1', async () => {
      const prev = process.env['TURBOSPARK_MAX_WORKFLOW_CONCURRENCY'];
      process.env['TURBOSPARK_MAX_WORKFLOW_CONCURRENCY'] = '1';
      try {
        ...
        if (prev === undefined)
          delete process.env['TURBOSPARK_MAX_WORKFLOW_CONCURRENCY'];
        else process.env['TURBOSPARK_MAX_WORKFLOW_CONCURRENCY'] = prev;
      }
    }, 15_000);
  ...
  describe('env-overridable caps', () => {
    it('resolveMaxAgentsPerRun defaults to 1000 and honors a valid override', () => {
      ...
      expect(
        resolveMaxAgentsPerRun({ TURBOSPARK_MAX_WORKFLOW_AGENTS: '50' }),
      ).toBe(50);
    });

    it('resolveMaxAgentsPerRun rejects a non-integer / <1 override and falls back', () => {
      expect(
        resolveMaxAgentsPerRun({ TURBOSPARK_MAX_WORKFLOW_AGENTS: '0' }),
      ).toBe(DEFAULT_MAX_AGENTS_PER_RUN);
      expect(
        resolveMaxAgentsPerRun({ TURBOSPARK_MAX_WORKFLOW_AGENTS: 'abc' }),
      ).toBe(DEFAULT_MAX_AGENTS_PER_RUN);
      expect(
        resolveMaxAgentsPerRun({ TURBOSPARK_MAX_WORKFLOW_AGENTS: '2.5' }),
      ).toBe(DEFAULT_MAX_AGENTS_PER_RUN);
    });
    ...
    it('resolveConcurrencyLimit clamps an over-ceiling override to the hard maximum', () => {
      expect(
        resolveConcurrencyLimit({
          TURBOSPARK_MAX_WORKFLOW_CONCURRENCY: '999999',
        }),
      ).toBe(64);
      // Just under the ceiling is preserved.
      expect(
        resolveConcurrencyLimit({ TURBOSPARK_MAX_WORKFLOW_CONCURRENCY: '63' }),
      ).toBe(63);
    });

    it('TURBOSPARK_MAX_WORKFLOW_AGENTS actually lowers the cap at run time', async () => {
      const prev = process.env['TURBOSPARK_MAX_WORKFLOW_AGENTS'];
      process.env['TURBOSPARK_MAX_WORKFLOW_AGENTS'] = '3';
      try {
        const orchestrator = new WorkflowOrchestrator(async () => 'ok');
        const outcome = await orchestrator.run({
          script: `return await parallel(
        ...
        if (prev === undefined)
          delete process.env['TURBOSPARK_MAX_WORKFLOW_AGENTS'];
        else process.env['TURBOSPARK_MAX_WORKFLOW_AGENTS'] = prev;
      }
    });
  });
});
...
describe('WorkflowOrchestrator P3 — agentType / model / isolation / schema', () => {
  ...
  it("isolation:'worktree' refuses when parent cwd is already inside a worktree", async () => {
    ...
    (config as unknown as { getTargetDir: () => string }).getTargetDir = () =>
      '/some/repo/.turbospark/worktrees/agent-existing/inner';
    const dispatch = createProductionDispatch(config);
