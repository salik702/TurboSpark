...
> {
  ...
  override async getDefaultPermission(): Promise<PermissionDecision> {
    ...
    if (
      ...
      // (per-project + user-level under ~/.turbospark/memories/) — never the
      // broad getMemoryBaseDir() — to avoid exposing sensitive ~/.turbospark
...
> {
  ...
  protected override validateToolParamValues(
    ...
    if (fileService.shouldIgnoreFile(params.file_path)) {
      return `File path '${filePath}' is ignored by .turbosparkignore pattern(s).`;
    }
