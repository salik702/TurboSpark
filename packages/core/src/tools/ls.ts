...
export interface LSToolParams {
  ...
  /**
   * Whether to respect .gitignore and .turbosparkignore patterns (optional, defaults to true)
   */
  file_filtering_options?: {
    respect_git_ignore?: boolean;
    respect_turbospark_ignore?: boolean;
  };
}
...
class LSToolInvocation extends BaseToolInvocation<LSToolParams, ToolResult> {
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    try {
      ...
      const { filteredPaths, gitIgnoredCount, turbosparkIgnoredCount } =
        fileDiscovery.filterFilesWithReport(relativePaths, {
          ...
          respectTurbosparkIgnore:
            this.params.file_filtering_options?.respect_turbospark_ignore ??
            this.config.getFileFilteringOptions().respectTurbosparkIgnore ??
            DEFAULT_FILE_FILTERING_OPTIONS.respectTurbosparkIgnore,
        });
      ...
      if (turbosparkIgnoredCount > 0) {
        ignoredMessages.push(`${turbosparkIgnoredCount} turbospark-ignored`);
      }
    ...
    super(
      ...
      Kind.Search,
      {
        properties: {
          ...
          file_filtering_options: {
            description:
              'Optional: Whether to respect ignore patterns from .gitignore or .turbosparkignore',
            type: 'object',
            properties: {
              respect_git_ignore: {
                description:
              ...
              respect_turbospark_ignore: {
                description:
                  'Optional: Whether to respect .turbosparkignore patterns when listing files. Defaults to true.',
                type: 'boolean',
              },
            },
          },
