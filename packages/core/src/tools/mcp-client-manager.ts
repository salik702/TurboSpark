 ...
 * doesn't pass one. Daemon-mode (`turbospark serve`) sets these when
...
function readBudgetFromEnv(): McpBudgetConfig {
  ...
  if (rawBudget !== undefined && rawBudget !== '') {
    ...
    if (Number.isFinite(parsed) && Number.isInteger(parsed) && parsed > 0) {
      ...
      // CLI parent (`commands/serve.ts` + `runTurbosparkServe.ts`)
      ...
      process.stderr.write(
        `turbospark serve: ignoring invalid QWEN_SERVE_MCP_CLIENT_BUDGET=` +
  ...
  if (rawMode === 'enforce' || rawMode === 'warn' || rawMode === 'off') {
    ...
    if (rawMode !== undefined && rawMode !== '') {
      ...
      // typo is visible.
      process.stderr.write(
        `turbospark serve: ignoring invalid QWEN_SERVE_MCP_BUDGET_MODE=` +
  ...
  // disabled. The CLI handler + `runTurbosparkServe` path both throw on
  ...
  if (
    ...
    process.stderr.write(
      `turbospark serve: QWEN_SERVE_MCP_BUDGET_MODE=${budgetMode} requires ` +
...
export class McpClientManager {
  ...
  constructor(
    ...
    // set by `turbospark serve --mcp-client-budget=N --mcp-budget-mode=X`
    ...
    // production callers (CLI handler, `runTurbosparkServe`, env-var
    ...
    if (
      ...
      process.stderr.write(
        `turbospark serve: McpClientManager constructed with budgetMode=${resolvedMode} ` +
  ...
  private refuseAndLog(
    ...
    process.stderr.write(
      `turbospark serve: MCP server '${serverName}' refused (budget exhausted, ` +
    ...
    try {
      ...
      const acquirePromises = Object.entries(servers).map(
        async ([name, config]) => {
          ...
          // constructed in `TurbosparkAgent` ctor without it. Routing SDK
