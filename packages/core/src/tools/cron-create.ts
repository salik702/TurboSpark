  ...
  async execute(): Promise<ToolResult> {
    ...
    try {
      ...
      const where = durable
        ...
        : 'Session-only (not written to disk, dies when TURBO SPARK exits)';
  ...
  constructor(private config: Config) {
    super(
      ...
      'Schedule a prompt to be enqueued at a future time. Use for both recurring schedules and one-shot reminders.\n\n' +
        ...
        '## Durability\n\n' +
        'By default (durable: false) the job lives only in this TURBO SPARK session — nothing is written to disk, and the job is gone when TURBO SPARK exits. ' +
      ...
      Kind.Other,
      {
        ...
        properties: {
          ...
          durable: {
            type: 'boolean',
            description: `true = persist to ${CRON_TASKS_DISPLAY_PATH} and survive restarts. false (default) = in-memory only, dies when TURBO SPARK exits. Use true only when the user asks the task to survive across sessions.`,
          },
        },
