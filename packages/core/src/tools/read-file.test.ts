  ...
  beforeEach(async () => {
    ...
    const mockConfigInstance = {
      ...
      storage: {
        ...
        getUserSkillsDirs: () => [path.join(os.homedir(), '.turbospark', 'skills')],
      },
  ...
  describe('execute', () => {
    ...
    describe('with FileReadCache', () => {
      ...
      it('records an auto-memory read in the cache so a follow-up Edit can pass enforcement', async () => {
        ...
        // read. Put a file under .turbospark/<auto-memory>/ via
        // TURBOSPARK_MEMORY_LOCAL=1 and assert recordRead happened.
        const previousLocal = process.env['TURBOSPARK_MEMORY_LOCAL'];
        ...
        try {
          ...
          if (previousLocal === undefined) {
            ...
            process.env['TURBOSPARK_MEMORY_LOCAL'] = previousLocal;
          }
        }
      });
      ...
      it('completely bypasses the cache when getFileReadCacheDisabled() is true', async () => {
        ...
        const disabledConfig = {
          ...
          storage: {
            ...
            getProjectDir: () => path.join(tempRootDir, '.project'),
            getUserSkillsDirs: () => [
              path.join(os.homedir(), '.turbospark', 'skills'),
            ],
          },
          getTruncateToolOutputThreshold: () => 2500,
          getTruncateToolOutputLines: () => 500,
    ...
    describe('with .turbosparkignore', () => {
      beforeEach(async () => {
        await fsp.writeFile(
      ...
      it('should throw error if path is ignored by a .turbosparkignore pattern', async () => {
        const ignoredFilePath = path.join(tempRootDir, 'foo.bar');
        await fsp.writeFile(ignoredFilePath, 'content', 'utf-8');
        const params: ReadFileToolParams = {
          file_path: ignoredFilePath,
        };
        const expectedError = `File path '${ignoredFilePath}' is ignored by .turbosparkignore pattern(s).`;
