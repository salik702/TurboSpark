...
/**
 * Per-process cache for `.turbosparkignore` discovery. The same directories show
 ...
 * `qwenIgnore`: dir → string | null (cached `.turbosparkignore` path or null)
 *
 * **Known staleness window:** a `.turbosparkignore` created mid-session, or a
...
> {
  ...
  private async performRipgrepSearch(options: {
    ...
    // Add file exclusions from .gitignore and .turbosparkignore
    const filteringOptions = this.getFileFilteringOptions();
    if (!filteringOptions.respectGitIgnore) {
      rgArgs.push('--no-ignore-vcs');
    }

    if (filteringOptions.respectTurbosparkIgnore) {
      // Load .turbosparkignore from each workspace directory, not just the primary one
      ...
      for (const searchPath of paths) {
        ...
        if (qwenIgnorePath === undefined) {
          const candidate = path.join(dir, '.turbosparkignore');
  ...
  private getFileFilteringOptions(): FileFilteringOptions {
    ...
    return {
      ...
      respectTurbosparkIgnore:
        options?.respectTurbosparkIgnore ??
