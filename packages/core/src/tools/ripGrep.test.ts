...
describe('RipGrepTool', () => {
  ...
  describe('execute', () => {
    ...
    it('should pass .turbosparkignore to ripgrep when respected', async () => {
      await fs.writeFile(
        path.join(tempRootDir, '.turbosparkignore'),
        'ignored.txt\n',
      );
      (runRipgrep as Mock).mockResolvedValue({
        stdout: '',
    ...
    it('should include .turbosparkignore matches when disabled in config', async () => {
      await fs.writeFile(path.join(tempRootDir, '.turbosparkignore'), 'kept.txt\n');
  ...
  describe('multi-directory workspace', () => {
    ...
    it('should load .turbosparkignore from each workspace directory', async () => {
      const secondDir = await fs.mkdtemp(
        path.join(os.tmpdir(), 'grep-tool-second-'),
      );
      await fs.writeFile(path.join(secondDir, '.turbosparkignore'), 'ignored.txt\n');
      await fs.writeFile(
        path.join(tempRootDir, '.turbosparkignore'),
        'other-ignored.txt\n',
      );
      ...
      // Verify both .turbosparkignore files were passed
      ...
      expect(ignoreFileArgs).toContain(path.join(tempRootDir, '.turbosparkignore'));
      expect(ignoreFileArgs).toContain(path.join(secondDir, '.turbosparkignore'));
