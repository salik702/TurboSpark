...
describe('ExtensionStorage', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(Storage).mockImplementation(
      () =>
        ({
          getExtensionsDir: () => path.join(mockHomeDir, '.turbospark', 'extensions'),
        }) as any, // eslint-disable-line @typescript-eslint/no-explicit-any
    );
    vi.mocked(Storage.getUserExtensionsDir).mockReturnValue(
      path.join(mockHomeDir, '.turbospark', 'extensions'),
    );
  ...
  it('should return the correct user extensions directory', () => {
    const expectedDir = path.join(mockHomeDir, '.turbospark', 'extensions');
