...
describe('substituteHookVariables', () => {
  ...
  it('should handle multiple event types with hooks', () => {
    const basePath = '/home/user/.turbospark/extensions/my-extension';
    ...
    expect(
      (result!['PreToolUse']![0].hooks![0] as { command: string }).command,
    ).toBe('/home/user/.turbospark/extensions/my-extension/scripts/pre-start.sh');
    expect(result!['UserPromptSubmit']).toHaveLength(1);
    expect(
      (result!['UserPromptSubmit']![0].hooks![0] as { command: string })
        .command,
    ).toBe('/home/user/.turbospark/extensions/my-extension/setup/install.py');
  });
...
describe('performVariableReplacement', () => {
  ...
  it('should replace .claude with .turbospark in markdown files', () => {
    const extDir = path.join(testDir, 'ext');
    fs.mkdirSync(extDir, { recursive: true });
    ...
    expect(result).toContain('.turbospark/loop.local.md');
    expect(result).toContain('rm .turbospark/loop.local.md');
    expect(result).toContain('$HOME/.turbospark/cache');
  ...
  it('should replace .claude with .turbospark in shell scripts', () => {
    ...
    expect(result).toContain('./.turbospark/local');
    expect(result).toContain('.turbospark/config');
    // Note: URLs are also being replaced in current implementation
    expect(result).toContain('https://example.com/.turbospark/page');
  });
