...
describe('Config workflows feature gate', () => {
  ...
  afterEach(() => {
    ...
    for (const key of [
      'TURBOSPARK_ENABLE_WORKFLOWS',
      'TURBOSPARK_DISABLE_WORKFLOWS',
    ]) {
  ...
  it('defaults to disabled', () => {
    delete process.env['TURBOSPARK_ENABLE_WORKFLOWS'];
    delete process.env['TURBOSPARK_DISABLE_WORKFLOWS'];
    const cfg = new Config({ ...baseParams });
    expect(cfg.isWorkflowsEnabled()).toBe(false);
  });

  it('respects TURBOSPARK_ENABLE_WORKFLOWS=1', () => {
    delete process.env['TURBOSPARK_DISABLE_WORKFLOWS'];
    process.env['TURBOSPARK_ENABLE_WORKFLOWS'] = '1';
    const cfg = new Config({ ...baseParams });
  ...
  it('TURBOSPARK_DISABLE_WORKFLOWS=1 overrides everything', () => {
    process.env['TURBOSPARK_DISABLE_WORKFLOWS'] = '1';
    process.env['TURBOSPARK_ENABLE_WORKFLOWS'] = '1';
    const cfg = new Config({ ...baseParams });
