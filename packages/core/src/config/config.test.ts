...
vi.mock('../memory/const.js', () => ({
  ...
  getCurrentGeminiMdFilename: vi.fn(() => 'TURBOSPARK.md'), // Mock the original filename
  getAllGeminiMdFilenames: vi.fn(() => ['TURBOSPARK.md', 'AGENTS.md']),
  DEFAULT_CONTEXT_FILENAME: 'TURBOSPARK.md',
}));
vi.mock('../tools/memory-config', () => ({
  setGeminiMdFilename: vi.fn(),
  getCurrentGeminiMdFilename: vi.fn(() => 'TURBOSPARK.md'),
  getAllGeminiMdFilenames: vi.fn(() => ['TURBOSPARK.md', 'AGENTS.md']),
  DEFAULT_CONTEXT_FILENAME: 'TURBOSPARK.md',
...
describe('Server Config (config.ts)', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(TurbosparkLogger.prototype, 'logStartSessionEvent').mockImplementation(
      async () => undefined,
    );
  ...
  it('wires file history snapshot updates to chat recording', async () => {
    ...
    const getGlobalTurbosparkDirSpy = vi
  ...
  describe('initialize', () => {
    ...
    it('honors TURBOSPARK_LEGACY_MCP_BLOCKING=1 by running MCP discovery inline', async () => {
      const originalLegacy = process.env['TURBOSPARK_LEGACY_MCP_BLOCKING'];
      ...
      try {
        ...
        if (originalLegacy === undefined) {
          ...
          process.env['TURBOSPARK_LEGACY_MCP_BLOCKING'] = originalLegacy;
        }
      }
    });
  ...
  describe('model switching optimization (QWEN_OAUTH)', () => {
    it('should switch turbospark-oauth model in-place without refreshing auth when safe', async () => {
      const config = new Config(baseParams);
      ...
      // Establish initial turbospark-oauth content generator config/content generator.
  ...
  it('refreshHierarchicalMemory should append managed auto-memory index when present', async () => {
    ...
    vi.mocked(loadServerHierarchicalMemory).mockResolvedValue({
      memoryContent: '--- Context from: TURBOSPARK.md ---\nProject rules',
      fileCount: 1,
      ruleCount: 0,
      conditionalRules: [],
      projectRoot: '/tmp',
  ...
  it('refreshHierarchicalMemory should include appended auto-memory in the context warning estimate', async () => {
    ...
    expect(config.getWarnings()).toContainEqual(
      expect.stringContaining('Loaded TURBOSPARK.md/context instructions'),
    );
  });
  ...
  it('refreshHierarchicalMemory should not warn for small always-loaded context', async () => {
    ...
    expect(
      config
        .getWarnings()
        .some((warning) =>
          warning.includes('Loaded TURBOSPARK.md/context instructions'),
        ),
    ).toBe(false);
  });
  ...
  it('refreshHierarchicalMemory should fire InstructionsLoaded hooks from memory notifications', async () => {
    ...
    await options?.onInstructionsLoaded?.({
      filePath: '/tmp/project/TURBOSPARK.md',
      memoryType: 'project',
      loadReason: 'include',
  ...
  describe('Usage Statistics', () => {
    ...
    it('logs the session start event', async () => {
      ...
      expect(TurbosparkLogger.prototype.logStartSessionEvent).toHaveBeenCalledOnce();
    });
  });
  ...
  // setter (proves TurbosparkAgent calls it pre-`initialize`); the manager
...
describe('Model Switching and Config Updates', () => {
  ...
  it('should trigger full refresh when switching to non-turbospark-oauth provider', async () => {
    const config = new Config(baseParams);

    // Initialize with turbospark-oauth
    const initialConfig: ContentGeneratorConfig = {
      ['model']: 'qwen3-coder-plus',
      ['authType']: AuthType.QWEN_OAUTH,
      ['apiKey']: 'test-key',
