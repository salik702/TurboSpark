...
import { DEFAULT_EMBEDDING_MODEL } from './models.js';
...
export interface ChatCompressionSettings {
   ...
   * Env override: `TURBOSPARK_IMAGE_TOKEN_ESTIMATE`.
   */
   ...
   * Env override: `TURBOSPARK_COMPACT_MAX_RECENT_FILES`.
   */
   ...
   * Env override: `TURBOSPARK_COMPACT_MAX_RECENT_IMAGES`.
   */
   ...
   * Env override: `TURBOSPARK_COMPACT_SCREENSHOT_TRIGGER` (`1`/`true`/`0`/`false`).
   */
   ...
   * Env override: `TURBOSPARK_COMPACT_SCREENSHOT_THRESHOLD`.
   */
 ...
 * data turbospark writes into outbound LLM API requests.
 *
 ...
 * the turbospark process and INTO third-party LLM provider request
  ...
  const pickBool = (v: unknown, fieldName: string): boolean => {
    ...
    if (typeof v === 'string') {
      ...
      if (!knownDisable.includes(lowered)) {
        ...
        // can see the actual coercion in TURBOSPARK_DEBUG_LOG_FILE.
...
export type ExtensionOriginSource = 'TurbosparkCode' | 'Claude' | 'Gemini';
 ...
 *   (a workspace `.turbospark/settings.json`) are checked-in / shareable and therefore
...
export interface WorktreeSettings {
   ...
   * resolve to git-internal paths (`.git`, `.turbospark`) are also rejected
...
export interface AgentsCollabSettings {
  ...
  arena?: {
    /** Custom base directory for Arena worktrees (default: ~/.turbospark/arena) */
...
export interface ConfigParameters {
   ...
   * the `TURBOSPARK_DISABLED_SLASH_COMMANDS` environment variable.
   */
   ...
   * Issue: turbospark/turbospark#4103.
   */
   ...
   * When set, TURBO SPARK outputs structured JSON events to this fd while
   ...
   * Only meaningful in headless mode (`turbospark -p`).
   */
  ...
  /** Glob patterns to exclude from .turbospark/rules/ loading. */
...
function loadMemoryPressureConfig(): MemoryPressureConfig {
  ...
  try {
    ...
    const fallbackMsg =
      '[TURBOSPARK] WARNING: Invalid memory pressure config; using defaults. ' +
...
// TURBOSPARK_SESSION_ID env var. Prevents throwaway Config instances from
// overwriting the real session's ID while still allowing nested turbospark
...
export class Config {
   ...
   * `TURBOSPARK_LEGACY_MCP_BLOCKING=1` and racing against background
  ...
  constructor(params: ConfigParameters) {
    ...
    // rather than checking env existence — otherwise a nested turbospark
    // launched from within a session would inherit the parent's ID and
    // never claim its own.
    if (!sessionEnvClaimed && process.env) {
      process.env['TURBOSPARK_SESSION_ID'] = this.sessionId;
    ...
    this.fileFiltering = {
      ...
      respectTurbosparkIgnore: params.fileFiltering?.respectTurbosparkIgnore ?? true,
  ...
  async initialize(options?: ConfigInitializeOptions): Promise<void> {
    ...
    // behavior with `TURBOSPARK_LEGACY_MCP_BLOCKING=1` — kept ≥ 1 release as
    // an escape hatch.
    ...
    // scan `<subdir>/.turbospark/worktrees/` — which never exists — and the
    // sweep would silently be a no-op forever.
    if (!this.getBareMode()) {
      void (async () => {
        try {
          // Resolve the repo top-level FIRST. The previous code bailed
          // on `fs.access(<targetDir>/.turbospark/worktrees)` before resolving,
          ...
          const worktreesDir = path.join(root, '.turbospark', 'worktrees');
          try {
   ...
   * - `TURBOSPARK_LEGACY_MCP_BLOCKING=1` is set (MCP already discovered
  ...
  async refreshHierarchicalMemory(
    ...
    if (this.getManagedAutoMemoryEnabled()) {
      ...
      // `~/.turbospark/memories/MEMORY.md` must not strip the whole managed-memory
  ...
  private buildMemoryContextWarning(memoryContent: string): string | undefined {
    ...
    return (
      `Warning: Loaded TURBOSPARK.md/context instructions use about ` +
  ...
  private async handleModelChange(
    ...
    // Hot update path: only supported for turbospark-oauth.
    // For other auth types we always refresh to recreate the ContentGenerator.
    //
    // Rationale:
    // - Non-turbospark providers may need to re-validate credentials / baseUrl / envKey.
    ...
    if (authType === AuthType.TURBOSPARK_OAUTH && !requiresRefresh) {
      const { config, sources } = resolveContentGeneratorConfigWithSources(
        this,
        authType,
        this.modelsConfig.getGenerationConfig(),
      ...
      // Hot-update fields (turbospark-oauth models share the same auth + client).
   ...
   * For turbospark-oauth model switches that are hot-update safe, this may update in place.
   *
   ...
   * Intended for the `turbospark serve` mutation surface
   * (`setWorkspaceToolEnabled` → ACP `turbospark/control/...` → here): the
   ...
   * injected by the daemon-mode `TurbosparkAgent`. When set, the wrapping
   ...
   * own per-session `McpClient`. Standalone `turbospark` (non-daemon) leaves
   * this `undefined` and the manager keeps its previous behavior.
   *
   * Eagerly instantiated by `TurbosparkAgent` (per Q6 resolved); the
  ...
  isCronEnabled(): boolean {
    if (process.env['TURBOSPARK_DISABLE_CRON'] === '1') return false;
    return this.cronEnabled;
  }

  isAgentTeamEnabled(): boolean {
    // Agent team is experimental and opt-in: enabled via settings or env var
    if (process.env['TURBOSPARK_ENABLE_AGENT_TEAM'] === '1') return true;
    return this.agentTeamEnabled;
  }

  isWorkflowsEnabled(): boolean {
    // Workflows are experimental and opt-in: enabled via settings or env var
    // P1 also honors a kill switch: TURBOSPARK_DISABLE_WORKFLOWS=1 forces off
    if (process.env['TURBOSPARK_DISABLE_WORKFLOWS'] === '1') return false;
    if (process.env['TURBOSPARK_ENABLE_WORKFLOWS'] === '1') return true;
   ...
   * `TURBOSPARK_MAX_TOKENS_PER_WORKFLOW` env knob. The registry-side
   ...
   * with the `TURBOSPARK_COMPUTER_USE_MAX_IMAGE_DIMENSION` env override at the point
   ...
   *
   * Env overrides (either direction): `TURBOSPARK_EMIT_TOOL_USE_SUMMARIES=0`
   * to force off, `=1` to force on.
   */
  getEmitToolUseSummaries(): boolean {
    const env = process.env['TURBOSPARK_EMIT_TOOL_USE_SUMMARIES'];
  ...
  getFileFilteringRespectTurbosparkIgnore(): boolean {
    return this.fileFiltering.respectTurbosparkIgnore;
  }

  getFileFilteringOptions(): FileFilteringOptions {
    return {
      respectGitIgnore: this.fileFiltering.respectGitIgnore,
      respectTurbosparkIgnore: this.fileFiltering.respectTurbosparkIgnore,
    };
  }
