...
import { DEFAULT_MODEL, MAINLINE_CODER_MODEL } from '../config/models.js';
...
export const AUTH_ENV_MAPPINGS = {
  openai: {
    ...
    model: ['OPENAI_MODEL', 'TURBOSPARK_MODEL'],
  },
  anthropic: {
...
export const DEFAULT_MODELS = {
  ...
  'turbospark-oauth': DEFAULT_MODEL,
} as Partial<Record<AuthType, string>>;

/**
 * Hard-coded TURBOSPARK OAuth models that are always available.
 * These cannot be overridden by user configuration.
 */
export const TURBOSPARK_OAUTH_MODELS: ModelConfig[] = [
  {
    id: 'coder-model',
    name: 'coder-model',
    description:
      'Efficient hybrid model with leading coding performance',
    capabilities: { vision: true },
  },
];

/**
 * Derive allowed models from TURBOSPARK_OAUTH_MODELS for authorization.
 * This ensures single source of truth (SSOT).
 */
export const TURBOSPARK_OAUTH_ALLOWED_MODELS = TURBOSPARK_OAUTH_MODELS.map(
  (model) => model.id,
) as readonly string[];
