...
import { TURBOSPARK_OAUTH_MODELS } from './constants.js';
import { createDebugLogger } from '../utils/debugLogger.js';

const debugLogger = createDebugLogger('MODEL_REGISTRY');

export { TURBOSPARK_OAUTH_MODELS } from './constants.js';
export { TURBOSPARK_OAUTH_MODELS as QWEN_OAUTH_MODELS } from './constants.js';
...
export class ModelRegistry {
  ...
  private getDefaultBaseUrl(authType: AuthType): string {
    switch (authType) {
      case AuthType.TURBOSPARK_OAUTH:
        return 'DYNAMIC_TURBOSPARK_OAUTH_BASE_URL';
  ...
  constructor(modelProvidersConfig?: ModelProvidersConfig) {
    ...
    // Always register turbospark-oauth models (hard-coded, cannot be overridden)
    this.registerAuthTypeModels(AuthType.TURBOSPARK_OAUTH, TURBOSPARK_OAUTH_MODELS);
    ...
    if (modelProvidersConfig) {
      for (const [rawKey, models] of Object.entries(modelProvidersConfig)) {
        ...
        // Skip turbospark-oauth as it uses hard-coded models
   ...
   * For turbospark-oauth, returns the coder model.
  ...
  getDefaultModelForAuthType(
    ...
    if (authType === AuthType.TURBOSPARK_OAUTH) {
      return this.getModel(authType, DEFAULT_MODEL);
    }
    const models = this.modelsByAuthType.get(authType);
   ...
   * Preserves hard-coded turbospark-oauth models.
   */
  reloadModels(modelProvidersConfig?: ModelProvidersConfig): void {
    // Clear existing user-configured models (preserve turbospark-oauth)
