...
describe('ModelRegistry', () => {
  describe('initialization', () => {
    it('should always include hard-coded turbospark-oauth models', () => {
    ...
    it('should ignore turbospark-oauth models in config (hard-coded)', () => {
      const modelProvidersConfig: ModelProvidersConfig = {
        'turbospark-oauth': [
          {
            id: 'custom-qwen',
      ...
      // Should still use hard-coded turbospark-oauth models
      const qwenModels = registry.getModelsForAuthType(AuthType.QWEN_OAUTH);
      expect(qwenModels.length).toBe(QWEN_OAUTH_MODELS.length);
  ...
  describe('getDefaultModelForAuthType', () => {
    it('should return coder-model for turbospark-oauth', () => {
      const registry = new ModelRegistry();
  ...
  describe('default base URLs', () => {
    it('should apply default dashscope URL for turbospark-oauth', () => {
      const registry = new ModelRegistry();
  ...
  describe('reloadModels', () => {
    ...
    it('should preserve hard-coded turbospark-oauth models after reload', () => {
      const registry = new ModelRegistry({
        openai: [{ id: 'gpt-4', name: 'GPT-4' }],
      });
    ...
    it('should ignore turbospark-oauth models in reload config', () => {
      const registry = new ModelRegistry();

      registry.reloadModels({
        'turbospark-oauth': [{ id: 'custom-qwen', name: 'Custom Qwen' }],
      });

      // turbospark-oauth should still use hard-coded models
      const qwenModels = registry.getModelsForAuthType(AuthType.QWEN_OAUTH);
      expect(qwenModels.length).toBe(QWEN_OAUTH_MODELS.length);
      expect(qwenModels.find((m) => m.id === 'custom-qwen')).toBeUndefined();
    });
