...
describe('ModelsConfig', () => {
  ...
  it('should always force TURBOSPARK OAuth apiKey placeholder when applying model defaults', async () => {
    ...
    // Switching within turbospark-oauth triggers applyResolvedModelDefaults().
  ...
  it('should apply TURBOSPARK OAuth apiKey placeholder during syncAfterAuthRefresh for fresh users', () => {
  ...
  it('should use default model for new authType when switching from different authType with env vars', () => {
    ...
    // User switches to turbospark-oauth via AuthDialog
    // refreshAuth calls syncAfterAuthRefresh with the current model (gpt-4o)
    // which doesn't exist in turbospark-oauth registry, so it should use default
    modelsConfig.syncAfterAuthRefresh(AuthType.QWEN_OAUTH, 'gpt-4o');

    const gc = currentGenerationConfig(modelsConfig);
    // Should use default turbospark-oauth model (coder-model), not the OPENAI model
    expect(gc.model).toBe('coder-model');
    expect(gc.apiKey).toBe('QWEN_OAUTH_DYNAMIC_TOKEN');
    expect(gc.apiKeyEnvKey).toBeUndefined();
  });
  ...
  it('should clear manual credentials when switching from USE_OPENAI to QWEN_OAUTH', () => {
    ...
    // User switches to turbospark-oauth
    // Since authType is not USE_OPENAI, manual credentials should be cleared
    // and default turbospark-oauth model should be applied
    modelsConfig.syncAfterAuthRefresh(AuthType.QWEN_OAUTH, 'gpt-4o');

    const gc = currentGenerationConfig(modelsConfig);
    // Should use default turbospark-oauth model, not preserve manual OpenAI credentials
    expect(gc.model).toBe('coder-model');
    expect(gc.apiKey).toBe('QWEN_OAUTH_DYNAMIC_TOKEN');
    // baseUrl should be set to turbospark-oauth default, not preserved from manual OpenAI config
  ...
  describe('getAllConfiguredModels', () => {
    it('should return all models across all authTypes and put turbospark-oauth first', () => {
      const modelProvidersConfig: ModelProvidersConfig = {
        openai: [
          {
            id: 'openai-model-1',
      ...
      // turbospark-oauth models should be ordered first
      ...
      // Should include turbospark-oauth models (hard-coded)
    ...
    it('should return empty array when no models are registered', () => {
      ...
      // Should still include turbospark-oauth models (hard-coded)
    ...
    it('should support filtering by authTypes and still put turbospark-oauth first when included', () => {
      const modelProvidersConfig: ModelProvidersConfig = {
        openai: [
          {
            id: 'openai-model-1',
      ...
      // Filter: OpenAI only (should not include turbospark-oauth)
      ...
      // Filter: include turbospark-oauth but request it later -> still ordered first
    ...
    it('should handle reload with empty config', async () => {
      ...
      expect(
        modelsConfig
          ...
          .filter((m) => m.authType !== 'turbospark-oauth').length,
      ).toBeGreaterThan(0);

      // Reload with empty config
      modelsConfig.reloadModelProvidersConfig({});

      // Only turbospark-oauth models should remain
      const models = modelsConfig.getAllConfiguredModels();
      expect(models.every((m) => m.authType === 'turbospark-oauth')).toBe(true);
    });

    it('should preserve turbospark-oauth models after reload', () => {
      const modelsConfig = new ModelsConfig({
        modelProvidersConfig: {
          openai: [{ id: 'gpt-4', name: 'GPT-4' }],
        },
      ...
      const initialQwenModels = modelsConfig
        .getAllConfiguredModels()
        .filter((m) => m.authType === 'turbospark-oauth');

      modelsConfig.reloadModelProvidersConfig({
        gemini: [{ id: 'gemini-pro', name: 'Gemini Pro' }],
      });

      // turbospark-oauth models should still exist
