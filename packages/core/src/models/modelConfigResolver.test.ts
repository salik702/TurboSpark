...
describe('modelConfigResolver', () => {
  describe('resolveModelConfig', () => {
    ...
    describe('TURBOSPARK OAuth auth type', () => {
      it('uses default model for TURBOSPARK OAuth', () => {
        const result = resolveModelConfig({
      ...
      it('allows coder-model for TURBOSPARK OAuth', () => {
        const result = resolveModelConfig({
          authType: AuthType.QWEN_OAUTH,
          cli: {
            model: 'coder-model',
      ...
      it('warns and falls back for unsupported TURBOSPARK OAuth models', () => {
        const result = resolveModelConfig({
          authType: AuthType.QWEN_OAUTH,
          cli: {
      ...
      it('TURBOSPARK_API_TIMEOUT_MS applies in TURBOSPARK OAuth path', () => {
        const result = resolveModelConfig({
          authType: AuthType.QWEN_OAUTH,
          cli: {},
          settings: {},
          env: {
            TURBOSPARK_API_TIMEOUT_MS: '45000',
          },
        });
        ...
        expect(result.sources['timeout'].envKey).toBe(
          'TURBOSPARK_API_TIMEOUT_MS',
        );
        expect(result.config.model).toBe(DEFAULT_QWEN_MODEL);
      });

      it('modelProvider timeout takes precedence over TURBOSPARK_API_TIMEOUT_MS in OAuth', () => {
        const result = resolveModelConfig({
          authType: AuthType.QWEN_OAUTH,
          cli: {},
          settings: {},
      ...
      it('invalid TURBOSPARK_API_TIMEOUT_MS ignored in OAuth path', () => {
        const result = resolveModelConfig({
          authType: AuthType.QWEN_OAUTH,
          cli: {},
          settings: {},
          env: {
            TURBOSPARK_API_TIMEOUT_MS: 'not-a-number',
          },
        });
      ...
      it('negative TURBOSPARK_API_TIMEOUT_MS ignored in OAuth path', () => {
        const result = resolveModelConfig({
          authType: AuthType.QWEN_OAUTH,
          cli: {},
          settings: {},
          env: {
            TURBOSPARK_API_TIMEOUT_MS: '-100',
          },
        });
      ...
      it('zero TURBOSPARK_API_TIMEOUT_MS ignored in OAuth path', () => {
        const result = resolveModelConfig({
          authType: AuthType.QWEN_OAUTH,
          cli: {},
          settings: {},
          env: {
            TURBOSPARK_API_TIMEOUT_MS: '0',
          },
        });
      ...
      it('TURBOSPARK_API_TIMEOUT_MS works with float value in OAuth', () => {
        const result = resolveModelConfig({
          authType: AuthType.QWEN_OAUTH,
          cli: {},
          settings: {},
          env: {
            TURBOSPARK_API_TIMEOUT_MS: '12345.67',
          },
        });
      ...
      it('TURBOSPARK_API_TIMEOUT_MS works with proxy in OAuth path', () => {
        const result = resolveModelConfig({
          authType: AuthType.QWEN_OAUTH,
          cli: {},
          settings: {},
          env: {
            TURBOSPARK_API_TIMEOUT_MS: '60000',
          },
    ...
    describe('generation config resolution', () => {
      ...
      it('TURBOSPARK_API_TIMEOUT_MS env var overrides settings timeout', () => {
        const result = resolveModelConfig({
          authType: AuthType.USE_OPENAI,
          cli: {},
          settings: {
          ...
          env: {
            OPENAI_API_KEY: 'key',
            TURBOSPARK_API_TIMEOUT_MS: '900000',
          },
        });
      ...
      it('modelProvider timeout wins over TURBOSPARK_API_TIMEOUT_MS', () => {
        const result = resolveModelConfig({
          authType: AuthType.USE_OPENAI,
          cli: {},
          settings: {},
      ...
      it('TURBOSPARK_API_TIMEOUT_MS applies when modelProvider has no timeout', () => {
        const result = resolveModelConfig({
          authType: AuthType.USE_OPENAI,
          cli: {},
          settings: {},
      ...
      it('ignores invalid TURBOSPARK_API_TIMEOUT_MS values', () => {
        const result = resolveModelConfig({
          authType: AuthType.USE_OPENAI,
          cli: {},
          settings: {
          ...
          env: {
            OPENAI_API_KEY: 'key',
            TURBOSPARK_API_TIMEOUT_MS: 'invalid',
          },
        });
      ...
      it('ignores negative or zero TURBOSPARK_API_TIMEOUT_MS values', () => {
        const result = resolveModelConfig({
          authType: AuthType.USE_OPENAI,
          cli: {},
          settings: {
      ...
      it('TURBOSPARK_API_TIMEOUT_MS works for Anthropic auth type', () => {
        const result = resolveModelConfig({
          authType: AuthType.USE_ANTHROPIC,
          cli: {},
          settings: {},
          env: {
            ...
            TURBOSPARK_API_TIMEOUT_MS: '600000',
          },
        });
      ...
      it('handles extremely large timeout values safely', () => {
        const result = resolveModelConfig({
          ...
          env: {
            OPENAI_API_KEY: 'key',
            TURBOSPARK_API_TIMEOUT_MS: '999999999',
          },
        });
      ...
      it('handles whitespace-padded env values', () => {
        const result = resolveModelConfig({
          ...
          env: {
            OPENAI_API_KEY: 'key',
            TURBOSPARK_API_TIMEOUT_MS: ' 300000 ',
          },
        });
      ...
      it('ignores negative TURBOSPARK_API_TIMEOUT_MS values', () => {
        const result = resolveModelConfig({
          authType: AuthType.USE_OPENAI,
          cli: {},
          settings: {
  ...
  describe('validateModelConfig', () => {
    ...
    it('always passes for TURBOSPARK OAuth', () => {
      const result = validateModelConfig({
        authType: AuthType.QWEN_OAUTH,
  ...
  describe('[Regression] timeout env override refactor', () => {
    it('[Regression] OAuth path must apply TURBOSPARK_API_TIMEOUT_MS (was broken before fix #3629)', () => {
      ...
      expect(result.sources['timeout']).toBeDefined();
      expect(result.sources['timeout'].kind).toBe('env');
      expect(result.sources['timeout'].envKey).toBe('TURBOSPARK_API_TIMEOUT_MS');
      expect(result.config.model).toBe(DEFAULT_QWEN_MODEL);
    });

    it('[Regression] non-OAuth path must apply TURBOSPARK_API_TIMEOUT_MS', () => {
      const result = resolveModelConfig({
        authType: AuthType.USE_OPENAI,
        cli: {},
        settings: { apiKey: 'key' },
  ...
  describe('[Additional] timeout env override edge cases', () => {
    it('handles scientific notation in TURBOSPARK_API_TIMEOUT_MS', () => {
      const result = resolveModelConfig({
        authType: AuthType.USE_OPENAI,
        cli: {},
        settings: { apiKey: 'key' },
        env: {
          OPENAI_API_KEY: 'key',
          TURBOSPARK_API_TIMEOUT_MS: '1.5e5',
        },
      });
    ...
    it('handles hex values in TURBOSPARK_API_TIMEOUT_MS', () => {
      const result = resolveModelConfig({
        authType: AuthType.USE_OPENAI,
        cli: {},
        ...
        env: {
          OPENAI_API_KEY: 'key',
          TURBOSPARK_API_TIMEOUT_MS: '0x2BF20', // 180000 in hex
        },
      });
    ...
    it('ignores empty string TURBOSPARK_API_TIMEOUT_MS', () => {
      const result = resolveModelConfig({
        authType: AuthType.USE_OPENAI,
        cli: {},
        settings: { apiKey: 'key', generationConfig: { timeout: 30000 } },
    ...
    it('applies env override for every supported auth type', () => {
      ...
      for (const { type, env } of authTypes) {
        const result = resolveModelConfig({
          ...
          env: { ...env, TURBOSPARK_API_TIMEOUT_MS: '99999' },
        });
  ...
  describe('[Regression] issue-4219 — env-var-only path must call defaultModalities()', () => {
    ...
    it('TURBOSPARK OAuth path: modalities auto-detected for default coder-model', () => {
      // resolveGenerationConfig is shared by both the OpenAI and TURBOSPARK OAuth
      ...
      // because the TURBOSPARK OAuth coder-model now supports vision (see warning
