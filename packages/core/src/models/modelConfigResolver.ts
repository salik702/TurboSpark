...
import {
  AUTH_ENV_MAPPINGS,
  DEFAULT_MODELS,
  TURBOSPARK_OAUTH_ALLOWED_MODELS,
...
/**
 * Applies TURBOSPARK_API_TIMEOUT_MS env override if modelProvider has not set a timeout.
...
function applyTimeoutEnvOverride(
  ...
  const raw = env['TURBOSPARK_API_TIMEOUT_MS'];
  ...
  if (Number.isFinite(parsed) && parsed > 0) {
    ...
    sources['timeout'] = {
      kind: 'env',
      envKey: 'TURBOSPARK_API_TIMEOUT_MS',
    };
  }
}
...
export function resolveModelConfig(
  ...
  // Special handling for TURBOSPARK OAuth
  if (authType === AuthType.TURBOSPARK_OAUTH) {
    return resolveTurbosparkOAuthConfig(input, warnings);
  }
  ...
  // ---- Env override: TURBOSPARK_API_TIMEOUT_MS ----
...
/**
 * Special resolver for TURBOSPARK OAuth authentication.
 * TURBOSPARK OAuth has fixed model options and uses dynamic tokens.
 */
function resolveTurbosparkOAuthConfig(
  input: ModelConfigSourcesInput,
  warnings: string[],
): ModelConfigResolutionResult {
  ...
  // TURBOSPARK OAuth only allows specific models
  const allowedModels = new Set<string>(TURBOSPARK_OAUTH_ALLOWED_MODELS);
  ...
  if (requestedModel && allowedModels.has(requestedModel)) {
    ...
    if (requestedModel) {
      ...
      const extraMessage = isVisionModel
        ...
        : '';
      warnings.push(
        `Warning: Unsupported TURBOSPARK OAuth model '${requestedModel}', falling back to '${DEFAULT_MODEL}'.${extraMessage}`,
      );
    }
    resolvedModel = DEFAULT_MODEL;
    modelSource = defaultSource(`fallback to '${DEFAULT_MODEL}'`);
  }

  sources['model'] = modelSource;
  sources['apiKey'] = computedSource('TURBOSPARK OAuth dynamic token');
  sources['authType'] = computedSource('provided by caller');
  ...
  const config: ContentGeneratorConfig = {
    authType: AuthType.TURBOSPARK_OAUTH,
    model: resolvedModel,
    apiKey: 'TURBOSPARK_OAUTH_DYNAMIC_TOKEN',
    proxy,
    ...generationConfig,
  };
