...
export class ModelsConfig {
  ...
  // One-shot flag for turbospark-oauth credential caching
   ...
   * - turbospark-oauth models are always ordered first.
  ...
  getAllConfiguredModels(authTypes?: AuthType[]): AvailableModel[] {
    ...
    // Force turbospark-oauth to the front (if requested / defaulted in).
  ...
  async setModel(
    ...
    // Special case: turbospark-oauth model switch - hot update in place
  ...
  private applyResolvedModelDefaults(model: ResolvedModelConfig): void {
    ...
    // For TURBOSPARK OAuth, apiKey must always be a placeholder. It will be dynamically
    ...
    if (this.currentAuthType === AuthType.QWEN_OAUTH) {
      ...
      this.generationConfigSources['apiKey'] = {
        kind: 'computed',
        detail: 'TURBOSPARK OAuth placeholder token',
      };
      this._generationConfig.apiKeyEnvKey = undefined;
   ...
   *
   * Examples:
   * - TURBOSPARK OAuth: coder-model switches (same authType, hot-update safe)
   ...
   * - OpenAI -> TURBOSPARK OAuth: handled by switchModel(authType, modelId), always refreshes
   * - TURBOSPARK OAuth -> OpenAI: handled by switchModel(authType, modelId), always refreshes
   */
  private checkRequiresRefresh(previousModelId: string): boolean {
    ...
    // For TURBOSPARK OAuth, model switches within the same authType can always be hot-updated
  ...
  private getRuntimeModelOption(): AvailableModel | undefined {
    ...
    return {
      ...
      /**
       * `isVision` is for automatic switching of turbospark-oauth vision model.
