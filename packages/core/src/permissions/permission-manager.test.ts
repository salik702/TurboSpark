...
describe('buildHumanReadableRuleLabel', () => {
  ...
  it('converts Read with absolute path specifier', () => {
    const label = buildHumanReadableRuleLabel(['Read(//Users/mochi/.turbospark/**)']);
    expect(label).toBe('read files in /Users/mochi/.turbospark/');
  });
...
describe('PermissionManager — compound shell write attribution', () => {
  it('deny rule matches a write after `cd` into a subdir', async () => {
    const pm = new PermissionManager(
      makeConfig({
        permissionsDeny: ['WriteFileTool(.turbospark/settings.json)'],
        cwd: '/repo',
        projectRoot: '/repo',
      }),
    );
  ...
  it('allow rule on the same shell command does NOT downgrade a virtual-op deny', async () => {
    ...
    // .turbospark/settings.json escalates the verdict. Allow + virtual-op deny
  ...
  it('hasRelevantRules sees protected writes after `cd` before compound recursion', () => {
    const pm = new PermissionManager(
      makeConfig({
        permissionsDeny: ['Write(.turbospark/settings.json)'],
        cwd: '/repo',
        projectRoot: '/repo',
      }),
    );
  ...
  it('hasMatchingAskRule sees writes after `cd` into a subdir', () => {
    const pm = new PermissionManager(
      makeConfig({
        permissionsAsk: ['WriteFileTool(.turbospark/settings.json)'],
        cwd: '/repo',
        projectRoot: '/repo',
      }),
    );
