...
function getNormalizedQwenHomePrefixes(): string[] {
  const qwenHome = process.env['TURBOSPARK_HOME'];
  if (!qwenHome) return [];
  if (
  ...
  try {
    ...
    // TURBOSPARK_HOME may not exist yet; the configured path still matters.
  }
...
export function passesAcceptEditsFastPath(
  ...
  // Qwen self-modification surfaces (.turbospark/settings*.json, configured context
  // files, .turbospark/rules|commands|agents|skills|hooks/, .mcp.json) must never
