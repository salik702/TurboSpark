# @turbospark/channel-plugin-example

A reference channel plugin for TURBO SPARK. It connects to a WebSocket server and routes messages through the full channel pipeline (access control, session routing, agent bridge).
...
```bash
npm install @turbospark/channel-plugin-example
```

### 2. Link it as a TURBO SPARK extension

The package ships a `turbospark-extension.json` manifest, so it works as an extension out of the box:

```bash
qwen extensions link ./node_modules/@turbospark/channel-plugin-example
```

### 3. Configure the channel

Add a channel entry to `~/.turbospark/settings.json`:
...
4. Add a `turbospark-extension.json` manifest
