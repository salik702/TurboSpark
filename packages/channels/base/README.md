# @turbospark/channel-base

Base infrastructure for building TURBO SPARK channel adapters. Provides the abstract base class, access control, session routing, and the ACP bridge that communicates with the agent.
...
```bash
npm install @turbospark/channel-base
```
...
For a complete working example, see [`@turbospark/channel-plugin-example`](../plugin-example/).
...
| `AcpBridge`     | Spawns and communicates with the `turbospark --acp` agent process |
...
Manages the `turbospark --acp` child process and ACP sessions.
...
Persists pairing state to `~/.turbospark/channels/{channelName}-pairing.json` and `{channelName}-allowlist.json`.
...
- [`@turbospark/channel-plugin-example`](../plugin-example/) — working reference implementation
