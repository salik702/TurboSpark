# TURBO SPARK Java SDK

## Project Overview

The TURBO SPARK Java SDK is a minimum experimental SDK for programmatic access to TURBO SPARK functionality. It provides a Java interface to interact with the TURBO SPARK CLI, allowing developers to integrate TURBO SPARK capabilities into their Java applications.

**Context Information:**

- Current Date: Monday 5 January 2026
- Operating System: darwin
- Working Directory: /Users/weigeng/repos/turbospark/packages/sdk-java

## Project Details

- **Group ID**: com.alibaba
- **Artifact ID**: turbosparkcode-sdk (as per pom.xml)
...
- **API Layer**: Provides the main entry points through `TurbosparkCli` class with simple static methods for basic usage
- **Session Layer**: Manages communication sessions with the TURBO SPARK CLI through the `Session` class
...
- `TurbosparkCli`: Main entry point with static methods for simple queries
...
The project includes basic unit tests using JUnit 5. The main test class `TurbosparkCliTest` demonstrates how to use the SDK to make simple queries to the TURBO SPARK CLI.
...
The `TransportOptions` class allows configuration of how the SDK communicates with the TURBO SPARK CLI:

- `pathToTurbosparkExecutable`: Path to the TURBO SPARK CLI executable
...
- **Session creation**: Use `TurbosparkCli.newSession()` to create a new session with custom options
...
- **Thread Naming**: "turbospark_cli-pool-{number}"
...
The SDK includes several example files in `src/test/java/com/alibaba/turbospark/code/cli/example/` that demonstrate different aspects of the API:
...
### Q: Do I need to install the TURBOSPARK CLI separately?
