...
vi.mock('@turbospark/turbospark-core', () => ({
  ...
  getAllGeminiMdFilenames: vi.fn(() => ['TURBOSPARK.md', 'AGENTS.md']),
  getAutoMemoryRoot: vi.fn(
    (projectRoot: string) => `${projectRoot}/.turbospark/memory`,
  ),
  ...
  Storage: {
    getGlobalTurbosparkDir: vi.fn(() => '/tmp/qwen-global-test'),
  },
...
vi.mock('../utils/languageUtils.js', () => ({
  ...
  writeOutputLanguageAndRegisterPath: vi.fn(
    (
      ...
      if (!p) {
        config?.setOutputLanguageFilePath('/mock/.turbospark/output-language.md');
      }
    },
  ),
  getOutputLanguageFilePath: vi
    .fn()
    .mockReturnValue('/mock/.turbospark/output-language.md'),
...
// ---------------------------------------------------------------------------
// Tests for TurbosparkAgent.initialize() mcpCapabilities + newSession SSE/HTTP
// ---------------------------------------------------------------------------

describe('TurbosparkAgent MCP SSE/HTTP support', () => {
  ...
  it('does not return discontinued turbospark-oauth as the only ACP auth option', async () => {
  ...
  function makeMemorySettings(
    ...
    const user = {
      path: '/home/test/.turbospark/settings.json',
      settings: { memory },
    };
  ...
  function makeCoreSettings(outputLanguage = 'English') {
    ...
    return {
      ...
      workspace: {
        path: '/work/.turbospark/settings.json',
  ...
  it('status ext methods expose workspace snapshots without secrets', async () => {
    ...
    mockConfig = {
      ...
      getAuthType: vi.fn().mockReturnValue('turbospark'),
      getAllConfiguredModels: vi.fn().mockReturnValue([
        {
          id: 'qwen-plus',
          label: 'Qwen Plus',
    ...
    expect(providers).toMatchObject({
      ...
      workspaceCwd: '/work/status',
      initialized: true,
      current: { authType: 'turbospark', modelId: 'qwen-plus(qwen)' },
      providers: [
        {
          kind: 'model_provider',
          status: 'ok',
  ...
  it('extMethod preflight surfaces SkillError as parse_error errorKind', async () => {
    const skillError = new (
      await import('@turbospark/turbospark-core')
  ...
  it('provider status marks current only for matching models', async () => {
    ...
    await expect(
      ...
      current: { authType: 'turbospark', modelId: 'missing-model(qwen)' },
      providers: [
        {
          authType: 'turbospark',
          current: false,
  ...
  it('provider status uses runtime model ids for base id and token limit', async () => {
    ...
    await expect(
      ...
      current: { authType: 'turbospark', modelId: 'runtime-qwen-plus(qwen)' },
      providers: [
        {
          authType: 'turbospark',
          current: true,
  ...
  it('qwen/settings extension methods read and update user memory settings', async () => {
    ...
    await expect(
      ...
      paths: {
        userMemoryFile: path.join('/tmp/qwen-global-test', 'TURBOSPARK.md'),
        projectMemoryFile: path.join('/tmp/qwen-memory-cwd-test', 'TURBOSPARK.md'),
        autoMemoryDir: '/tmp/qwen-memory-root-test/.turbospark/memory',
      },
    });
    await expect(
  ...
  it('qwen/skills/install installs a GitHub directory skill through ACP', async () => {
    ...
    vi.mocked(Storage.getGlobalTurbosparkDir).mockReturnValue(tempHome);
  ...
  it('qwen/skills setEnabled resolves project skills from the ext method cwd', async () => {
    ...
    const skillDir = path.join(tempProject, '.turbospark', 'skills', 'issue-fixer');
    const skillFile = path.join(skillDir, 'SKILL.md');
    await fs.mkdir(skillDir, { recursive: true });
    await fs.writeFile(
      skillFile,
    ...
    try {
      ...
      expect(loadSkillsFromDir).toHaveBeenCalledWith(
        path.join(tempProject, '.turbospark', 'skills'),
        'project',
      );
  ...
  it('per-session newSession surfaces MCP failures to stderr (round-7 fix: was silent before)', async () => {
    // Round-7 regression: `TurbosparkAgent.initializeConfig()` (per-session ACP
...
describe('TurbosparkAgent extMethod renameSession routing', () => {
  type AgentSideConnectionLike = { closed: Promise<void> };
  type AgentLike = {
    initialize: (args: Record<string, unknown>) => Promise<unknown>;
    newSession: (args: Record<string, unknown>) => Promise<unknown>;
...
// Tests for TurbosparkAgent.loadSession() and TurbosparkAgent.unstable_resumeSession()
...
// resume does not).
describe('TurbosparkAgent loadSession / unstable_resumeSession', () => {
  let capturedAgentFactory:
...
describe('TurbosparkAgent extMethod runtime MCP add/remove (T2.8)', () => {
  let capturedAgentFactory:
    | ((conn: { closed: Promise<void> }) => {
        initialize: (args: Record<string, unknown>) => Promise<unknown>;
        extMethod: (
...
describe('sessionLanguage multi-session propagation', () => {
  ...
  it('propagates language write and refresh to all sessions with varying paths', async () => {
    const cfgA = makeConfig({
      ...
      getOutputLanguageFilePath: vi
        .fn()
        .mockReturnValue('/proj-a/.turbospark/output-language.md'),
    });
    const cfgB = makeConfig({
      getSessionId: vi.fn().mockReturnValue('s-b'),
      getOutputLanguageFilePath: vi
        .fn()
        .mockReturnValue('/proj-b/.turbospark/output-language.md'),
    });
    ...
    expect(updateOutputLanguageFile).toHaveBeenCalledWith(
      'zh',
      '/proj-b/.turbospark/output-language.md',
    );
  ...
  it('still refreshes sessions when a file write fails', async () => {
    ...
    const cfgFail = makeConfig({
      ...
      getOutputLanguageFilePath: vi
        .fn()
        .mockReturnValue('/readonly/.turbospark/output-language.md'),
    });
    ...
    vi.mocked(updateOutputLanguageFile).mockImplementation(
      (_value: string, path?: string) => {
        if (path === '/readonly/.turbospark/output-language.md') {
