 ...
 * here, or explicitly waived for non-env-based auth like turbospark-oauth). The
...
const PROJECT_SKILL_DIRS = ['.turbospark', '.agents'] as const;
...
const QWEN_CORE_SETTING_DEFINITIONS = {
  ...
  'context.fileFiltering.respectTurbosparkIgnore': { type: 'boolean' },
...
async function resolveQwenMemoryPaths(params: {
  ...
  const fallbackFilename = getAllGeminiMdFilenames()[0] ?? 'TURBOSPARK.md';
...
export async function runAcpAgent(
  ...
  let agentInstance: TurbosparkAgent | undefined;
  const connection = new AgentSideConnection((conn) => {
    agentInstance = new TurbosparkAgent(config, settings, argv, conn);
...
function parsePoolDrainMs(envValue: string | undefined): number {
  ...
  if (!/^\d+$/.test(trimmed)) {
    process.stderr.write(
      `turbospark serve: QWEN_SERVE_MCP_POOL_DRAIN_MS=${JSON.stringify(envValue)} ` +
...
class TurbosparkAgent implements Agent {
  ...
  constructor(
    ...
    // roll back without rebuilding. `runTurbosparkServe.ts` sets this when
  ...
  private broadcastBudgetEvent(event: McpBudgetEvent): void {
    // The TurbosparkAgent's `this.connection` is the single ACP channel to
  ...
  async authenticate({ methodId }: AuthenticateRequest): Promise<void> {
    ...
    if (method === AuthType.QWEN_OAUTH) {
      qwenOAuth2Events.once(TurbosparkOAuth2Event.AuthUri, authUriHandler);
    }
    ...
    try {
      ...
      if (method === AuthType.QWEN_OAUTH) {
        qwenOAuth2Events.off(TurbosparkOAuth2Event.AuthUri, authUriHandler);
      }
    }
  }
  ...
  private async buildWorkspaceMcpStatus(
    ...
    try {
      ...
      try {
        ...
        process.stderr.write(
          `turbospark serve: pool snapshot for workspace MCP status failed: ` +
            `${err instanceof Error ? err.message : String(err)}\n`,
        );
      }
      ...
      if (wsBudget !== undefined) {
        ...
        try {
          ...
          process.stderr.write(
            `turbospark serve: getMcpClientAccounting failed: ` +
              `${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
  ...
  private buildAuthPreflightCell(config: Config): ServePreflightCell {
    try {
      ...
      // No env-var registration → either OAuth-style auth (turbospark-oauth) or
  ...
  private async installSkillFromUrl(
    ...
    const skillsBaseDir = path.join(Storage.getGlobalTurbosparkDir(), 'skills');
  ...
  private async deleteGlobalSkill(
    ...
    // never a filesystem root or the global TURBOSPARK dir itself, so a malformed
    ...
    const globalDir = path.resolve(Storage.getGlobalTurbosparkDir());
      ...
      case SERVE_CONTROL_EXT_METHODS.workspaceMcpRestart: {
        ...
        try {
          ...
          if (mergedDisabled !== undefined && !Array.isArray(mergedDisabled)) {
            process.stderr.write(
              `turbospark serve: MCP restart for ${JSON.stringify(serverName)}: ` +
          ...
          async () => {
            ...
            try {
              ...
              sessionService.removeSession(newSessionId).catch((rmErr) => {
                process.stderr.write(
                  `turbospark serve: failed to clean up orphan session ${newSessionId}: ${rmErr instanceof Error ? rmErr.message : rmErr}\n`,
                );
              });
              throw err;
            }
  ...
  private async ensureAuthenticated(config: Config): Promise<void> {
    ...
    if (!selectedType) {
      throw RequestError.authRequired(
        ...
        'Use TURBO SPARK CLI to authenticate first.',
      );
    }
