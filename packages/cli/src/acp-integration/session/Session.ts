...
import { NOT_CURRENTLY_GENERATING_CANCEL_MESSAGE } from '@turbospark/acp-bridge/bridgeErrors';
...
import { MID_TURN_QUEUE_DRAIN_METHOD } from '@turbospark/acp-bridge/bridgeTypes';
...
const USER_CANCEL_ABORT_REASON = 'turbospark:user-cancel';
...
export class Session implements SessionContext {
  ...
  async #startCronSchedulerIfNeeded(): Promise<void> {
    ...
    // Durable tasks live under ~/.turbospark (user-owned, not in the working
  ...
  async #emitBackgroundNotificationDisplay(
    ...
    await this.sendUpdate({
      ...
      _meta: {
        ...
        turbosparkDiscreteMessage: true,
        backgroundTask: {
          taskId: item.taskId,
          status: item.status,
          kind: item.kind,
  ...
  async #emitBackgroundNotificationEndTurn(
    ...
    try {
      await this.client.extNotification('_turbosparkcode/end_turn', {
        sessionId: this.sessionId,
        reason,
        source: 'background_notification',
      });
  ...
  private async runToolCalls(
    ...
    // `TURBOSPARK_MAX_TOOL_CONCURRENCY` (default 10). Results are returned
        ...
        try {
            ...
            if (
              ...
              if (hooksEnabled && messageBus) {
                this.fireNotificationHookWithTerminalSequence(
                  messageBus,
                  `TURBO SPARK needs your permission to use ${toolName}`,
          ...
          try {
            const sleepInhibitorHandle = acquireSleepInhibitor(
              this.config,
              `TURBO SPARK is executing tool ${toolName}`,
            );
            try {
