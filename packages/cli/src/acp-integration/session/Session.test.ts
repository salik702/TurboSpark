...
import type { ChatRecord, Config, GeminiChat } from '@turbospark/turbospark-core';
import {
  ApprovalMode,
  AuthType,
...
describe('Session', () => {
  ...
  describe('prompt', () => {
    ...
    it('degrades an oversized inline image to a text placeholder before sending to the model', async () => {
      const ENV_KEY = 'TURBOSPARK_MAX_INLINE_MEDIA_BYTES';
    ...
    it('routes ACP protected L4 allow writes through AUTO review', async () => {
      ...
      const invocation = {
        params: { file_path: '/repo/.turbospark/settings.json', content: '{}' },
        getDefaultPermission: vi.fn().mockResolvedValue('ask'),
        getConfirmationDetails: vi.fn().mockResolvedValue({
          type: 'edit',
          title: 'Confirm file write',
          fileName: '/repo/.turbospark/settings.json',
          fileDiff: 'diff',
          onConfirm: vi.fn(),
        }),
        getDescription: vi.fn().mockReturnValue('Write file'),
              ...
              functionCalls: [
                {
                  ...
                  name: core.ToolNames.WRITE_FILE,
                  args: {
                    file_path: '/repo/.turbospark/settings.json',
                    content: '{}',
                  },
                },
              ],
  ...
  describe('runToolCalls', () => {
    ...
    it('skips unstarted Agent calls after nested ask_user_question cancellation', async () => {
      ...
      process.env['TURBOSPARK_MAX_TOOL_CONCURRENCY'] = '1';
      try {
        const eventEmitter = new EventEmitter();
        const respond = vi.fn().mockResolvedValue(undefined);
        const questionExecute = vi
