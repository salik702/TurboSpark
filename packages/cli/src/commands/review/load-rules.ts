...
// PR cannot inject `.turbospark/review-rules.md` content that bypasses scrutiny.
// Sources, in order:
//
//   1. `.turbospark/review-rules.md`
...
//   4. `TURBOSPARK.md`   — only the `## Code Review` section
//
...
function loadCombined(baseRef: string): {
  ...
  // 1. Qwen-native rules.
  const qwenRules = showFile(baseRef, '.turbospark/review-rules.md');
  if (qwenRules) {
    sections.push(`### From .turbospark/review-rules.md\n\n${qwenRules.trim()}`);
    loaded.push('.turbospark/review-rules.md');
  }
  ...
  // 4. TURBOSPARK.md — extract Code Review section only.
  const qwenMd = showFile(baseRef, 'TURBOSPARK.md');
  if (qwenMd) {
    const section = extractCodeReviewSection(qwenMd);
    if (section) {
      sections.push(`### From TURBOSPARK.md\n\n${section}`);
      loaded.push('TURBOSPARK.md');
    }
  }
...
export const loadRulesCommand: CommandModule = {
  ...
  describe:
    'Read project review rules from the base branch (.turbospark/review-rules.md, .github/copilot-instructions.md, AGENTS.md, TURBOSPARK.md) and write a combined Markdown file',
  builder: (yargs) =>
    yargs
      .positional('base_ref', {
        type: 'string',
