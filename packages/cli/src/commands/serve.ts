...
// handler below so it only loads when the user actually runs `turbospark serve`.
 ...
 * in `gemini.tsx`. SIGINT / SIGTERM in `runTurbosparkServe` is the sole exit
 * route.
 */
...
export async function maybeOpenWebShellBrowser(
  ...
  try {
    ...
    if (handle.resolvedToken) {
      ...
      writeStderrLine(
        'turbospark serve: --open passes the token in the browser launch command ' +
    ...
    writeStderrLine(
      `turbospark serve: failed to open browser: ${browserErr instanceof Error ? browserErr.message : String(browserErr)}`,
    );
  }
}
...
export const serveCommand: CommandModule<unknown, ServeArgs> = {
  command: 'serve',
  describe:
    'Run TURBO SPARK as a local HTTP daemon (Stage 1 experimental: --http-bridge)',
  builder: (yargs: Argv) =>
    yargs
      .option('port', {
        type: 'number',
      ...
      .option('token', {
        type: 'string',
        description:
          'Bearer token required on every request. Falls back to the TURBOSPARK_SERVER_TOKEN env var.',
      })
      .option('max-sessions', {
        type: 'number',
        default: 20,
      ...
      .option('workspace', {
        ...
        description:
          ...
          'For multi-workspace deployments, run one `turbospark serve` per workspace ' +
      ...
      .option('require-auth', {
        ...
        description:
          ...
          '127.0.0.1. Requires --token or TURBOSPARK_SERVER_TOKEN. /health also ' +
  ...
  handler: async (argv) => {
    if (!argv['http-bridge']) {
      writeStderrLine(
        'turbospark serve: --no-http-bridge (native mode) is not yet implemented; ' +
    ...
    if (argv.token) {
      ...
      writeStderrLine(
        'turbospark serve: --token is visible in the process command line; ' +
          'prefer the TURBOSPARK_SERVER_TOKEN env var for any non-trivial ' +
          'deployment.',
      );
    }
    ...
    if (mcpClientBudget !== undefined) {
      if (
        ...
        writeStderrLine(
          'turbospark serve: --mcp-client-budget must be a positive integer.',
        );
        process.exit(1);
      }
    }
    if (mcpBudgetMode === 'enforce' && mcpClientBudget === undefined) {
      writeStderrLine(
        'turbospark serve: --mcp-budget-mode=enforce requires --mcp-client-budget=N.',
      );
      process.exit(1);
    }
    ...
    if (
      ...
      writeStderrLine(
        'turbospark serve: --max-pending-prompts-per-session must be a non-negative integer (0 / Infinity = unlimited).',
      );
      process.exit(1);
    }
    if (mcpClientBudget !== undefined) {
      ...
      writeStderrLine(
        `turbospark serve: --mcp-client-budget=${mcpClientBudget} mode=${resolvedMcpMode}` +
    ...
    try {
      ...
      const suppress = process.env['TURBOSPARK_SUPPRESS_YOLO_WARNING'];
    ...
    if (rateLimit) {
      ...
      for (const [name, value] of [
        ...
        if (
          ...
          writeStderrLine(`turbospark serve: ${name} must be a positive integer.`);
          process.exit(1);
        }
      }
      if (
        ...
        writeStderrLine(
          'turbospark serve: --rate-limit-window-ms must be an integer >= 1000.',
        );
        process.exit(1);
      }
    }
    ...
    const { runTurbosparkServe } = await import('../serve/index.js');
    try {
      ...
      writeStderrLine(
        `turbospark serve: ${err instanceof Error ? err.message : String(err)}`,
      );
      process.exit(1);
    }
    await blockForever();
