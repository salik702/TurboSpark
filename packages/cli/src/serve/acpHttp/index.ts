...
export function mountAcpHttp(
  ...
  const registry = new ConnectionRegistry(
    ...
    (sessionId, clientId) => {
      void bridge.detachClient(sessionId, clientId).catch((err: unknown) => {
        writeStderrLine(
          `turbospark serve: /acp detachClient(${sessionId}) failed: ${
  ...
  app.post(path, async (req: Request, res: Response) => {
    ...
    if (!parsed.ok) {
      writeStderrLine(
        `turbospark serve: /acp malformed request from ${req.socket?.remoteAddress}: ${parsed.error.error.message}`,
      );
    ...
    if (isRequest(message) && message.method === 'initialize') {
      ...
      if (!conn) {
        ...
        writeStderrLine(
          `turbospark serve: /acp connection cap reached (max=${registry.connectionCap}), rejecting initialize`,
        );
      ...
      writeStderrLine(
        `turbospark serve: /acp connection established ${conn.connectionId.slice(0, 8)} ` +
    ...
    await dispatcher
      ...
      .catch((err: unknown) => {
        writeStderrLine(
          `turbospark serve: /acp handle error: ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
      });
  ...
  app.get(path, (req: Request, res: Response) => {
    ...
    if (!sessionId) {
      ...
      const stream = new SseStream(
        ...
        () => {
          writeStderrLine(
            `turbospark serve: /acp connection stream closed (${connId.slice(0, 8)})`,
          );
          ...
          conn.connGraceTimer = setTimeout(() => {
            if (
              ...
              writeStderrLine(
                `turbospark serve: /acp reaping connection ${connId.slice(0, 8)} (conn stream gone, no live session stream)`,
              );
    ...
    void dispatcher.pumpSessionEvents(conn, sessionId, ac.signal).then(
      ...
      (err: unknown) => {
        writeStderrLine(
          `turbospark serve: /acp event pump error (${sessionId}): ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
        closeIfCurrent();
  ...
  app.delete(path, (req: Request, res: Response) => {
    ...
    if (existed) {
      writeStderrLine(
        `turbospark serve: /acp connection deleted ${connectionId.slice(0, 8)} (remaining=${registry.size})`,
      );
    }
    res.status(202).end();
  });
  ...
  function setupWebSocket(httpServer: import('node:http').Server): void {
    ...
    upgradeListener = (req: IncomingMessage, socket: Duplex, head: Buffer) => {
      ...
      wss!.handleUpgrade(req, socket, head, (ws: WebSocket) => {
        ...
        const initTimer = setTimeout(() => {
          if (!initialized) {
            writeStderrLine(
              `turbospark serve: /acp WS initialize timeout (30s) from ${rawAddr}`,
            );
        ...
        ws.on('message', (rawData: Buffer | string) => {
          messageQueue = messageQueue
            ...
            .catch((err) => {
              writeStderrLine(
                `turbospark serve: /acp WS message handler error: ${err instanceof Error ? err.message : String(err)}`,
              );
            });
        });
        ...
        async function handleWsMessage(
          ...
          if (!initialized) {
            ...
            const stream = new WsStream(
              ...
              () => {
                writeStderrLine(
                  `turbospark serve: /acp WS closed (${conn.connectionId.slice(0, 8)})`,
                );
            ...
            connRef = conn;
            writeStderrLine(
              `turbospark serve: /acp WS established ${conn.connectionId.slice(0, 8)} (loopback=${fromLoopback}, active=${registry.size})`,
            );
            return;
          }
              ...
              if (
                ...
                void dispatcher
                  ...
                  .then(cleanupSession, (err: unknown) => {
                    writeStderrLine(
                      `turbospark serve: /acp WS pump error (${sid}): ${err instanceof Error ? err.message : String(err)}`,
                    );
                    cleanupSession();
                  });
              }
          ...
          const dispatchP = dispatcher
            ...
            .catch((err: unknown) => {
              writeStderrLine(
                `turbospark serve: /acp WS handle error: ${err instanceof Error ? err.message : String(err)}`,
              );
            });
    ...
    writeStderrLine(`turbospark serve: /acp WebSocket transport enabled on ${path}`);
  }
