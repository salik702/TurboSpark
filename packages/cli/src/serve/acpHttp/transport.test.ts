...
import { SessionService } from '@turbospark/turbospark-core';
...
const fakeWorkspace = {
  ...
  async initWorkspace() {
    return { path: '/ws/TURBOSPARK.md', action: 'created' as const };
  },
...
describe('ACP Streamable HTTP transport (over the wire)', () => {
  ...
  it('initialize omits _qwen/session/shell by default', async () => {
    ...
    const result = body['result'] as {
      agentCapabilities: {
        _meta: { turbospark: { methods: string[] } };
      };
    };
    expect(result.agentCapabilities._meta.turbospark.methods).not.toContain(
      '_qwen/session/shell',
    );
  });
  ...
  it('initialize advertises _qwen/session/shell when enabled', async () => {
    ...
    expect(result.agentCapabilities._meta.turbospark.methods).toContain(
      '_qwen/session/shell',
    );
  });
  ...
  it('permission request round-trips agent→client→agent', async () => {
    ...
    try {
      ...
      expect(reqFrame.params._meta['turbospark'].requestId).toBe('perm-1');
