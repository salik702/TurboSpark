...
import type { HttpAcpBridge } from '@turbospark/acp-bridge/bridgeTypes';
import type { BridgeEvent } from '@turbospark/acp-bridge/eventBus';
import {
...
export class AcpDispatcher {
  ...
  private killOrphanSession(sessionId: string): void {
    void this.bridge
      ...
      .catch((err) =>
        writeStderrLine(
          `turbospark serve: /acp orphan killSession(${logSafe(sessionId)}) failed: ${logSafe(errMsg(err))}`,
        ),
      );
  }
  ...
  private async configOptionsFor(
    ...
    try {
      ...
      writeStderrLine(
        `turbospark serve: /acp configOptionsFor(${logSafe(sessionId)}) failed: ${logSafe(errMsg(err))}`,
      );
      return undefined;
    }
  }
  ...
  cancelAbandonedPermission(
    ...
    try {
      ...
      writeStderrLine(
        `turbospark serve: /acp cancelAbandonedPermission(${logSafe(req.sessionId)}) failed: ${logSafe(msg)}`,
      );
      return false;
    }
  }
  ...
  private requireOwned(
    ...
    if (id === undefined) {
      ...
      writeStderrLine(
        `turbospark serve: /acp notification for unowned session ${logSafe(sessionId)} (dropped)`,
      );
      return false;
    }
    conn.sendConn(
  ...
  async handle(
    ...
    if (isResponse(msg)) {
      try {
        ...
        writeStderrLine(
          `turbospark serve: /acp response handling error: ${logSafe(errMsg(err))}`,
        );
      }
      return;
    }
    ...
    try {
      switch (method) {
        ...
        case 'session/resume': {
          ...
          if (conn.destroyed || closeRaced) {
            ...
            void cleanup.catch((err) =>
              writeStderrLine(
                `turbospark serve: /acp orphan ${restored.attached ? 'detach' : 'kill'}(${logSafe(sessionId)}) teardown-race: ${logSafe(errMsg(err))}`,
              ),
            );
        ...
        case 'session/close': {
          ...
          try {
            ...
            try {
              ...
              writeStderrLine(
                `turbospark serve: /acp session/close local teardown failed (${logSafe(sessionId)}): ${logSafe(errMsg(teardownErr))}`,
              );
            }
        ...
        case `${QWEN_METHOD_NS}session/shell`: {
          ...
          writeStderrLine(
            `turbospark serve: /acp session/shell session=${logSessionId} client=${logClientId} cmd=${logCommand}`,
          );
        ...
        case `${QWEN_METHOD_NS}workspace/memory/write`: {
          ...
          writeStderrLine(
            `turbospark serve: /acp workspace/memory/write scope=${scope} mode=${mode} client=${conn.clientId?.slice(0, 8)} bytes=${Buffer.byteLength(content, 'utf8')}`,
          );
        ...
        case `${QWEN_METHOD_NS}sessions/delete`: {
            ...
            ids.map(async (sid) => {
              try {
                ...
                if (
                  ...
                  writeStderrLine(
                    `turbospark serve: /acp sessions/delete closeSession(${safeSessionId}) failed: ${safeMessage}`,
                  );
          ...
          for (const e of removeResult.errors) {
            ...
            writeStderrLine(
              `turbospark serve: /acp sessions/delete removeSessions(${safeSessionId}) failed: ${safeMessage}`,
            );
          }
          this.replyConn(conn, id, {
      ...
      writeStderrLine(
        `turbospark serve: /acp dispatch error (${logSafe(method)}): ${logSafe(errMsg(err))}`,
      );
      if (id !== undefined) {
        const { code, message, data } = toRpcError(err);
    ...
    switch (event.type) {
      ...
      case 'permission_request': {
        ...
        if (!binding?.stream || binding.stream.isClosed) {
          ...
          if (!cancelled) {
            writeStderrLine(
              `turbospark serve: /acp permission cancel FAILED for ${logSafe(sessionId)} (mediator stuck; no automatic recovery)`,
            );
          }
          return;
        }
  ...
  private resolveClientResponse(
    ...
    try {
      ...
      writeStderrLine(
        `turbospark serve: /acp permission vote failed (${logSafe(pending.sessionId)}): ${logSafe(errMsg(err))}`,
      );
  ...
  private async handlePrompt(
    ...
    try {
      ...
      if (id !== undefined) {
        ...
        writeStderrLine(
          `turbospark serve: /acp prompt error (${logSafe(sessionId)}, notification): ${logSafe(errMsg(err))}`,
        );
      }
    } finally {
  ...
  private replySession(
    ...
    if (conn.sessions.has(sessionId)) {
      ...
      // session teardown.
      writeStderrLine(
        `turbospark serve: /acp replySession(${logSafe(sessionId)}) binding gone mid-flight, ` +
