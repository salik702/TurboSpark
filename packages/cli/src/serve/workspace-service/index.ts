...
} from '@turbospark/acp-bridge/status';
...
import { mapDomainErrorToErrorKind } from '@turbospark/acp-bridge/status';
import { MCP_RESTART_SERVER_DEADLINE_MS } from '@turbospark/acp-bridge/mcpTimeouts';
  ...
  return {
    ...
    async getWorkspaceEnvStatus(_ctx: WorkspaceRequestContext) {
      ...
      try {
        ...
        writeStderrLine(
          `turbospark serve: getEnvStatus failed: ${err instanceof Error ? err.message : String(err)}`,
        );
        return createIdleEnvStatus(boundWorkspace, acpChannelLive);
      }
    },
    ...
    async getWorkspacePreflightStatus(_ctx: WorkspaceRequestContext) {
      ...
      if (statusProvider) {
        try {
          ...
          writeStderrLine(
            `turbospark serve: getDaemonPreflightCells failed: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
      }
    ...
    async restartMcpServer(
      ...
      if ('entries' in result) {
        ...
        if (!Array.isArray(result.entries)) {
          writeStderrLine(
            `turbospark serve: pool restart response carried 'entries' field ` +
        ...
        for (const entry of entries) {
          if (
            ...
            writeStderrLine(
              `turbospark serve: skipping malformed pool restart entry ` +
    ...
    async reload(ctx: WorkspaceRequestContext) {
      if (deps.reloadDaemonEnv) {
        try {
          ...
          writeStderrLine(
            `turbospark serve: daemon reload failed: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
      }
      ...
      try {
        ...
        if (err instanceof SessionNotFoundError) {
          ...
          writeStderrLine(`turbospark serve: reload failed: ${childError}`);
        }
      }
    ...
    async refreshExtensionsForAllSessions() {
      try {
        ...
        writeStderrLine(
          `turbospark serve: refreshExtensionsForAllSessions failed: ${err instanceof Error ? err.message : String(err)}`,
        );
