 ...
 * silently re-enable the P2-1 bug (init writes default `TURBOSPARK.md`
 ...
 * the runTurbosparkServe boot path itself stays integration-tested
 * end-to-end via the daemon-process tests in
 * `integration-tests/cli/turbospark-serve-routes.test.ts`.
 */
 ...
 * during `runTurbosparkServe` boot + shutdown. Uses a fake bridge to avoid
 * spawning real `qwen --acp` child processes.
 */
describe('runTurbosparkServe daemon logger wiring', () => {
  let tmpDir: string;
  ...
  it('creates a daemon log file at boot and flushes on shutdown', async () => {
    ...
    // Minimal fake bridge satisfying the shape runTurbosparkServe expects.
    ...
    const origEnv = process.env['TURBOSPARK_RUNTIME_DIR'];
    process.env['TURBOSPARK_RUNTIME_DIR'] = tmpDir;
    ...
    try {
      ...
      if (origEnv !== undefined) {
        process.env['TURBOSPARK_RUNTIME_DIR'] = origEnv;
      }
    }
  });
});
...
/**
 * Boot validation for the embedded `runTurbosparkServe` API: a non-finite
 ...
 */
describe('runTurbosparkServe permissionResponseTimeoutMs validation', () => {
  let tmpDir: string;
  ...
  it('rejects a non-finite permissionResponseTimeoutMs', async () => {
    ...
    // the validation throw doesn't write into the real ~/.turbospark.
    const origEnv = process.env['TURBOSPARK_RUNTIME_DIR'];
    process.env['TURBOSPARK_RUNTIME_DIR'] = tmpDir;
    try {
      await expect(
...
describe('runTurbosparkServe Web Shell signals on RunHandle', () => {
  let tmpDir: string;
