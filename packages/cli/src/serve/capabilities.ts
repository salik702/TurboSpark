...
export const SERVE_CAPABILITY_REGISTRY = {
  ...
  // hierarchical TURBOSPARK.md state and accepts append/replace writes scoped
  // to either the bound workspace or the global ~/.turbospark directory.
  ...
  // `TURBOSPARK.md` (or whatever `getCurrentGeminiMdFilename()` returns) at
  ...
  // pool (`TurbosparkAgent.mcpPool`); `GET /workspace/mcp` reflects pool-level
