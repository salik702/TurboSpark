...
import { runTurbosparkServe, type RunHandle } from './runTurbosparkServe.js';
...
import * as qwenCore from '@turbospark/turbospark-core';
import {
...
describe('createServeApp', () => {
  ...
  describe('Web Shell static serving', () => {
    ...
    const INDEX_HTML =
      '<!doctype html><html><head><title>TURBO SPARK Web terminal</title>' +
  ...
  describe('GET /capabilities', () => {
    ...
    it('omits mcp_workspace_pool / mcp_pool_restart when mcpPoolActive=false (F2 #4175 commit 5)', async () => {
      // Mirrors the env-var kill switch path: `runTurbosparkServe.ts` infers
  ...
  describe('GET /workspace/:id/sessions', () => {
    ...
    beforeEach(async () => {
      previousRuntimeDir = process.env['TURBOSPARK_RUNTIME_DIR'];
    ...
    afterEach(async () => {
      if (previousRuntimeDir === undefined) {
        ...
        process.env['TURBOSPARK_RUNTIME_DIR'] = previousRuntimeDir;
      }
  ...
  describe('POST /workspace/init (#4175 Wave 4 PR 17)', () => {
    ...
    it('200 with action:created and force=false on success', async () => {
      ...
      try {
        ...
        expect(res.body.path).toContain('TURBOSPARK.md');
      } finally {
    ...
    it('forwards force:true to the bridge', async () => {
      // Create a workspace with an existing non-empty TURBOSPARK.md to trigger
      // the conflict → force:true overwrite path.
      const wsRoot = await fsp.mkdtemp(
        path.join(os.tmpdir(), 'qwen-init-force-'),
      );
      try {
        await fsp.writeFile(path.join(wsRoot, 'TURBOSPARK.md'), 'existing content');
        const bridge = fakeBridge();
        const opts: ServeOptions = {
          ...baseOpts,
          token: 'secret',
    ...
    it('409 with structured payload when bridge throws WorkspaceInitConflictError', async () => {
      ...
      try {
        await fsp.writeFile(
          path.join(wsRoot, 'TURBOSPARK.md'),
  ...
  describe('bearer auth', () => {
    ...
    it('gates /health behind bearer auth when --require-auth is set on loopback (#4175 PR 15)', async () => {
      ...
      // `runTurbosparkServe` guarantees a token whenever the flag is on,
  ...
  describe('GET /daemon/status', () => {
    ...
    it('returns summary diagnostics without querying workspace status', async () => {
      ...
      const app = createServeApp(baseOpts, undefined, {
        bridge,
        daemonLog,
        turbosparkCodeVersion: '1.2.3-test',
      });
...
describe('runTurbosparkServe', () => {
  ...
  afterEach(async () => {
    ...
    delete process.env['TURBOSPARK_SERVER_TOKEN'];
  ...
  it("starts with --allow-origin '*' when a token is configured", async () => {
    handle = await runTurbosparkServe({
      hostname: '127.0.0.1',
      port: 0,
      mode: 'http-bridge',
      token: 'secret',
  ...
  it('uses normalized token for session shell capability across REST and ACP initialize', async () => {
    ...
    const init = (await initRes.json()) as {
      result: { agentCapabilities: { _meta: { turbospark: { methods: string[] } } } };
    };
    expect(init.result.agentCapabilities._meta.turbospark.methods).toContain(
      '_qwen/session/shell',
    );
  });
  ...
  it('warns and does not advertise session shell when flag is set without a token', async () => {
    ...
    expect(init.result.agentCapabilities._meta.turbospark.methods).not.toContain(
      '_qwen/session/shell',
    );
  });

  // PR 14 fix (review #4247): runTurbosparkServe is the documented embedded
  ...
  // `runTurbosparkServe` would have passed all existing tests. Mirror the
  ...
  // mutation tests. `runTurbosparkServe` now passes per-handle env
  ...
  // below assert (a) runTurbosparkServe doesn't touch process.env and
  // (b) a pre-existing process.env value survives runTurbosparkServe
  // calls unrelated to MCP overrides (proving runTurbosparkServe is no
  ...
  it('preserves pre-existing process.env values (no longer wipes globals on omit) (#4247 R6 line 216)', async () => {
    ...
    // process.env. Post-R6 runTurbosparkServe doesn't touch process.env
  ...
  it('accepts TURBOSPARK_SERVER_TOKEN from the env when binding non-loopback', async () => {
    process.env['TURBOSPARK_SERVER_TOKEN'] = 'env-secret';
    handle = await runTurbosparkServe({
      hostname: '0.0.0.0',
      port: 0,
      mode: 'http-bridge',
  ...
  it('strips brackets from `[::1]` before passing to app.listen()', async () => {
    ...
    // would fail with ENOTFOUND. The fixup is in runTurbosparkServe's
  ...
  it('wires fsFactory + emit through to the read routes (#4175 PR 19 follow-up #2)', async () => {
    // Pin the contract that `runTurbosparkServe` constructs the workspace
  ...
  it('honors deps.fsFactory override (#4175 PR 19 follow-up #2)', async () => {
    ...
    // `runTurbosparkServe` does NOT silently shadow a caller-supplied
  ...
  it('trust snapshot=false flows through deps.trustedWorkspace into the boundary (#4175 PR 19 follow-up #2)', async () => {
    ...
    // construct the same factory that runTurbosparkServe would build,
    // with the same `trusted` value runTurbosparkServe would pass, and
    ...
    try {
      // Mirror runTurbosparkServe's construction. If `runTurbosparkServe`
  ...
  it('detaches its SIGINT/SIGTERM listeners after close completes', async () => {
    ...
    // runTurbosparkServe attaches one of each.
    ...
    // After drain completes, the listener that runTurbosparkServe added is gone.
...
describe('GET /session/:id/events (SSE)', () => {
  ...
  it('stamps errorKind on stream_error when the thrown error is classified (#4175 F4 prereq, chiga0 #19 P0)', async () => {
    ...
    // string.
    const { BridgeTimeoutError } = await import('@turbospark/acp-bridge');
    const bridge = fakeBridge({
      async *subscribeImpl(_sessionId, _opts) {
        yield { id: 1, v: 1, type: 'session_update', data: 'first' };
...
describe('runTurbosparkServe SIGINT handler', () => {
  it('does not register signal handlers until the listener is up', () => {
    ...
    expect(typeof runTurbosparkServe).toBe('function');
...
describe('auth device-flow routes', () => {
  ...
  function makeFakeProvider(): {
    ...
    return {
      provider: {
        providerId: 'turbospark-oauth' as const,
        async start() {
          starts += 1;
          return {
            deviceCode:
  ...
  it('POST /workspace/auth/device-flow returns 201 on fresh start with redacted body', async () => {
    ...
    const res = await request(app)
      ...
      .set('Host', `127.0.0.1:${baseOpts.port}`)
      .send({ providerId: 'turbospark-oauth' });
    expect(res.status).toBe(201);
    expect(res.body.providerId).toBe('turbospark-oauth');
  ...
  it('POST with unknown providerId returns 400 unsupported_provider', async () => {
    ...
    expect(res.body.supportedProviders).toContain('turbospark-oauth');
  });
  ...
  it('GET /workspace/auth/status surfaces pending flows and supported providers', async () => {
    ...
    expect(status.body.supportedDeviceFlowProviders).toContain('turbospark-oauth');
  ...
  it('sweeper-driven auto-expiry transitions a stale entry to status:error and surfaces over GET', async () => {
    ...
    const registry = new DeviceFlowRegistry({
      events: { publish: () => {} },
      resolveProvider: (id) => (id === 'turbospark-oauth' ? fakeProvider : undefined),
      now: () => now,
...
describe('T2.9 prompt absolute deadline (issue #4514)', () => {
  ...
  describe('POST /session/:id/prompt', () => {
    ...
    it('returns cleanly when client disconnects in the same tick the deadline fires (wenshao review #3)', async () => {
      ...
      const localHandle = await runTurbosparkServe(
        {
          hostname: '127.0.0.1',
          port: 0,
          mode: 'http-bridge',
...
describe('T2.9 serve-side errorKind taxonomy (issue #4514)', () => {
  it('publishes the new error kinds in SERVE_ERROR_KINDS', async () => {
    ...
    // fails CI.
    const { SERVE_ERROR_KINDS } = await import('@turbospark/acp-bridge/status');
