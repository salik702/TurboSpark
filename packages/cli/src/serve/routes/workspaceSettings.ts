...
export function registerWorkspaceSettingsRoutes(
  ...
  app.get('/workspace/settings', (_req: Request, res: Response) => {
    try {
      ...
      writeStderrLine(
        `turbospark serve: GET /workspace/settings error: ${
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        writeStderrLine(
          `turbospark serve: POST /workspace/settings persist error (key=${key}, scope=${scope}, workspace=${boundWorkspace}): ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
        res.status(500).json({
      ...
      try {
        ...
        writeStderrLine(
          `turbospark serve: POST /workspace/settings broadcast error (key=${key}, scope=${scope}): ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
      }
