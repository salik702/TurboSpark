...
import type { PermissionPolicy } from '@turbospark/acp-bridge';
import { getCliVersion } from '../utils/version.js';
import { getRateLimiter } from './rateLimit.js';
import type { AcpHttpHandle } from './acpHttp/index.js';

const TURBOSPARK_SERVER_TOKEN_ENV = 'TURBOSPARK_SERVER_TOKEN';
...
/**
 * Boot-time policy validation error. The catch block in `runTurbosparkServe`
...
export function validatePolicyConfig(
  ...
  if (
    ...
    throw new InvalidPolicyConfigError(
      `turbospark serve: invalid policy.permissionStrategy ` +
  ...
  if (
    ...
    throw new InvalidPolicyConfigError(
      `turbospark serve: invalid policy.consensusQuorum ` +
  ...
  if (
    ...
    onWarning(
      'turbospark serve: policy.consensusQuorum is set but ' +
...
export interface RunTurbosparkServeDeps {
  ...
  /**
   * Workspace filesystem factory. When omitted, `runTurbosparkServe`
   ...
   * actually wired through `runTurbosparkServe → createServeApp →
   * fsFactory`.
   */
...
function shouldPreheatBridge(deps: RunTurbosparkServeDeps): boolean {
 ...
 *   2. `TURBOSPARK_SERVER_TOKEN` env var
 *
 ...
 */
export async function runTurbosparkServe(
  optsIn: Omit<ServeOptions, 'token'> & { token?: string },
  deps: RunTurbosparkServeDeps = {},
): Promise<RunHandle> {
  // Trim both sources. Common gotcha: `export TURBOSPARK_SERVER_TOKEN=$(cat
  ...
  const rawToken = optsIn.token ?? process.env[TURBOSPARK_SERVER_TOKEN_ENV];
  const token =
  ...
  if (optsIn.enableSessionShell === true && token === undefined) {
    writeStderrLine(
      `turbospark serve: --enable-session-shell ignored because no bearer token ` +
        `is configured. Set ${TURBOSPARK_SERVER_TOKEN_ENV} or pass --token to ` +
  ...
  if (!isLoopbackBind(opts.hostname) && !token) {
    throw new Error(
      `Refusing to bind ${opts.hostname}:${opts.port} without a bearer token. ` +
        `Set ${TURBOSPARK_SERVER_TOKEN_ENV} or pass --token, or rebind to loopback ` +
  ...
  if (opts.requireAuth && !token) {
    throw new Error(
      `Refusing to start with --require-auth set but no bearer token ` +
        `configured. Set ${TURBOSPARK_SERVER_TOKEN_ENV} or pass --token, or omit ` +
    ...
    if (parsed.allowAny && !token) {
      throw new Error(
        `Refusing to start with --allow-origin '*' but no bearer token ` +
          ...
          `${TURBOSPARK_SERVER_TOKEN_ENV} or pass --token, or list specific ` +
          `origins instead of '*'.`,
      );
    }
    writeStderrLine(
      `turbospark serve: --allow-origin: ${opts.allowOrigins.join(', ')}` +
        (parsed.allowAny
  ...
  if (opts.allowPrivateAuthBaseUrl) {
    writeStderrLine(
      'turbospark serve: --allow-private-auth-base-url enabled; ' +
  ...
  // `runTurbosparkServe({ permissionResponseTimeoutMs })` also fail loud: the
  ...
  try {
    ...
    writeStderrLine(
      `turbospark serve: could not read settings for context.fileName / ` +
  ...
  daemonTelemetry.metrics = {
    sessionLifecycle(action) {
      ...
      emitDaemonLog(
        `Session ${action}.`,
        {
          'turbospark.workspace.hash': daemonWorkspaceHash,
        },
        {
          eventName: `turbospark.daemon.session.${action}`,
        },
      );
    },
    channelLifecycle(action, expected) {
      ...
      emitDaemonLog(
        ...
        {
          ...(action === 'exit'
            ? { 'turbospark.daemon.channel.expected': expected ?? true }
            : {}),
        },
        {
          eventName: `turbospark.daemon.channel.${action}`,
  ...
  const workspaceService = createDaemonWorkspaceService({
    boundWorkspace,
    contextFilename: contextFilenameForInit ?? 'TURBOSPARK.md',
  ...
  if (opts.serveWebShell !== false) {
    if (!webShellDir) {
      writeStderrLine(
        'turbospark serve: Web Shell assets not found; serving API only. ' +
      ...
      writeStderrLine(`turbospark serve: Web Shell UI served from ${webShellDir}`);
      if (!isLoopbackBind(opts.hostname)) {
        writeStderrLine(
          'turbospark serve: Web Shell UI is served WITHOUT auth on a non-loopback ' +
        ...
        if (!opts.allowOrigins || opts.allowOrigins.length === 0) {
          writeStderrLine(
            'turbospark serve: without --allow-origin the Web Shell is read-only on a ' +
  ...
  const app = createServeApp(opts, () => actualPort, {
    ...
    webShellDir,
    boundWorkspace,
    turbosparkCodeVersion: cliVersion,
    fsFactory,
    daemonLog,
      ...
      withSettingsLock(
        ...
        async (): Promise<ServeAuthProviderInstallResult> => {
          ...
          emitDaemonLog('Auth provider installed.', {
            'turbospark.daemon.auth.provider_id': provider.id,
            'turbospark.daemon.auth.auth_type': plan.authType,
          });
          return {
            v: 1,
  ...
  // `turbospark serve --hostname [::1]` would pass the loopback/token check
  ...
  return await new Promise<RunHandle>((resolve, reject) => {
    const server = app.listen(opts.port, listenHostname, () => {
      ...
      writeStdoutLine(
        `turbospark serve listening on ${url} (mode=${opts.mode}, ` +
      ...
      writeStderrLine(
        `turbospark serve: bound to workspace ${JSON.stringify(boundWorkspace)}`,
      );
      if (!token) {
        writeStderrLine(
          `turbospark serve: bearer auth disabled (loopback default). Set ${TURBOSPARK_SERVER_TOKEN_ENV} to enable.`,
        );
        ...
        writeStderrLine(
          'turbospark serve: --require-auth enabled (bearer token mandatory ' +
          ...
          closePromise = new Promise<void>((res, rej) => {
            ...
            const finish = (err?: Error | null) => {
              ...
              void shutdownTelemetry()
                .catch((telemetryErr) => {
                  writeStderrLine(
                    `turbospark serve: telemetry shutdown error: ${
      ...
      if (!deps.bridge && shouldPreheatBridge(deps)) {
        bridge.preheat().catch((err) => {
          writeStderrLine(
            `turbospark serve: ACP preheat failed, will retry on first session: ${
              err instanceof Error ? err.message : String(err)
            }`,
          );
        });
