...
import { mapDomainErrorToErrorKind } from '@turbospark/acp-bridge';
import { QwenOAuthDeviceFlowProvider } from './auth/turbosparkDeviceFlowProvider.js';
 ...
 * logs instead of disappearing. `runTurbosparkServe` replaces this with a
...
export function createDefaultFsAuditEmit(): (event: BridgeEvent) => void {
  ...
  return (event: BridgeEvent) => {
    ...
    if (droppedCount === 1 || droppedCount % WARN_EVERY === 0) {
      ...
      writeStderrLine(
        `turbospark serve: fs audit emit is the default no-op — ${droppedCount} event(s) dropped so far. ` +
 ...
 * `runTurbosparkServe` and `createServeApp`'s default bridge wiring.
 ...
 *   - `runTurbosparkServe` defaults to `trusted: true`
...
export interface ServeAppDeps {
   ...
   * UI at the daemon root before `bearerAuth`. Production `runTurbosparkServe`
  ...
  webShellDir?: string;
  /**
   * TURBO SPARK version advertised to web/SDK clients. Production passes the
   ...
   * boot-time stat we can avoid). `runTurbosparkServe` passes this after
   ...
   * used by interactive `/auth`. Production `runTurbosparkServe` injects a
...
/**
 * Build the Express app for `turbospark serve`. Pure function — no side effects on
 * the network or process; `runTurbosparkServe` does the listen/signal handling.
 *
 ...
 * path. `runTurbosparkServe` is the production entry point and DOES
 ...
 * `runTurbosparkServe` validation (or call into a shared helper if one is
...
export function createServeApp(
  ...
  // only the `runTurbosparkServe` path piped the option through.
  //
  ...
  // `runTurbosparkServe`; when omitted we canonicalize ourselves.
  ...
  if (!deps.fsFactory && !deps.bridge && !warnedDefaultTrust) {
    ...
    process.stderr.write(
      'turbospark serve: createServeApp default fsFactory uses trusted=false ' +
  ...
  if (!deviceFlowProviderMap.has('turbospark-oauth')) {
    deviceFlowProviderMap.set('turbospark-oauth', new QwenOAuthDeviceFlowProvider());
  }
  ...
  // `runTurbosparkServe`'s shutdown dispose call.
  ...
  const workspace: DaemonWorkspaceService =
    ...
    createDaemonWorkspaceService({
      boundWorkspace,
      contextFilename: deps.contextFilename ?? 'TURBOSPARK.md',
      statusProvider: createDaemonStatusProvider(),
    ...
    void enqueueExtensionInstall(async () => {
      try {
        ...
        try {
          ...
          writeStderrLine(
            `turbospark serve: extensions ${operation}: refreshed ${result.refreshed} session(s), ${result.failed} failed`,
          );
        } catch (refreshErr) {
          ...
          try {
            ...
            writeStderrLine(
              `turbospark serve: extensions ${operation}: failed to broadcast refresh failure: ${
          ...
          writeStderrLine(
            `turbospark serve: extensions ${operation}: mutation succeeded but refresh failed: ${message}`,
          );
        }
      } catch (err) {
        const message = redactUrlCredentials(
        ...
        try {
          ...
          writeStderrLine(
            `turbospark serve: extensions ${operation}: failed to broadcast failure: ${
              broadcastErr instanceof Error
                ? redactUrlCredentials(broadcastErr.message)
                : String(broadcastErr)
            }`,
        ...
        try {
          writeStderrLine(
            `turbospark serve: extensions ${operation}: background task failed: ${message}`,
          );
        } catch {
      ...
      try {
        writeStderrLine(
          `turbospark serve: extensions ${operation}: queued task failed: ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
      } catch {
  ...
  // installed. Pattern parsing happens in `runTurbosparkServe.ts` for validation;
  ...
  const demoHandler = (
    ...
    try {
      ...
      writeStderrLine(
        `turbospark serve: /demo render failed: ${err instanceof Error ? err.message : String(err)}`,
      );
  ...
  // arbitrary IP:port to confirm a `turbospark serve` is listening), so
  ...
  // (`docs/users/turbospark-serve.md` + `turbospark-serve-protocol.md`) clarify
  ...
  const healthHandler = (
    ...
    try {
      ...
      writeStderrLine(
        `turbospark serve: /health deep probe failed: ${err instanceof Error ? err.message : String(err)}`,
      );
    ...
    app.use((req, res, next) => {
      ...
      res.on('finish', () => {
        try {
          ...
          const clientId = req.headers['x-turbospark-client-id'] as
            | string
            | undefined;
          const ctx = {
  ...
  // (runTurbosparkServe) and injected via deps.webShellDir; `--no-web` sets
    ...
    try {
      res.status(200).json(
        await buildDaemonStatusResponse(detail.detail, {
          ...
          workspace,
          daemonLog,
          turbosparkCodeVersion: deps.turbosparkCodeVersion,
      ...
      writeStderrLine(
        `turbospark serve: /daemon/status failed: ${err instanceof Error ? err.message : String(err)}`,
      );
      res.status(500).json({
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      if (!callerIsDeviceFlowInitiator(view, clientId) && isServeDebugMode()) {
        writeStderrLine(
          `turbospark serve debug: GET /workspace/auth/device-flow/${id} redacted verification fields — caller-clientId mismatch (initiator=${view.initiatorClientId ?? 'anonymous'}, caller=${clientId ?? 'anonymous'})`,
        );
      }
  ...
  app.post('/session/:id/prompt', mutate(), async (req, res) => {
    ...
    addDaemonRequestAttribute('turbospark.prompt_id', promptId);
  ...
  app.post('/sessions/delete', mutate(), async (req, res) => {
    ...
    try {
      ...
      for (let i = 0; i < closeResults.length; i++) {
        ...
        if (r.status === 'fulfilled') {
          ...
          if (closeErr instanceof SessionNotFoundError) {
            ...
            writeStderrLine(
              `turbospark serve: closeSession failed for ${safeLogValue(id)}: ${safeLogValue(msg)}`,
            );
      ...
      for (const e of result.errors) {
        ...
        writeStderrLine(
          `turbospark serve: removeSession failed for ${safeLogValue(e.sessionId)}: ${safeLogValue(msg)}`,
        );
      }
      res.status(200).json({
      ...
      writeStderrLine(
        `turbospark serve: failed to batch delete sessions: ${safeLogValue(message)}`,
      );
      res.status(500).json({
  ...
  app.get('/workspace/:id/sessions', async (req, res) => {
    ...
    try {
      ...
      writeStderrLine(
        `turbospark serve: failed to list sessions for workspace ${safeLogValue(
          key,
  ...
  app.post('/workspace/init', mutate({ strict: true }), async (req, res) => {
    ...
    // writes an empty TURBOSPARK.md without invoking the LLM. Default refuses
  ...
  app.get('/session/:id/events', (req, res) => {
    ...
    try {
      ...
      if (err instanceof SubscriberLimitExceededError) {
        writeStderrLine(
          `turbospark serve: subscriber limit reached for session ${sessionId} (limit=${err.limit}); rejecting new SSE client with 429`,
        );
    ...
    if (daemonLog) {
      ...
      const sseClientId = req.headers['x-turbospark-client-id'] as string | undefined;
    ...
    if (trackWriterIdle) {
      ...
      idleTimer = setInterval(() => {
        ...
        try {
          writeStderrLine(
            `turbospark serve: evicting SSE client (session ${sessionId}) — ` +
    ...
    res.on('error', (err) => {
      ...
      writeStderrLine(
        `turbospark serve: SSE socket error (session ${sessionId}): ${err.message}`,
      );
      cleanup();
    });
    ...
    void (async () => {
      try {
          ...
          if (next.value.type === 'state_resync_required') {
            ...
            const gap =
              ...
              typeof data.lastDeliveredId === 'number'
                ...
                : undefined;
            writeStderrLine(
              `turbospark serve: SSE ring eviction detected (session ${sessionId}): ` +
        ...
        if (!res.writableEnded) {
          ...
          writeStderrLine(
            `turbospark serve: bridge iterator error (session ${sessionId}): ` +
              `${errorMessage(err)}` +
              (errorKind ? ` [${errorKind}]` : ''),
          );
  ...
  app.use(
    (
      ...
      if (sendJsonBodyParserError(res, err)) return;
      writeStderrLine(
        `turbospark serve: unhandled error: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`,
      );
      if (!res.headersSent) {
...
const CLIENT_ID_HEADER = 'x-turbospark-client-id';
...
function parseMaxQueuedQuery(
  ...
  if (typeof raw !== 'string' || !/^\d+$/.test(raw)) {
    ...
    // `sendBridgeError`.
    writeStderrLine(
      `turbospark serve: rejected ?maxQueued ${safeLogValue(raw)} ` +
...
function parseLastEventId(raw: unknown): number | undefined {
  ...
  if (typeof raw !== 'string' || !/^\d+$/.test(raw)) {
    ...
    if (typeof raw === 'string' && raw.length > 0) {
      writeStderrLine(
        `turbospark serve: rejected Last-Event-ID ${safeLogValue(raw)} ` +
          `(not a decimal integer)`,
      );
    }
    return undefined;
...
function sendBridgeErrorImpl(
  ...
  if (err instanceof WorkspaceMismatchError) {
    ...
    // readability.
    writeStderrLine(
      `turbospark serve: workspace_mismatch (POST /session): ` +
  ...
  if (daemonLog) {
    ...
    writeStderrLine(
      `turbospark serve: bridge error${ctxStr}: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`,
    );
  }
