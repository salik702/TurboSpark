...
import { resolveBundleDir } from '@turbospark/turbospark-core';
...
function createSendIndex(webShellDir: string): (res: Response) => void {
  ...
  return (res: Response): void => {
    ...
    res.sendFile(indexPath, { cacheControl: false }, (err) => {
      ...
      writeStderrLine(
        `turbospark serve: Web Shell index send failed: ${err instanceof Error ? err.message : String(err)}`,
      );
      if (!res.headersSent) {
...
export function mountWebShellAssets(
  ...
  app.use('/assets', (req: Request, res: Response) => {
    ...
    if (isServeDebugMode()) {
      writeStderrLine(
        `turbospark serve: Web Shell asset not found: ${req.originalUrl}`,
      );
    }
...
export function mountWebShellSpaFallback(
  ...
  app.use((req: Request, res: Response, next: NextFunction) => {
    ...
    if (isServeDebugMode()) {
      writeStderrLine(
        `turbospark serve: Web Shell SPA fallback served for ${req.method} ${req.originalUrl}`,
      );
    }
    sendIndex(res);
  });
