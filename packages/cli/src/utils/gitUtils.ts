...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/turbospark/turbospark-action/releases/latest`;
    ...
    debugLogger.debug(
      `Failed to determine latest turbospark-action release:`,
      _error,
    );
    throw new Error(
      `Unable to determine the latest turbospark-action release on GitHub.`,
    );
  }
};
