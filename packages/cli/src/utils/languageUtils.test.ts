...
describe('languageUtils', () => {
  ...
  describe('writeOutputLanguageFile', () => {
    ...
    it('should create directory and write file', () => {
      ...
      const globalDir = '/mock/home/.turbospark';
    ...
    it('should include machine-readable marker', () => {
      ...
      const writtenContent = vi.mocked(fs.writeFileSync).mock.calls[0][1];
      expect(writtenContent).toContain(
        '<!-- turbospark:llm-output-language: Chinese -->',
      );
    });
    ...
    it('should sanitize language for marker (remove dangerous characters)', () => {
      ...
      expect(writtenContent).toContain(
        '<!-- turbospark:llm-output-language: TestLanguage -->',
      );
    });
    ...
    it('should write to custom targetPath when provided', () => {
      writeOutputLanguageFile('Korean', '/proj/.turbospark/output-language.md');

      expect(fs.mkdirSync).toHaveBeenCalledWith('/proj/.turbospark', {
        recursive: true,
      });
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        '/proj/.turbospark/output-language.md',
...
<!-- turbospark:llm-output-language: French -->
`,
      );
...
`,
  ...
  describe('output-language.md path resolution priority', () => {
    it('should prefer project-level path over global path', () => {
      const projectPath = '/project/.turbospark/output-language.md';
      const globalPath = '/mock/home/.turbospark/output-language.md';
    ...
    it('writes to config-bound path when available', () => {
      const config = {
        getOutputLanguageFilePath: vi
          .fn()
          .mockReturnValue('/proj/.turbospark/output-language.md'),
