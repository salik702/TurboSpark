...
import type { Config, SandboxConfig } from '@turbospark/turbospark-core';
import {
  FatalSandboxError,
  Storage,
  isSubpath,
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'turbospark-sandbox';
const SANDBOX_NETWORK_NAME = 'turbospark-sandbox';
const SANDBOX_PROXY_NAME = 'turbospark-sandbox-proxy';
...
export async function start_sandbox(
  ...
  if (config.command === 'sandbox-exec') {
    ...
    // on missing dirs and a custom TURBOSPARK_HOME / TURBOSPARK_RUNTIME_DIR may not exist
    // yet on first run.
    ...
    const args = [
      ...
      '-D',
      `QWEN_DIR=${fs.realpathSync(turbosparkDir)}`,
      '-D',
    ...
    // start and set up proxy if TURBOSPARK_SANDBOX_PROXY_COMMAND is set
    const proxyCommand = process.env['TURBOSPARK_SANDBOX_PROXY_COMMAND'];
  ...
  // determine full path for turbospark to distinguish linked vs installed setting
  ...
  // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under turbospark repo
  //
  // note this can only be done with binary linked from turbospark repo
  if (process.env['BUILD_SANDBOX']) {
    if (!gcPath.includes('turbospark/packages/')) {
      throw new FatalSandboxError(
        'Cannot build sandbox using installed TURBO SPARK binary; ' +
      ...
      execSync(
        `cd ${gcRoot} && node scripts/build_sandbox.js -s ${buildArgs}`,
        {
          ...
          env: {
            ...process.env,
            TURBOSPARK_SANDBOX: config.command, // in case sandbox is enabled via flags (see config.ts under cli package)
          },
        },
      );
    }
  ...
  if (!(await ensureSandboxImageIsPresent(config.command, image))) {
    const remedy =
      image === LOCAL_DEV_SANDBOX_IMAGE_NAME
        ? 'Try running `npm run build:all` or `npm run build:sandbox` under the turbospark repo to build it locally, or check the image name and your network connection.'
        : 'Please check the image name, your network connection, or notify turbospark-dev@service.alibaba.com if the issue persists.';
    throw new FatalSandboxError(
  ...
  // Mount user settings at /home/node/.turbospark and at the canonical host path
  // used by TURBOSPARK_HOME, unless that host path is already covered by a broader
  ...
  // Pass TURBOSPARK_HOME so the sandboxed CLI resolves the global qwen dir to the
  // same path the host did, instead of relying on the /home/node/.turbospark mount
  // being the default fallback.
  args.push('--env', `TURBOSPARK_HOME=${userSettingsDirContainerPath}`);

  // Mount the runtime base dir and pass TURBOSPARK_RUNTIME_DIR when it diverges
  ...
  if (!runtimeSameAsUserSettings) {
    args.push('--env', `TURBOSPARK_RUNTIME_DIR=${runtimeBaseDirContainerPath}`);
  }
  ...
  // TURBOSPARK_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
  const proxyCommand = process.env['TURBOSPARK_SANDBOX_PROXY_COMMAND'];
  ...
  const isIntegrationTest =
    process.env['TURBOSPARK_INTEGRATION_TEST'] === 'true';
  let containerName;
  if (isIntegrationTest) {
    containerName = `turbospark-integration-test-${randomBytes(4).toString(
      'hex',
    )}`;
  ...
  // copy TURBOSPARK_TEST_VAR for integration tests
  if (process.env['TURBOSPARK_TEST_VAR']) {
    args.push(
      '--env',
      `TURBOSPARK_TEST_VAR=${process.env['TURBOSPARK_TEST_VAR']}`,
    );
  }
  for (const envVar of [
    'QWEN_DEBUG_LOG_FILE',
    'TURBOSPARK_LEGACY_MCP_BLOCKING',
  ] as const) {
  ...
  if (process.env['TURBOSPARK_MCP_APPROVALS_PATH']) {
    args.push(
      '--env',
      `TURBOSPARK_MCP_APPROVALS_PATH=${getContainerPath(
  ...
  // tests that need to access host's ~/.turbospark (e.g., --resume functionality)
  ...
  if (useCurrentUser) {
    ...
    // allowing tests that need to access host's ~/.turbospark (e.g., --resume) to work.
