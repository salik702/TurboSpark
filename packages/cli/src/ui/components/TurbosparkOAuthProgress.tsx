...
interface TurbosparkOAuthProgressProps {
  onTimeout: () => void;
  onCancel: () => void;
...
export function TurbosparkOAuthProgress({
  onTimeout,
  onCancel,
  deviceAuth,
  authStatus,
  authMessage,
}: TurbosparkOAuthProgressProps): React.JSX.Element {
    ...
    return (
      ...
      >
        <Text bold color={theme.status.error}>
          {t('TURBOSPARK OAuth Authentication Timeout')}
        </Text>
  ...
  if (authStatus === 'error') {
    return (
      ...
      >
        <Text bold color={theme.status.error}>
          {t('TURBOSPARK OAuth Authentication Error')}
        </Text>
  ...
  if (!deviceAuth) {
    return (
      <Box
        ...
        width="100%"
      >
        <Text bold>{t('TURBOSPARK OAuth Authentication')}</Text>

        <Box marginTop={1} flexDirection="column">
          <Text>{t('Waiting for TURBOSPARK OAuth authentication...')}</Text>
          <Text>
