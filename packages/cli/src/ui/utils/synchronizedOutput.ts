...
export function terminalSupportsSynchronizedOutput(
  ...
  if (
    env['TURBOSPARK_DISABLE_SYNCHRONIZED_OUTPUT'] === '1' ||
    env['TURBOSPARK_SYNCHRONIZED_OUTPUT'] === '0'
  ) {
    return false;
  }

  if (
    env['TURBOSPARK_FORCE_SYNCHRONIZED_OUTPUT'] === '1' ||
    env['TURBOSPARK_SYNCHRONIZED_OUTPUT'] === '1'
  ) {
    return true;
  }
