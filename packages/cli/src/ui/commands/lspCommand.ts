  ...
  action: async (
    ...
    if (!config.isLspEnabled()) {
      return emitMessage(
        ...
        'info',
        t(
          'LSP is not enabled. Start TURBO SPARK with `--experimental-lsp` to enable LSP support.',
        ),
      );
    }
    ...
    if (!client) {
      return emitMessage(
        ...
        'info',
        t(
          'LSP is enabled but no client is connected. Check debug logs under `${TURBOSPARK_RUNTIME_DIR:-~/.turbospark}/debug/` or see the LSP troubleshooting docs.',
        ),
      );
    }
