...
export const summaryCommand: SlashCommand = {
  ...
  action: async (context): Promise<SlashCommandActionReturn> => {
    ...
    const saveSummaryToDisk = async (
      ...
      // Ensure .turbospark directory exists
      const projectRoot = config.getProjectRoot();
      const turbosparkDir = path.join(projectRoot, '.turbospark');
      try {
        await fsPromises.mkdir(turbosparkDir, { recursive: true });
      } catch (_err) {
        // Directory might already exist, ignore error
      }

      // Save the summary to PROJECT_SUMMARY.md
      const summaryPath = path.join(turbosparkDir, 'PROJECT_SUMMARY.md');
...
`;
      ...
      return {
        filePathForDisplay: '.turbospark/PROJECT_SUMMARY.md',
