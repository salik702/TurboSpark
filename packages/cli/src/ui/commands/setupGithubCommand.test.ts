...
describe('setupGithubCommand', async () => {
  ...
  it('returns a tool action to download github workflows and handles paths', async () => {
    ...
    const expectedSubstrings = [
      `set -eEuo pipefail`,
      `fakeOpenCommand "https://github.com/turbospark/turbospark-action`,
    ];
    ...
    if (gitignoreExists) {
      ...
      expect(gitignoreContent).toContain('.turbospark/');
...
describe('updateGitignore', () => {
  ...
  it('creates a new .gitignore file when none exists', async () => {
    ...
    expect(content).toBe('.turbospark/\ngha-creds-*.json\n');
  });
  ...
  it('does not add duplicate entries', async () => {
    const gitignorePath = path.join(scratchDir, '.gitignore');
    const existingContent = '.turbospark/\nsome-other-file\ngha-creds-*.json\n';
    await fs.writeFile(gitignorePath, existingContent);
  ...
  it('adds only missing entries when some already exist', async () => {
    ...
    // Should not duplicate .turbospark/ entry
    expect((content.match(/\.turbospark\//g) || []).length).toBe(1);
  });

  it('does not get confused by entries in comments or as substrings', async () => {
    const gitignorePath = path.join(scratchDir, '.gitignore');
    const existingContent = [
      '# This is a comment mentioning .turbospark/ folder',
      'my-app.turbospark/config',
    ...
    expect(content).toContain('.turbospark/');
    expect(content).toContain('gha-creds-*.json');
    ...
    const lines = content
      ...
      .filter((line) => line);
    expect(lines).toContain('.turbospark/');
    expect(lines).toContain('gha-creds-*.json');
    expect(lines).toContain('my-app.turbospark/config');
