...
export async function resolveAtCommandQuery({
  ...
  for (const atPathPart of atPathCommandParts) {
    ...
    const qwenIgnored =
      respectFileIgnore.respectTurbosparkIgnore &&
      fileDiscovery.shouldIgnoreFile(pathName, {
    ...
    if (gitIgnored || qwenIgnored) {
      const reason =
        gitIgnored && qwenIgnored ? 'both' : gitIgnored ? 'git' : 'turbospark';
  ...
  const totalIgnored =
    ...
    ignoredByReason['turbospark'].length +
  ...
  if (totalIgnored > 0) {
    ...
    if (ignoredByReason['turbospark'].length) {
      messages.push(`Qwen-ignored: ${ignoredByReason['turbospark'].join(', ')}`);
    }
