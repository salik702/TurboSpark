...
import type { Config, FileSearch } from '@turbospark/turbospark-core';
import { FileSearchFactory, escapePath } from '@turbospark/turbospark-core';
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        const searcher = FileSearchFactory.create({
          ...
          useTurbosparkignore:
            config?.getFileFilteringOptions()?.respectTurbosparkIgnore ?? true,
          cache: true,
    ...
    const search = async () => {
      ...
      try {
        ...
        // isDirectory relies on crawler.ts in @turbospark/turbospark-core
