...
describe('handleAtCommand', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getFileFilteringRespectTurbosparkIgnore: () => true,
  ...
  describe('qwen-ignore filtering', () => {
    it('should skip qwen-ignored files in @ commands', async () => {
      await createTestFile(
        path.join(testRootDir, '.turbosparkignore'),
        'build/output.js',
      );
  ...
  it('should process non-ignored files when .turbosparkignore is present', async () => {
    await createTestFile(
      path.join(testRootDir, '.turbosparkignore'),
      'build/output.js',
    );
