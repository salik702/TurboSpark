...
describe('FileCommandLoader - Extension Commands Support', () => {
  ...
  const projectCommandsDir = path.join(projectRoot, '.turbospark', 'commands');
  ...
  it('should load commands from extension with config.commands path', async () => {
    ...
    mock({
      ...
      [projectCommandsDir]: {},
      [extensionDir]: {
        'turbospark-extension.json': JSON.stringify(extensionConfig),
        'custom-cmds': {
          'test.md':
  ...
  it('should load commands from multiple extensions in alphabetical order', async () => {
    const ext1Dir = path.join(projectRoot, '.turbospark', 'extensions', 'ext-b');
    const ext2Dir = path.join(projectRoot, '.turbospark', 'extensions', 'ext-a');

    mock({
      [userCommandsDir]: {},
      [projectCommandsDir]: {},
      [ext1Dir]: {
        'turbospark-extension.json': JSON.stringify({
          name: 'ext-b',
          version: '1.0.0',
        }),
        commands: {
