...
import { memoryCommand, rememberCommand } from '../commands/memory.js';
...
function resolveLocaleForExtensions(settings: Settings): string {
  const envLang = process.env['TURBOSPARK_LANG'];
...
export interface CliArgs {
   ...
   * the CLI chdirs into `<repoRoot>/.turbospark/worktrees/<slug>/` and the entire
...
export async function parseArguments(): Promise<CliArgs> {
  ...
  if (
    rawArgv.length > 0 &&
    (rawArgv[0].endsWith('/dist/turbospark-cli/cli.js') ||
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .usage(
      'Usage: qwen [options] [command]\n\nTURBO SPARK - Launch an interactive CLI, use -p/--prompt for non-interactive mode',
    )
    .option('telemetry', {
      type: 'boolean',
      description:
    ...
    .option('proxy', {
      type: 'string',
      description: 'Proxy for TURBO SPARK, like schema://user:password@host:port',
    })
    .deprecateOption(
      'proxy',
    ...
    .command('$0 [query..]', 'Launch TURBO SPARK CLI', (yargsInstance: Argv) =>
      yargsInstance
        .positional('query', {
          description:
        ...
        .option('worktree', {
          type: 'string',
          description:
            'Start the session inside a git worktree at <repoRoot>/.turbospark/worktrees/<slug>/. ' +
    ...
    // Register `turbospark serve` (Stage 1 daemon)
    .command(serveCommand)
    // Register sessions subcommands
    .command(sessionsCommand)
    // Register doctor command for diagnostics
    .command(doctorCommand)
    // Register project context command
    .command(contextCommand)
    // Register persistent memory commands
    .command(memoryCommand)
    .command(rememberCommand)
    // Register debug / bug reproduction command
    .command(debugCommand)
    // Register one-command feature builder
    .command(addCommand)
    // Register smart refactor engine
    .command(refactorCommand)
    // Register multi-model brain switching
    .command(modelCommand)
    // Register explain project mode
  ...
  if (
    ...
    // forever (after the listener is up); SIGINT/SIGTERM in runTurbosparkServe
...
export async function loadCliConfig(
  ...
  // Set runtime output directory from settings (env var TURBOSPARK_RUNTIME_DIR
  // is auto-detected inside getRuntimeBaseDir() at each call site).
  // Pass cwd so that relative paths like ".turbospark" resolve per-project.
