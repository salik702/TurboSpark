...
import * as ServerConfig from '@turbospark/turbospark-core';
...
describe('loadCliConfig', () => {
  ...
  it('should reset context file names to TURBOSPARK.md and AGENTS.md by default', async () => {
  ...
  it('should reset context filenames to defaults when context.fileName is not configured', async () => {
    ...
    const argv = await parseArguments();
    const settings: Settings = {};
    const defaultContextFiles = ['TURBOSPARK.md', 'AGENTS.md'];
    const getAllSpy = vi
...
describe('loadCliConfig fileFiltering', () => {
  ...
  const testCases: Array<{
    ...
    {
      property: 'respectTurbosparkIgnore',
      getter: (c) => c.getFileFilteringRespectTurbosparkIgnore(),
      value: true,
    },
    {
      property: 'respectTurbosparkIgnore',
...
describe('sandbox image resolution precedence', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(os.homedir).mockReturnValue('/mock/home/user');
    vi.stubEnv('GEMINI_API_KEY', 'test-api-key');
    delete process.env['TURBOSPARK_SANDBOX_IMAGE'];
  });
  ...
  it('uses --sandbox-image over env and settings', async () => {
    vi.stubEnv('TURBOSPARK_SANDBOX_IMAGE', 'env-image');
    process.argv = [
      'node',
      'script.js',
      '--sandbox',
  ...
  it('uses TURBOSPARK_SANDBOX_IMAGE over tools.sandboxImage', async () => {
    vi.stubEnv('TURBOSPARK_SANDBOX_IMAGE', 'env-image');
...
describe('loadCliConfig runtimeOutputDir', () => {
  ...
  it('should resolve relative runtimeOutputDir against cwd', async () => {
    const argv = await parseArguments();
    const settings: Settings = {
      advanced: { runtimeOutputDir: '.turbospark' },
    };
  ...
  it('should let TURBOSPARK_RUNTIME_DIR env var take priority over settings', async () => {
