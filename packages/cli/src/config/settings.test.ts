...
import { QWEN_DIR } from '@turbospark/turbospark-core';
...
// must keep going through `getUserSettingsPath()` to pick up `TURBOSPARK_HOME`
...
describe('Settings Loading and Merging', () => {
  ...
  describe('loadSettings', () => {
    ...
    it('should add version field to V2 settings without version and write to disk', () => {
      ...
      expect(writtenContent.model?.name).toBe('turbosparkr');
    ...
    it('should correctly handle partially migrated settings without version field', () => {
      ...
      expect(writtenContent.model).toEqual({ name: 'turbosparkr' });
    ...
    it('should not search dirname(turbosparkDir)/.env when TURBOSPARK_HOME is set (#4466)', () => {
      const customHome = '/custom/turbospark/home';
      process.env['TURBOSPARK_HOME'] = customHome;
    ...
    it('should resolve ${VAR} from ~/.env when TURBOSPARK_HOME is not set (#4466)', () => {
    ...
    it('should prefer ~/.turbospark/.env over ~/.env for the same key (first-write-wins) (#4466)', () => {
      const turbosparkEnvPath = path.join(path.dirname(USER_SETTINGS_PATH), '.env');
      const homeEnvPath = path.join(
        path.dirname(path.dirname(USER_SETTINGS_PATH)),
        '.env',
      );
      ...
      (mockFsExistsSync as Mock).mockImplementation(
        (p: fs.PathLike) =>
          p === USER_SETTINGS_PATH || p === turbosparkEnvPath || p === homeEnvPath,
      );
      (fs.readFileSync as Mock).mockImplementation(
        (p: fs.PathOrFileDescriptor) => {
          if (p === USER_SETTINGS_PATH)
            return JSON.stringify(userSettingsContent);
          if (p === turbosparkEnvPath) return 'PRECEDENCE_TOKEN=from_qwen_dir';
    ...
    it('should succeed with unresolved placeholder when .env read throws (#4466)', () => {
      ...
      (mockFsExistsSync as Mock).mockImplementation(
        (p: fs.PathLike) => p === USER_SETTINGS_PATH || p === turbosparkEnvPath,
      );
      (fs.readFileSync as Mock).mockImplementation(
        (p: fs.PathOrFileDescriptor) => {
          if (p === USER_SETTINGS_PATH)
            return JSON.stringify(userSettingsContent);
          if (p === turbosparkEnvPath) throw new Error('EACCES: permission denied');
          return '{}';
        },
      );
    ...
    describe('when TURBOSPARK_SYSTEM_SETTINGS_PATH is set', () => {
      const MOCK_ENV_SYSTEM_SETTINGS_PATH = '/mock/env/system/settings.json';

      beforeEach(() => {
        process.env['TURBOSPARK_SYSTEM_SETTINGS_PATH'] =
          MOCK_ENV_SYSTEM_SETTINGS_PATH;
      });

      afterEach(() => {
        delete process.env['TURBOSPARK_SYSTEM_SETTINGS_PATH'];
      });
  ...
  describe('loadEnvironment', () => {
    ...
    describe('settings.env field', () => {
      ...
      it('should never set TURBOSPARK_HOME or TURBOSPARK_RUNTIME_DIR from settings.env', () => {
        ...
        const userSettingsContent: Settings = {
          env: {
            TURBOSPARK_HOME: '/redirected/by/settings',
            TURBOSPARK_RUNTIME_DIR: '/redirected/runtime',
            HARMLESS_VAR: 'ok',
          },
        };
        ...
        expect(process.env['TURBOSPARK_HOME']).toBeUndefined();
    ...
    describe('TURBOSPARK_HOME custom directory', () => {
      const originalQwenHome = process.env['TURBOSPARK_HOME'];

      beforeEach(() => {
        delete process.env['DEBUG'];
        delete process.env['DEBUG_MODE'];
        delete process.env['TURBOSPARK_HOME_TEST_VAR'];
      });
      ...
      afterEach(() => {
        if (originalQwenHome === undefined) {
          ...
          process.env['TURBOSPARK_HOME'] = originalQwenHome;
        }
        delete process.env['DEBUG'];
        delete process.env['DEBUG_MODE'];
        delete process.env['TURBOSPARK_HOME_TEST_VAR'];
      ...
      it('does not exclude DEBUG/DEBUG_MODE from .env in a TURBOSPARK_HOME dir not named .turbospark', () => {
        ...
        (fs.readFileSync as Mock).mockImplementation(
          (p: fs.PathOrFileDescriptor) => {
            ...
            if (p === customGlobalEnvPath)
              return 'DEBUG=true\nDEBUG_MODE=1\nTURBOSPARK_HOME_TEST_VAR=hello';
            return '{}';
          },
        );
        ...
        // because the .env lives inside the user-level TURBOSPARK_HOME directory.
        expect(process.env['DEBUG']).toEqual('true');
        expect(process.env['DEBUG_MODE']).toEqual('1');
        expect(process.env['TURBOSPARK_HOME_TEST_VAR']).toEqual('hello');
      });
      ...
      it('ignores global-state paths set in a project .env', () => {
        ...
        (fs.readFileSync as Mock).mockImplementation(
          (p: fs.PathOrFileDescriptor) => {
            ...
            if (p === projectEnvPath)
              return [
                'TURBOSPARK_HOME=/tmp/hijack',
                'TURBOSPARK_RUNTIME_DIR=/tmp/hijack-runtime',
                'TURBOSPARK_MCP_APPROVALS_PATH=/tmp/preapproved.json',
                'OTHER_VAR=ok',
              ].join('\n');
            return '{}';
          },
        ...
        expect(process.env['TURBOSPARK_HOME']).toBeUndefined();
        expect(process.env['TURBOSPARK_RUNTIME_DIR']).toBeUndefined();
        expect(process.env['TURBOSPARK_MCP_APPROVALS_PATH']).toBeUndefined();
      ...
      it('still honors TURBOSPARK_HOME from a user-level .env (~/.turbospark/.env)', () => {
        delete process.env['TURBOSPARK_HOME'];
        ...
        (fs.readFileSync as Mock).mockImplementation(
          (p: fs.PathOrFileDescriptor) => {
            if (p === USER_SETTINGS_PATH) return JSON.stringify({});
            if (p === userQwenEnvPath) return 'TURBOSPARK_HOME=/tmp/from-user-env';
            return '{}';
          },
        );
        ...
        expect(process.env['TURBOSPARK_HOME']).toEqual('/tmp/from-user-env');
        cwdSpy.mockRestore();
      });

      it('does not exclude DEBUG/DEBUG_MODE from a workspace .turbospark/.env', () => {
        const cwdSpy = vi
          .spyOn(process, 'cwd')
          .mockReturnValue(MOCK_WORKSPACE_DIR);
        ...
        // Per docs, `.turbospark/.env` files are never filtered by excludedEnvVars,
      ...
      it('still blocks TURBOSPARK_HOME from a workspace .turbospark/.env', () => {
        delete process.env['TURBOSPARK_HOME'];
        delete process.env['TURBOSPARK_RUNTIME_DIR'];
        ...
        // A workspace `.turbospark/.env` is exempt from `excludedEnvVars` but must
      ...
      it('redirects user settings path when TURBOSPARK_HOME is set in ~/.turbospark/.env', () => {
        delete process.env['TURBOSPARK_HOME'];
        ...
        // The pre-pass propagates TURBOSPARK_HOME from ~/.turbospark/.env into
      ...
      it('warns when TURBOSPARK_HOME redirects but the legacy ~/.turbospark still has settings', () => {
        ...
        // Active TURBOSPARK_HOME has nothing yet; legacy ~/.turbospark has settings.json.
        const customSettingsPath = path.join(customHome, 'settings.json');
        ...
        (mockFsExistsSync as Mock).mockImplementation((p: fs.PathLike) => {
          const s = p.toString();
          // Legacy settings exist, active TURBOSPARK_HOME settings do not.
        ...
        const warningMatch = loaded.migrationWarnings.find((w) =>
          w.includes('TURBOSPARK_HOME points to'),
        );
      ...
      it('does not warn when TURBOSPARK_HOME points to a directory with settings.json', () => {
      ...
      it('does not warn when TURBOSPARK_HOME is unset (default ~/.turbospark)', () => {
        delete process.env['TURBOSPARK_HOME'];
        const legacySettings = path.join(
          '/mock/home/user',
          QWEN_DIR,
      ...
      it('prefers TURBOSPARK_HOME/.env over ~/.env at the home-dir step', () => {
        const customHome = '/tmp/qwen-home-custom';
        process.env['TURBOSPARK_HOME'] = customHome;
        const customGlobalEnvPath = path.join(customHome, '.env');
        ...
        (fs.readFileSync as Mock).mockImplementation(
          (p: fs.PathOrFileDescriptor) => {
            if (p === USER_SETTINGS_PATH) return JSON.stringify({});
            if (p === customGlobalEnvPath)
              return 'TURBOSPARK_HOME_TEST_VAR=fromQwenHome';
            if (p === homeEnvPath) return 'TURBOSPARK_HOME_TEST_VAR=fromHomeDir';
            return '{}';
          },
        );
        ...
        // TURBOSPARK_HOME/.env must win — without the precedence fix, ~/.env would
        // be returned by the walk-up before the TURBOSPARK_HOME fallback was ever
        // consulted.
        expect(process.env['TURBOSPARK_HOME_TEST_VAR']).toEqual('fromQwenHome');
      });

      it('falls back to legacy ~/.turbospark/.env for non-routing keys when <TURBOSPARK_HOME>/.env is absent', () => {
        // User keeps OPENAI_API_KEY in ~/.turbospark/.env and adds TURBOSPARK_HOME to the
        ...
        // Only the legacy ~/.turbospark/.env exists; <TURBOSPARK_HOME>/.env, the active
        // settings.json under <TURBOSPARK_HOME>, and ~/.env all do not.
        (mockFsExistsSync as Mock).mockImplementation((p: fs.PathLike) =>
        ...
        expect(process.env['TURBOSPARK_HOME']).toEqual(customHome);
