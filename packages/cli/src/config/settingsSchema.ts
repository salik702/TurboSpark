...
const SETTINGS_SCHEMA = {
  ...
  plansDirectory: {
    ...
    description:
      'Custom directory for approved Plan Mode files. Relative paths are resolved from the project root, and the resolved path must stay within the project root. Defaults to ~/.turbospark/plans.',
    showInDialog: false,
  },
  ...
  general: {
    ...
    properties: {
      ...
      cleanupPeriodDays: {
        ...
        default: 30,
        description:
          'Number of days to retain ~/.turbospark/file-history/ session backups used by /rewind. Backups older than this are removed by a background housekeeping pass that runs at most once per day. Set to 0 for minimum retention (~1 hour) — protects sessions touched in the last hour, plus the currently active session. Other persistent caches will honor the same setting in the future.',
        showInDialog: true,
      },
      gitCoAuthor: {
        type: 'object',
        ...
        description:
          'Attribution added to git commits and pull requests created through TURBO SPARK.',
        showInDialog: false,
        ...
        properties: {
          commit: {
            ...
            default: true,
            description:
              'Add a Co-authored-by trailer to git commit messages AND attach a per-file AI-attribution git note (`refs/notes/ai-attribution`) for commits made through TURBO SPARK. Disabling skips both.',
            showInDialog: true,
          },
          pr: {
            type: 'boolean',
            ...
            default: true,
            description:
              'Append a TURBO SPARK attribution line to PR descriptions when running `gh pr create`.',
            showInDialog: true,
          },
        },
      },
      ...
      language: {
        ...
        description:
          ...
          'in ~/.turbospark/locales/ (e.g., ~/.turbospark/locales/es.js).',
        showInDialog: true,
      ...
      preventSystemSleep: {
        ...
        default: true,
        description:
          'Prevent the system from sleeping while TURBO SPARK is streaming a model response or executing tools. Idle prompt time and permission prompts do not inhibit sleep.',
        showInDialog: true,
      },
      chatRecording: {
        type: 'boolean',
  ...
  ui: {
    ...
    properties: {
      ...
      showStatusInTitle: {
        ...
        default: true,
        description:
          'Show TURBO SPARK session name and status in the terminal window title',
        showInDialog: true,
      },
      hideTips: {
        type: 'boolean',
    ...
    jsonSchemaOverride: {
      ...
      properties: {
        propagateTraceContext: {
          description:
            "Requires `telemetry.enabled: true`. Inject W3C `traceparent` on outbound `fetch` requests (LLM SDK calls, MCP StreamableHTTP, WebFetch, ...) AND as a `TRACEPARENT` environment variable in shell child processes (Bash tool, hooks, monitor). When enabled, any existing `TRACEPARENT` in the parent environment is overwritten with turbospark's own trace context. Default: false — trace context stays internal to the operator's OTLP collector. Set true when you want cross-process trace stitching with an OTel-aware LLM provider (e.g. ARMS+DashScope) or need shell scripts / CLI tools to participate in distributed tracing.",
          type: 'boolean',
          default: false,
        },
      },
  ...
  model: {
    ...
    properties: {
      ...
      skipWorkflowUsageWarning: {
        ...
        default: false,
        description:
          'Suppress the one-time Workflow tool usage banner that describes the TURBOSPARK_MAX_TOKENS_PER_WORKFLOW env knob. The banner fires at most once per session regardless of this setting.',
        showInDialog: false,
      },
      skipLoopDetection: {
        type: 'boolean',
      ...
      generationConfig: {
        ...
        properties: {
          ...
          splitToolMedia: {
            ...
            default: true,
            description:
              'When true, media (images / audio / video / files) returned by tool calls — including the built-in read_file and MCP tools — is split into a follow-up user message instead of being embedded in the `role: "tool"` message. The OpenAI Chat Completions spec only permits text on tool messages, so strict OpenAI-compatible servers (e.g., doubao / new-api / LM Studio) silently drop or reject embedded media and the model never sees an image read via read_file (turbospark/turbospark#4876, #3616). Default true is spec-compliant and safe for permissive providers; set false only to restore the legacy embed-in-tool-message behavior.',
            parentKey: 'generationConfig',
            showInDialog: false,
          },
          toolResultContentFormat: {
            ...
            default: 'parts',
            description:
              'Controls how text-only tool results are serialized in OpenAI-compatible requests. Use "parts" for the default content-part array shape. Use "string" only for legacy OpenAI-compatible runtimes whose tool templates ignore text content parts (for example older GLM-5.1 vLLM/SGLang templates; turbospark/turbospark#3361). Tool-returned media is still handled by splitToolMedia.',
            parentKey: 'generationConfig',
            showInDialog: false,
            options: [
          ...
          contextWindowSize: {
            ...
            default: undefined,
            description:
              "Overrides the default context window size for the selected model. Use this setting when a provider's effective context limit differs from TURBO SPARK's default. This value defines the model's assumed maximum context capacity, not a per-request token limit.",
            parentKey: 'generationConfig',
            showInDialog: false,
          },
        },
    ...
    properties: {
      ...
      fileFiltering: {
        ...
        properties: {
          ...
          respectTurbosparkIgnore: {
            type: 'boolean',
            label: 'Respect .turbosparkignore',
            category: 'Context',
            requiresRestart: true,
            default: true,
            description: 'Respect .turbosparkignore files when searching',
            showInDialog: true,
          },
  ...
  tools: {
    ...
    properties: {
      ...
      sandboxImage: {
        ...
        default: undefined as string | undefined,
        description:
          'Sandbox image URI used by Docker/Podman when --sandbox-image and TURBOSPARK_SANDBOX_IMAGE are not set.',
        showInDialog: false,
      },
      toolSearch: {
        type: 'object',
      ...
      computerUse: {
        ...
        default: {},
        description:
          "Cross-platform desktop automation via the cua-driver native driver (trycua/cua). On first invocation a pinned, signed + notarized binary (~20MB) is downloaded into ~/.turbospark/computer-use/ and the user is walked through macOS Accessibility / Screen Recording permissions if needed. Exposes cua-driver's full tool surface (click, type_text, scroll, drag, press_key, get_window_state, page, launch_app, and more).",
        showInDialog: false,
        properties: {
          enabled: {
            type: 'boolean',
  ...
  policy: {
    ...
    description:
      ...
      'between concurrent HTTP clients sharing one `turbospark serve` daemon.',
    showInDialog: false,
    properties: {
      permissionStrategy: {
        type: 'enum',
      ...
      consensusQuorum: {
        ...
        description:
          ...
          'Requires daemon restart — read once at boot.',
        showInDialog: false,
        // runTurbosparkServe.ts validates `Number.isInteger(n) && n >= 1` and
    ...
    properties: {
      ...
      runtimeOutputDir: {
        ...
        description:
          ...
          'Config files remain at ~/.turbospark (or TURBOSPARK_HOME if set). Env var TURBOSPARK_RUNTIME_DIR takes priority.',
        showInDialog: false,
      },
    },
  },
      ...
      arena: {
        ...
        properties: {
          worktreeBaseDir: {
            ...
            default: undefined as string | undefined,
            description:
              'Custom base directory for Arena worktrees. Defaults to ~/.turbospark/arena.',
            showInDialog: false,
          },
          preserveArtifacts: {
            type: 'boolean',
  ...
  stopHookBlockingCap: {
    ...
    description:
      'Maximum consecutive blocking Stop/SubagentStop hook decisions before TURBO SPARK overrides the hook loop and ends the turn. Can be overridden by TURBOSPARK_STOP_HOOK_BLOCK_CAP.',
  ...
  experimental: {
    ...
    properties: {
      cron: {
        ...
        default: true,
        description:
          'Enable in-session cron/loop tools. When enabled, the model can create recurring prompts using cron_create, cron_list, and cron_delete tools. Can be disabled via TURBOSPARK_DISABLE_CRON=1 environment variable.',
        showInDialog: true,
      },
      agentTeam: {
        type: 'boolean',
        ...
        default: false,
        description:
          'Enable agent team collaboration tools (experimental). When enabled, the model can create agent teams and coordinate work using team_create, team_delete, send_message, task_create, task_update, and task_list tools. Can also be enabled via TURBOSPARK_ENABLE_AGENT_TEAM=1 environment variable.',
        showInDialog: true,
      },
    ...
    properties: {
      symlinkDirectories: {
        ...
        description:
          ...
          '`.turbospark` (the CLI-managed metadata tree, which contains ' +
