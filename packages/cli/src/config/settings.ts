...
// Lazy getters: must NOT be top-level consts. `TURBOSPARK_HOME` may be resolved
// from `~/.env` or `~/.turbospark/.env` by `preResolveHomeEnvOverrides()` in
...
export const ENV_CORRUPTED_PATH = 'TURBOSPARK_SETTINGS_CORRUPTED_PATH';
export const ENV_WAS_RECOVERED = 'TURBOSPARK_SETTINGS_WAS_RECOVERED';

// TURBOSPARK_HOME and TURBOSPARK_RUNTIME_DIR control where global state (settings, OAuth
...
const PROJECT_ENV_HARDCODED_EXCLUSIONS = [
  'TURBOSPARK_HOME',
  'TURBOSPARK_RUNTIME_DIR',
  'TURBOSPARK_MCP_APPROVALS_PATH',
  ENV_CORRUPTED_PATH,
  ENV_WAS_RECOVERED,
];
...
export function getSystemSettingsPath(): string {
  if (process.env['TURBOSPARK_SYSTEM_SETTINGS_PATH']) {
    return process.env['TURBOSPARK_SYSTEM_SETTINGS_PATH'];
  }
  if (platform() === 'darwin') {
    return '/Library/Application Support/QwenCode/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\turbospark\\settings.json';
  } else {
    return '/etc/turbospark/settings.json';
  }
}

export function getSystemDefaultsPath(): string {
  if (process.env['TURBOSPARK_SYSTEM_DEFAULTS_PATH']) {
    return process.env['TURBOSPARK_SYSTEM_DEFAULTS_PATH'];
  }
  return path.join(
 ...
 * `.env` (which respects `TURBOSPARK_HOME`). When `TURBOSPARK_HOME` redirects elsewhere,
 * the legacy `<homedir>/.turbospark/.env` is also included so credentials users
...
/**
 * Pre-resolves TURBOSPARK_HOME and TURBOSPARK_RUNTIME_DIR from user-level `.env` files
 * before any settings or storage paths are read. Required because
 * module-load `Storage.getGlobalTurbosparkDir()` would otherwise snapshot legacy
 ...
 * global state between `~/.turbospark/...` and `<TURBOSPARK_HOME>/...`.
 *
...
export function preResolveHomeEnvOverrides(): void {
  ...
  // Storage.getGlobalTurbosparkDir() shares the same homedir resolution as the
  // rest of the storage layer; when TURBOSPARK_HOME is unset it equals
  // `<homedir>/.turbospark`, so path.dirname() recovers `<homedir>`.
  const initialQwenHome = process.env['TURBOSPARK_HOME'];
  const initialQwenDir = Storage.getGlobalTurbosparkDir();
  ...
  if (discoveredQwenHome && discoveredQwenHome !== initialQwenHome) {
    const discoveredDir = Storage.getGlobalTurbosparkDir();
 ...
 *
 * Candidates are iterated most-specific-first (`~/.turbospark/.env` before
...
function getHomeEnvFallbackVars(): Record<string, string> {
  ...
  // When TURBOSPARK_HOME is set, skip ~/.env to avoid surprise cross-contamination
  ...
  if (!process.env['TURBOSPARK_HOME']) {
...
/**
 * Surfaces a one-shot warning when TURBOSPARK_HOME has been redirected but the
...
function detectQwenHomeRedirectWithoutMigration(
  ...
  // Compute the legacy path by briefly unsetting TURBOSPARK_HOME so Storage uses
  ...
  const activeQwenDir = Storage.getGlobalTurbosparkDir();
  const savedQwenHome = process.env['TURBOSPARK_HOME'];
  delete process.env['TURBOSPARK_HOME'];
  let legacyQwenDir: string;
  try {
    legacyQwenDir = Storage.getGlobalTurbosparkDir();
  } finally {
    process.env['TURBOSPARK_HOME'] = savedQwenHome;
  }
  ...
  return (
    `TURBOSPARK_HOME points to "${activeQwenDir}" but no settings.json was found there. ` +
...
function findEnvFile(
  ...
  // ~/.turbospark/.env (only when TURBOSPARK_HOME redirects), then ~/.env.
  // Users who add `TURBOSPARK_HOME=` to an existing global env file shouldn't lose
  ...
  while (true) {
    if (currentDir === homeDir) {
      ...
      // Workspace step: prefer .turbospark/.env, then plain .env.
...
export function loadEnvironment(settings: Settings): void {
  ...
  if (envFilePath) {
    try {
      ...
      // homeScoped: `.env` lives under the user's home TURBOSPARK dir or `~/.env` —
      //   only these may set TURBOSPARK_HOME / TURBOSPARK_RUNTIME_DIR.
      // qwenScoped: any `.env` whose immediate parent is `.turbospark` (including
      //   `<repo>/.turbospark/.env`) — exempt from the user `excludedEnvVars` list.
 ...
 * System Defaults → User (~/.turbospark/settings.json) → Workspace → System.
 */
...
export function loadSettings(
  ...
  const opts: LoadSettingsOptions =
    typeof consumeCorruptionEnvVars === 'object'
      ...
      : { consumeCorruptionEnvVars };
  // Apply any TURBOSPARK_HOME / TURBOSPARK_RUNTIME_DIR set in user-level `.env` files
  // BEFORE any code reads a path derived from them. After this call, the
  // lazy `getUserSettingsPath()` / `Storage.getGlobalTurbosparkDir()` getters
