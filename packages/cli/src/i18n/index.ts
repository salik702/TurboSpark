...
import { Storage, resolveBundleDir } from '@turbospark/turbospark-core';
import {
...
const getUserLocalesDir = (): string =>
  path.join(Storage.getGlobalTurbosparkDir(), 'locales');
 ...
 * @returns The path to ~/.turbospark/locales
 */
...
export function detectSystemLanguage(): SupportedLanguage {
  const envLang = process.env['TURBOSPARK_LANG'] || process.env['LANG'];
  if (envLang) {
...
export function resolveLanguageSetting(
  ...
  return (
    process.env['TURBOSPARK_LANG'] || settingsLanguage || 'auto'
