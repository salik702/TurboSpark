...
// Priority: TURBOSPARK_RUNTIME_DIR > TURBOSPARK_HOME > ~/.turbospark
function getRuntimeBaseDir() {
  const runtimeDir = process.env.TURBOSPARK_RUNTIME_DIR;
  if (runtimeDir) {
    return resolvePath(runtimeDir);
  }
  const homeEnv = process.env.TURBOSPARK_HOME;
  if (homeEnv) {
    return resolvePath(homeEnv);
  }
  return path.join(os.homedir(), '.turbospark');
}

// Project-level .turbospark directory in the workspace
const WORKSPACE_TURBOSPARK_DIR = path.join(projectRoot, '.turbospark');
...
// Workspace settings remain in the project's .turbospark directory
export const WORKSPACE_SETTINGS_FILE = path.join(
  WORKSPACE_TURBOSPARK_DIR,
  'settings.json',
);
...
export function getJson(url) {
  const tmpFile = path.join(
    os.tmpdir(),
    `turbospark-releases-${Date.now()}.json`,
  );
  try {
    const result = spawnSync(
      'curl',
      ['-sL', '-H', 'User-Agent: turbospark-dev-script', '-o', tmpFile, url],
...
export async function ensureBinary(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'turbospark-telemetry-'));
