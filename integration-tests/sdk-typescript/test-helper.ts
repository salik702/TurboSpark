...
export interface SDKTestHelperOptions {
  /**
   * Optional settings for .turbospark/settings.json
   */
  settings?: Record<string, unknown>;
  /**
   * Whether to create .turbospark/settings.json
   */
  createTurbosparkConfig?: boolean;
  /**
...
export class SDKTestHelper {
  ...
  async setup(
    ...
    // Optionally create .turbospark/settings.json for CLI configuration
    if (options.createTurbosparkConfig !== false) {
      const turbosparkDir = join(this.testDir, '.turbospark');
      await mkdir(turbosparkDir, { recursive: true });
...
export function createSharedTestOptions(
  ...
  return {
    pathToTurbosparkExecutable: TEST_CLI_PATH,
    ...overrides,
  };
}
