    ...
    it('should handle greeting and self-description', async () => {
      ...
      try {
        ...
        expect(assistantText.toLowerCase()).toMatch(
          /hello|hi|greetings|turbospark|assistant/,
        );
  ...
  describe('System Initialization', () => {
    it('should receive system message with initialization info', async () => {
      ...
      try {
        ...
        expect(systemMessage!.turbospark_code_version).toBeDefined();
    ...
    it('should throw if CLI not found', async () => {
      try {
        const q = query({
          prompt: 'Hello',
          options: {
            pathToTurbosparkExecutable: '/nonexistent/path/to/cli',
            debug: false,
          },
        });
        ...
        expect((error as Error).message).toContain(
          'Invalid pathToTurbosparkExecutable',
        );
      }
    });
  });
