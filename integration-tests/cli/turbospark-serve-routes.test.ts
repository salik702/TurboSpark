...
/**
 * `turbospark serve` daemon — HTTP route + middleware integration tests.
 *
 ...
 * in `turbospark-serve-streaming.test.ts` and skip when no auth is set.
 */
...
describe('turbospark serve — bearer auth (timing-safe compare)', () => {
  ...
  it('/health exempt: missing Authorization header → 200', async () => {
    ...
    // docs/developers/turbospark-serve-protocol.md so a future middleware
...
describe('turbospark serve — CORS browser-origin denial', () => {
...
describe('turbospark serve — capabilities envelope', () => {
...
describe('turbospark serve — POST /session validation + concurrent coalescing', () => {
  ...
  it('omits cwd → falls back to bound workspace (#3803 §02)', async () => {
    ...
    // daemon process verifies the runTurbosparkServe → createServeApp →
...
describe('turbospark serve — POST /permission/:requestId validation', () => {
...
describe('turbospark serve — SSE Content-Type guard (SDK side)', () => {
...
describe('turbospark serve — Last-Event-ID strict parsing', () => {
...
describe('turbospark serve — cancel + list', () => {
...
describe('turbospark serve — DELETE /session/:id', () => {
...
describe('turbospark serve — PATCH /session/:id/metadata', () => {
