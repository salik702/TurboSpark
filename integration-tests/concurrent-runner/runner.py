...
class RunConfig:
    ...
    worktree_base: Path = field(default_factory=lambda: Path.home() / ".turbospark" / "worktrees")
...
class GitWorktreeManager:
    ...
    async def collect_session_log(self, worktree_dir: Path, output_dir: Path) -> Optional[Tuple[Path, str, Path]]:
        ...
        ~/.turbospark/projects/{projectId}/chats/{sessionId}.jsonl
...
def load_config(config_path: Path) -> RunConfig:
    ...
    return RunConfig(
        ...
        worktree_base=Path(data.get("worktree_base", "~/.turbospark/worktrees")).expanduser(),
