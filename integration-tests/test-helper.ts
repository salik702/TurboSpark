...
export class TestRig {
  ...
  getDefaultTimeout() {
    ...
    if (env['TURBOSPARK_SANDBOX']) return 30000; // 30s in containers
  ...
  setup(
    ...
    const turbosparkDir = join(this.testDir, '.turbospark');
    mkdirSync(turbosparkDir, { recursive: true });
    ...
    const settings = {
      ...
      sandbox: env.TURBOSPARK_SANDBOX !== 'false' ? env.TURBOSPARK_SANDBOX : false,
      ...options.settings, // Allow tests to override/add settings
    };
    writeFileSync(
      join(turbosparkDir, 'settings.json'),
  ...
  /**
   * The command and args to use to invoke TURBO SPARK CLI. Allows us to switch
   * between using the bundled gemini.js (the default) and using the installed
   * 'turbospark' (used to verify npm bundles).
   */
  private _getCommandAndArgs(extraInitialArgs: string[] = []): {
    ...
    const command = isNpmReleaseTest ? 'turbospark' : 'node';
    ...
    const promise = new Promise<string>((resolve, reject) => {
      child.on('close', (code: number) => {
        if (code === 0) {
          ...
          let result = stdout;
          if (env['TURBOSPARK_SANDBOX'] === 'podman') {
    ...
    return this.poll(
      () => {
        ...
        return logs.some(
          (logData) =>
            logData.attributes &&
            logData.attributes['event.name'] === `turbospark.${eventName}`,
        );
      },
      timeout,
      100,
          ...
          if (braceDepth === 0) {
            ...
            try {
              ...
              if (
                ...
                obj.attributes &&
                obj.attributes['event.name'] === 'turbospark.tool_call'
              ) {
                logs.push({
  ...
  readToolLogs() {
    ...
    for (const logData of parsedLogs) {
      ...
      if (
        logData.attributes &&
        logData.attributes['event.name'] === 'turbospark.tool_call'
      ) {
  ...
  readLastApiRequest(): Record<string, unknown> | null {
    ...
    const apiRequests = logs.filter(
      (logData) =>
        logData.attributes &&
        logData.attributes['event.name'] === 'turbospark.api_request',
    );
      ...
      if (logData.scopeMetrics) {
        for (const scopeMetric of logData.scopeMetrics) {
          for (const metric of scopeMetric.metrics) {
            if (metric.descriptor.name === `turbospark.${metricName}`) {
              return metric;
            }
          }
        }
