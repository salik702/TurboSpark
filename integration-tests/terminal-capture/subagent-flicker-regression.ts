 ...
 *
 * Useful env:
 *   TURBOSPARK_TUI_E2E_REPO=/path/to/turbospark
 *   TURBOSPARK_TUI_E2E_OUT=/tmp/turbospark-tui-subagent-flicker
 *   TURBOSPARK_TUI_E2E_MAX_CLEAR_PAIRS=10       (default: 10)
 *   TURBOSPARK_TUI_E2E_MAX_CLEAR_SCREEN=20      (default: 20)
 *   TURBOSPARK_TUI_E2E_MAX_ERASE_LINE=460       (default: 460 — separates fix from
 *                                          no-fix; reverting the fix raises
 *                                          this counter to ~469)
 *   TURBOSPARK_TUI_E2E_SUBAGENT_TOOL_CALLS=5
 */
...
function chatCompletionId(suffix: string): string {
  return `chatcmpl-turbospark-tui-subagent-${suffix}-${Date.now()}`;
}
...
function buildMainAgentToolCall(packageJsonPath: string) {
  return {
    id: 'chatcmpl-turbospark-tui-subagent-dispatch',
...
async function startFakeOpenAIServer(
  ...
  const verbose = process.env['TURBOSPARK_TUI_E2E_VERBOSE'] === '1';
...
async function main(): Promise<void> {
  ...
  const defaultOut = join(
    tmpdir(),
    'turbospark-tui-subagent-flicker',
    basename(repoRoot),
  );
  const outputDir = resolve(process.env['TURBOSPARK_TUI_E2E_OUT'] ?? defaultOut);
  const maxClearPairs = envNumber('TURBOSPARK_TUI_E2E_MAX_CLEAR_PAIRS', 10);
  const maxClearScreen = envNumber('TURBOSPARK_TUI_E2E_MAX_CLEAR_SCREEN', 20);
  ...
  const maxEraseLine = envNumber('TURBOSPARK_TUI_E2E_MAX_ERASE_LINE', 460);
  const subagentToolCalls = envNumber('TURBOSPARK_TUI_E2E_SUBAGENT_TOOL_CALLS', 5);
  ...
  // Sandbox HOME to keep ~/.turbospark settings out of the run.
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    // Intentionally NOT setting TURBOSPARK_SIMPLE so the agent tool stays in
    // the registry — see comment in turbosparkArgs() above.
