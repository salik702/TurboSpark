...
This file provides guidance to TURBO SPARK when working with code in this
repository.
...
cd integration-tests && \
  cross-env TURBOSPARK_SANDBOX=false npx vitest run cli interactive
```
...
1. **Design doc for non-trivial work** — write one in `.turbospark/design/` if the
...
2. **Test plan for behavioral changes** — write an E2E test plan in
   `.turbospark/e2e-tests/` when the change affects user-observable behavior. Dry-run
   against the global `turbospark` CLI first to confirm the baseline.
...
Project artifacts live under `.turbospark/`:

| Directory               | Purpose                              |
| ----------------------- | ------------------------------------ |
| `.turbospark/design/`         | Design docs for planned features     |
| `.turbospark/e2e-tests/`      | E2E test plans and results           |
| `.turbospark/issues/`         | Issue drafts before filing on GitHub |
| `.turbospark/pr-drafts/`      | PR drafts before submitting          |
| `.turbospark/pr-reviews/`     | PR review notes                      |
| `.turbospark/investigations/` | Structured debugging journals        |
| `.turbospark/scripts/`        | Utility scripts                      |
