...
Per [#3803](https://github.com/turbospark/turbospark/issues/3803) §02 each daemon binds to one workspace at boot (the current `cwd`, or override with `--workspace /path/to/dir`). The daemon's bound path is advertised on `/capabilities.workspaceCwd` so clients can pre-flight check + omit `cwd` from `POST /session`.
...
const client = new DaemonClient({
  ...
  // back to `process.env.TURBOSPARK_SERVER_TOKEN` automatically — same env
...
Two clients pointed at the **same daemon** end up on the same session. Per #3803 §02 each daemon is bound to ONE workspace at boot, so the daemon launched as `turbospark serve --workspace /work/repo` (or `cd /work/repo && turbospark serve`) is what both clients connect to:
...
**SDK env fallback (PR 27, v0.16-alpha)** — `DaemonClient` reads `TURBOSPARK_SERVER_TOKEN` from the environment automatically when `token` is omitted, mirroring the daemon's own `--token` CLI fallback. So if your shell has `export TURBOSPARK_SERVER_TOKEN=...`, this is equivalent to the above:
...
- [HTTP protocol reference](../turbospark-serve-protocol.md) — full route spec with status codes
- [Daemon mode user guide](../../users/turbospark-serve.md) — operator-side docs
