# `turbospark serve` HTTP protocol reference

Stage 1 of the [turbospark daemon design](https://github.com/turbospark/turbospark/issues/3803). All routes live under the daemon's base URL (default `http://127.0.0.1:4170`).

## Authentication

When the daemon was started with `--token` or `TURBOSPARK_SERVER_TOKEN`, **every route except `/health` on loopback binds** must carry:
...
`mcp_guardrail_events` (issue [#4175](https://github.com/turbospark/turbospark/issues/4175) PR 14b) advertises the typed SSE push events that surface MCP budget state crossings without a poll loop. Two frame types arrive on `GET /session/:id/events`:
...
**MCP client guardrails (issue [#4175](https://github.com/turbospark/turbospark/issues/4175) PR 14).** Post-PR-14 daemons extend the payload with four additive fields and one workspace-level cell:
...
```json
{
  ...
  "workspaceCwd": "/canonical/path",
  "initialized": true,
  "current": { "authType": "turbospark", "modelId": "qwen3(qwen)" },
  "providers": [
    {
      "kind": "model_provider",
      "status": "ok",
...
`DASHSCOPE_API_KEY`, `OPENROUTER_API_KEY`, and `TURBOSPARK_SERVER_TOKEN`. Other
...
Issue [#4175](https://github.com/turbospark/turbospark/issues/4175) Wave 4 PR 17 adds four mutation control routes that let remote clients change runtime posture without touching the daemon host's CLI. All four:

- Are gated by the **strict** mutation gate from PR 15. A daemon configured without a bearer token rejects them with `401 {code: 'token_required'}`. Configure `--token` (or `TURBOSPARK_SERVER_TOKEN`) before opting in.
...
Scaffold an empty `TURBOSPARK.md` (or whatever `getCurrentGeminiMdFilename()` returns under `--memory-file-name` overrides) at the daemon's bound workspace root. Mechanical only — for AI-driven content fill, follow up with `POST /session/:id/prompt`.
...
```json
{ "path": "/work/bound/TURBOSPARK.md", "action": "created" }
```
...
- Send `Last-Event-ID: <n>` to replay events with `id > n` from the per-session ring (default depth **8000**, tunable via `turbospark serve --event-ring-size <n>`)
...
Capability tag: `auth_device_flow` (always advertised). Supported providers in v1: `turbospark-oauth`.

**Runtime locality.** The daemon never spawns a browser — even if it can. The client decides whether to call `open(verificationUri)` locally; on a headless pod (the canonical Mode B deployment) the user opens the URL on whatever device they have a browser on. See `docs/users/turbospark-serve.md` for the recommended UX.
...
```json
{
  ...
  "verificationUriComplete": "https://chat.turbospark.ai/api/v1/oauth2/device?user_code=USER-1",
...
| Var                 | Purpose                                                                                                                                                             |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `TURBOSPARK_SERVER_TOKEN` | Bearer token. Stripped of leading/trailing whitespace at boot.                                                                                                      |
| `SKIP_LLM_TESTS`    | Set to `1` to **skip** LLM-required integration tests in `integration-tests/cli/turbospark-serve-streaming.test.ts` (default-on for CI envs that lack provider API keys). |
...
| `packages/cli/src/serve/runTurbosparkServe.ts`             | listener lifecycle + signal handling                                                                       |
...
| `integration-tests/cli/turbospark-serve-routes.test.ts`    | 18 cases, no LLM                                                                                           |
| `integration-tests/cli/turbospark-serve-streaming.test.ts` | 3 cases, real `qwen --acp` child (skipped when `SKIP_LLM_TESTS=1`)                                         |
