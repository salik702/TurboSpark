...
This document describes the `todo_write` tool for TURBO SPARK.
...
## How to use `todo_write` with TURBO SPARK
...
The tool stores todo lists in your home directory (`~/.turbospark/todos/`) with session-specific files, so each coding session maintains its own task list.
