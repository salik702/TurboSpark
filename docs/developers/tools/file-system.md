# TURBO SPARK file system tools

TURBO SPARK provides a comprehensive suite of tools for interacting with the local file system. These tools allow the model to read from, write to, list, search, and modify files and directories, all under your control and typically with confirmation for sensitive operations.
...
- **Behavior:**
  ...
  - Respects .gitignore and .turbosparkignore patterns by default.
...
- **Behavior:**
  ...
  - Respects .gitignore and .turbosparkignore patterns.
...
When reading files, TURBO SPARK detects the file's encoding using a multi-step strategy:
...
Set it in `.turbospark/settings.json` or `~/.turbospark/settings.json`:
...
These file system tools provide a foundation for TURBO SPARK to understand and interact with your local project context.
