...
`errorKind` (closed enum — see `KNOWN_DEVICE_FLOW_ERROR_KINDS` exported from `@turbospark/sdk/daemon` for the canonical list, currently: `expired_token` / `access_denied` / `invalid_grant` / `upstream_error` / `persist_failed` / `not_found_or_evicted`).
...
- [PR #4328](https://github.com/turbospark/turbospark/pull/4328) — base PR with the shared UI transcript layer
- [PR #4353](https://github.com/turbospark/turbospark/pull/4353) — this PR (unified completeness follow-up)
- [Issue #3803](https://github.com/turbospark/turbospark/issues/3803) — daemon mode proposal
- [Issue #4175](https://github.com/turbospark/turbospark/issues/4175) — Mode B v0.16 implementation tracker
