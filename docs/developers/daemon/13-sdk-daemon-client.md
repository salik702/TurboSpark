...
| `DaemonClient.ts`        | Low-level HTTP/SSE facade — one method per `turbospark-serve-protocol.md` route.                                                     |
...
sequenceDiagram
    ...
    App->>AF: start({providerId: 'turbospark-oauth'})
...
- No transitive dependencies on `@turbospark/turbospark-core` or `@turbospark/acp-bridge` — the SDK package is fully decoupled so external consumers do not pull in the daemon's internals.

## `ui/*` subpackage ([#4328](https://github.com/turbospark/turbospark/pull/4328) + [#4353](https://github.com/turbospark/turbospark/pull/4353))
...
The subpackage is exported from the `@turbospark/sdk/daemon` subpath. Existing
