...
- Toggle input comes from `runTurbosparkServe` / `createServeApp`: `{ requireAuth, mcpPoolActive, allowOriginActive, promptDeadlineMs, writerIdleTimeoutMs, persistSettingAvailable, sessionShellCommandEnabled, rateLimit, reloadAvailable }`.
...
- The route catalog lives in [`../turbospark-serve-protocol.md`](../turbospark-serve-protocol.md); this page intentionally does not duplicate it.
