...
This is the developer-facing technical documentation for **turbospark daemon mode**: the `turbospark serve` HTTP daemon, the `@turbospark/acp-bridge` package, the workspace-scoped MCP transport pool, multi-client permission mediation, typed daemon event schema v1, the TypeScript SDK daemon client, and the adapters that connect to the daemon.

It complements, rather than replaces, these existing docs:

| Existing doc                                                                         | Audience              | Source of truth for                                      |
| ------------------------------------------------------------------------------------ | --------------------- | -------------------------------------------------------- |
| [`../../users/turbospark-serve.md`](../../users/turbospark-serve.md)                             | Operators             | User quickstart, flags, threat model                     |
| [`../turbospark-serve-protocol.md`](../turbospark-serve-protocol.md)                             | Protocol implementers | HTTP route catalog, request/response shapes, error codes |
...
If you want to **start a daemon and use it**, read `turbospark-serve.md` first. If you want to **build a client against the wire format**, read `turbospark-serve-protocol.md`. If you want to **understand, extend, or debug the daemon internals**, read this set.
...
- [`02-serve-runtime.md`](./02-serve-runtime.md) - `runTurbosparkServe` bootstrap, Express app, middleware chain, graceful shutdown.
- [`03-acp-bridge.md`](./03-acp-bridge.md) - `@turbospark/acp-bridge` package internals, session multiplexing, channel factory, ACP child spawn.
...
- **acp-bridge** - the `@turbospark/acp-bridge` package (`packages/acp-bridge/`). Owns session multiplexing, the permission mediator, the event bus, and the channel factory.
...
- **F1 / F2 / F3 / F4** - internal milestones tracked in [#4175](https://github.com/turbospark/turbospark/issues/4175). F1: bridge extraction and `BridgeFileSystem`. F2: workspace-scoped MCP transport pool. F3: multi-client permission mediation. F4: protocol completion and daemon client surfaces.
...
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| Bootstrap and HTTP assembly         | `packages/cli/src/serve/runTurbosparkServe.ts`, `server.ts`, `/demo`                                                                                                                                                                                            | [`02`](./02-serve-runtime.md), [`20`](./20-quickstart-operations.md)                                                   |
| ACP bridge and session multiplexing | `packages/acp-bridge/src/bridge.ts`, `packages/acp-bridge/src/bridgeTypes.ts`, `@turbospark/acp-bridge`                                                                                                                                                    | [`03`](./03-acp-bridge.md), [`08`](./08-session-lifecycle.md)                                                          |
...
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| Bootstrap / listen path   | `turbospark serve` lazy-loads `runTurbosparkServe`, validates auth/workspace/budget/settings, builds an Express app, then calls `app.listen` and blocks forever until signal.                | [`02`](./02-serve-runtime.md), [`20`](./20-quickstart-operations.md)      |
...
| Area          | Current state                                                                                                                                       | Primary docs                                                                                                  |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| HTTP routes   | The route catalog lives in `turbospark-serve-protocol.md`; this daemon set only references it and explains implementation ownership.                      | [`../turbospark-serve-protocol.md`](../turbospark-serve-protocol.md), [`20`](./20-quickstart-operations.md)               |
...
| Area                    | Current state                                                                                                                                             | Primary docs                          |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- |
| Configuration           | Full `turbospark serve` flags, env vars, `settings.json`, `ServeOptions`, `BridgeOptions`, and important constants are collected in one page.                   | [`17`](./17-configuration.md)         |
...
This doc set reflects the daemon mode surface currently merged into `main`, including the follow-up work from [#4412](https://github.com/turbospark/turbospark/pull/4412). It intentionally describes current behavior instead of earlier F-series planning snapshots.
