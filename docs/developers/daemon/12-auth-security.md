...
`turbospark serve` is a local daemon by default and an exposed surface in the wrong configuration. Its security model is **layered** so that misconfiguration fails closed:
...
Separate OAuth surface for provider authentication (TURBOSPARK OAuth, etc.):
...
- `@turbospark/acp-bridge` — surfaces device-flow events on the per-session SSE bus.

## Configuration

| Source          | Knob                                                                                    | Effect                                                                  |
| --------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| Env             | `TURBOSPARK_SERVER_TOKEN`                                                                     | Bearer token (trimmed).                                                 |
...
- `packages/cli/src/serve/runTurbosparkServe.ts` (refuse rules)
- `packages/cli/src/serve/loopbackBinds.ts`
- `packages/cli/src/serve/auth/deviceFlow.ts`
- `packages/cli/src/serve/auth/turbosparkDeviceFlowProvider.ts`
- User-facing threat model: [`../../users/turbospark-serve.md`](../../users/turbospark-serve.md).
