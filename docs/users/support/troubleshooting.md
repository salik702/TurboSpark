...
- **Error: `TURBOSPARK OAuth free tier was discontinued on 2026-04-15`**
  - **Cause:** TURBOSPARK OAuth is no longer available as of April 15, 2026.
...
- **Error: `Device authorization flow failed: fetch failed`**
  - **Cause:** Node.js could not reach TURBOSPARK OAuth endpoints (often a proxy or SSL/TLS trust issue). When available, TURBO SPARK will also print the underlying error cause (for example: `UNABLE_TO_VERIFY_LEAF_SIGNATURE`). Note: this error is specific to the legacy TURBOSPARK OAuth flow.
  - **Solution:**
    - If you are still using TURBOSPARK OAuth, switch to API Key or Coding Plan via `/auth`.
...
- **Issue: Unable to display UI after authentication failure**
  ...
  - **Solution:** Clear the `security.auth.selectedType` configuration item in your `settings.json` file:
    - Open `~/.turbospark/settings.json` (or `./.turbospark/settings.json` for project-specific settings)
    - Remove the `security.auth.selectedType` field
    - Restart the CLI to allow it to prompt for authentication again

## Frequently asked questions (FAQs)

- **Q: How do I update TURBO SPARK to the latest version?**
  - A: If you installed TURBO SPARK with the standalone installer, rerun the standalone install command. If you installed it globally via `npm`, update it using the command `npm install -g @turbospark/turbospark@latest`. If you compiled it from source, pull the latest changes from the repository, and then rebuild using the command `npm run build`.

- **Q: Where are the TURBO SPARK configuration or settings files stored?**
  - A: The TURBO SPARK configuration is stored in two `settings.json` files:
    1. In your home directory: `~/.turbospark/settings.json`.
    2. In your project's root directory: `./.turbospark/settings.json`.

    Refer to [TURBO SPARK Configuration](../configuration/settings) for more details.
...
- **Error: Command not found (when attempting to run TURBO SPARK with `qwen`).**
  - **Cause:** The CLI is not correctly installed or it is not in your system's `PATH`.
  - **Solution:**
    The update depends on how you installed TURBO SPARK:
    - If you installed `qwen` with the standalone installer, rerun the standalone install command and then open a new terminal.
    - If you installed `qwen` globally, check that your `npm` global binary directory is in your `PATH`. You can update using the command `npm install -g @turbospark/turbospark@latest`.
...
- **Error: "Operation not permitted", "Permission denied", or similar.**
  - **Cause:** When sandboxing is enabled, TURBO SPARK may attempt operations that are restricted by your sandbox configuration, such as writing outside the project directory or system temp directory.
...
- **TURBO SPARK is not running in interactive mode in "CI" environments**
  - **Issue:** TURBO SPARK does not enter interactive mode (no prompt appears) if an environment variable starting with `CI_` (e.g. `CI_TOKEN`) is set. This is because the `is-in-ci` package, used by the underlying UI framework, detects these variables and assumes a non-interactive CI environment.
...
- **DEBUG mode not working from project .env file**
  ...
  - **Solution:** Use a `.turbospark/.env` file instead, or configure the `advanced.excludedEnvVars` setting in your `settings.json` to exclude fewer variables.
...
- **Trackpad scrolling in tmux changes prompt history instead of scrolling the conversation**
  ...
  - **Cause:** tmux can translate wheel gestures into plain arrow-key sequences. Those sequences are indistinguishable from real arrow-key presses by the time turbospark receives them.
...
- Restart the integrated terminal after installing the extension so it inherits:
  - `TURBOSPARK_IDE_WORKSPACE_PATH`
  - `TURBOSPARK_IDE_SERVER_PORT`
- If running in a container, verify `host.docker.internal` resolves. Otherwise, map the host appropriately.
- Reinstall the companion with `/ide install` and use “TURBO SPARK: Run” in the Command Palette to verify it launches.

## Exit Codes

The TURBO SPARK uses specific exit codes to indicate the reason for termination. This is especially useful for scripting and automation.
...
If you encounter an issue that was not covered here in this _Troubleshooting guide_, consider searching the TURBO SPARK [Issue tracker on GitHub](https://github.com/turbospark/turbospark/issues). If you can't find an issue similar to yours, consider creating a new GitHub Issue with a detailed description. Pull requests are also welcome!
