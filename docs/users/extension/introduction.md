# TURBO SPARK Extensions

TURBO SPARK extensions package prompts, MCP servers, subagents, skills and custom commands into a familiar and user-friendly format. With extensions, you can expand the capabilities of TURBO SPARK and share those capabilities with others. They are designed to be easily installable and shareable.

Extensions and plugins from [Gemini CLI Extensions Gallery](https://geminicli.com/extensions/) and [Claude Code Marketplace](https://claudemarketplaces.com/) can be directly installed into TURBO SPARK. This cross-platform compatibility gives you access to a rich ecosystem of extensions and plugins, dramatically expanding TURBO SPARK's capabilities without requiring extension authors to maintain separate versions.
...
Changes made here hot-reload immediately, without restarting TURBO SPARK.
...
TURBO SPARK also supports plugins from the [Claude Code Marketplace](https://claudemarketplaces.com/). Install from a marketplace and choose a plugin:
...
Claude plugins are automatically converted to TURBO SPARK format during installation:

- `claude-plugin.json` is converted to `turbospark-extension.json`
...
This command opens the respective marketplace in your default browser, allowing you to discover new extensions to enhance your TURBO SPARK experience.

> **Cross-Platform Compatibility**: This allows you to leverage the rich extension ecosystems from both Gemini CLI and Claude Code, dramatically expanding the available functionality for TURBO SPARK users.

#### From Gemini CLI Extensions

TURBO SPARK fully supports extensions from the [Gemini CLI Extensions Gallery](https://geminicli.com/extensions/). Simply install them using the git URL:
...
Gemini extensions are automatically converted to TURBO SPARK format during installation:

- `gemini-extension.json` is converted to `turbospark-extension.json`
- TOML command files are automatically migrated to Markdown format
- MCP servers, context files, and settings are preserved

#### From npm Registry

TURBO SPARK supports installing extensions from npm registries using scoped package names. This is ideal for teams with private registries that already have auth, versioning, and publishing infrastructure in place.
...
> **Note:** npm extensions must include a `turbospark-extension.json` file at the package root, following the same format as any other TURBO SPARK extension. See [Extension Releasing](./extension-releasing.md#releasing-through-npm-registry) for packaging details.
...
```
qwen extensions uninstall turbospark-cli-security
```
...
On startup, TURBO SPARK looks for extensions in `<home>/.turbospark/extensions`

Extensions exist as a directory that contains a `turbospark-extension.json` file. For example:

`<home>/.turbospark/extensions/my-extension/turbospark-extension.json`

### `turbospark-extension.json`

The `turbospark-extension.json` file contains the configuration for the extension. The file has the following structure:
...
- `contextFileName`: The name of the file that contains the context for the extension. This will be used to load the context from the extension directory. If this property is not used but a `TURBOSPARK.md` file is present in your extension directory, then that file will be loaded.
...
- **User level** (default): Settings apply across all projects (`~/.turbospark/.env`)
- **Workspace level**: Settings apply only to the current project (`.turbospark/.env`)
...
When TURBO SPARK starts, it loads all the extensions and merges their configurations. If there are any conflicts, the workspace configuration takes precedence.
...
```
.turbospark/extensions/my-extension/
├── turbospark-extension.json
└── skills/
    └── pdf-processor/
        └── SKILL.md
...
TURBO SPARK extensions allow variable substitution in `turbospark-extension.json`. This can be useful if e.g., you need the current directory to run an MCP server using `"cwd": "${extensionPath}${/}run.ts"`.

**Supported variables:**

| variable                   | description                                                                                                                                                   |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `${extensionPath}`         | The fully-qualified path of the extension in the user's filesystem e.g., '/Users/username/.turbospark/extensions/example-extension'. This will not unwrap symlinks. |
