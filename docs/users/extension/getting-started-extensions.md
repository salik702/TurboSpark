# Getting Started with TURBO SPARK Extensions

This guide will walk you through creating your first TURBO SPARK extension. You'll learn how to set up a new extension, add a custom tool via an MCP server, create a custom command, and provide context to the model with a `TURBOSPARK.md` file.

## Prerequisites

Before you start, make sure you have the TURBO SPARK installed and a basic understanding of Node.js and TypeScript.
...
This is the manifest file for your extension. It tells TURBO SPARK how to load and use your extension.
...
- `mcpServers`: This section defines one or more Model Context Protocol (MCP) servers. MCP servers are how you can add new tools for the model to use.
  - `command`, `args`, `cwd`: These fields specify how to start your server. Notice the use of the `${extensionPath}` variable, which TURBO SPARK replaces with the absolute path to your extension's installation directory. This allows your extension to work regardless of where it's installed.
...
Before you can use the extension, you need to compile the TypeScript code and link the extension to your TURBO SPARK installation for local development.
...
2.  **Build the server:**
    ...
    This will compile `example.ts` into `dist/example.js`, which is the file referenced in your `turbospark-extension.json`.

3.  **Link the extension:**

    The `link` command creates a symbolic link from the TURBO SPARK extensions directory to your development directory. This means any changes you make will be reflected immediately without needing to reinstall.

    ```bash
    qwen extensions link .
    ```

Now, restart your TURBO SPARK session. The new `fetch_posts` tool will be available. You can test it by asking: "fetch posts".
...
After saving the file, restart the TURBO SPARK. You can now run `/fs:grep-code "some pattern"` to use your new command.

## Step 5: Add Custom Skills and Subagents (Optional)

Extensions can also provide custom skills and subagents to extend TURBO SPARK's capabilities.
...
After restarting TURBO SPARK, your custom skills will be available via `/skills` and subagents via `/agents manage`.

## Step 6: Add a Custom `TURBOSPARK.md`

You can provide persistent context to the model by adding a `TURBOSPARK.md` file to your extension. This is useful for giving the model instructions on how to behave or information about your extension's tools. Note that you may not always need this for extensions built to expose commands and prompts.

1.  Create a file named `TURBOSPARK.md` in the root of your extension directory:
...
2.  Update your `turbospark-extension.json` to tell the CLI to load this file:
...
Restart the CLI again. The model will now have the context from your `TURBOSPARK.md` file in every session where the extension is active.
...
You've successfully created a TURBO SPARK extension! You learned how to:
...
From here, you can explore more advanced features and build powerful new capabilities into the TURBO SPARK.
