# TURBO SPARK Keyboard Shortcuts

This document lists the available keyboard shortcuts in TURBO SPARK.
...
When `ui.useTerminalBuffer` is on, the terminal forwards mouse events to turbospark so the wheel can drive the in-app viewport. As a side effect, **native click-and-drag text selection is consumed by the program** — hold `Shift` (or `Option` on macOS Terminal / iTerm) while dragging to bypass mouse capture and select text the usual way.

### tmux trackpad scrolling

Inside tmux, some terminals translate trackpad or wheel gestures into plain `Up Arrow` and `Down Arrow` sequences before turbospark sees them. Those bytes are identical to real arrow-key presses, so turbospark cannot tell whether you meant to scroll the viewport or navigate prompt history.
