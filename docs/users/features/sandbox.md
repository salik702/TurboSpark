...
This document explains how to run TURBO SPARK inside a sandbox to reduce risk when tools execute shell commands or modify files.

## Prerequisites

Before using sandboxing, you need to install and set up TURBO SPARK:

```bash
npm install -g @turbospark/turbospark
```
...
By default, TURBO SPARK uses a published sandbox image (configured in the CLI package) and will pull it as needed.

The container sandbox mounts your workspace and your `~/.turbospark` directory into the container so auth and settings persist between runs.
...
export TURBOSPARK_SANDBOX=true   # true auto-picks a provider (see notes below)
turbospark -p "run the test suite"
...
>
> - On **macOS**, `TURBOSPARK_SANDBOX=true` typically selects `sandbox-exec` (Seatbelt) if available.
> - On **Linux/Windows**, `TURBOSPARK_SANDBOX=true` requires `docker` or `podman` to be installed.
> - To force a provider, set `TURBOSPARK_SANDBOX=docker|podman|sandbox-exec`.

## Configuration

### Enable sandboxing (in order of precedence)

1. **Environment variable**: `TURBOSPARK_SANDBOX=true|false|docker|podman|sandbox-exec`
...
> [!important]
>
> If `TURBOSPARK_SANDBOX` is set, it **overrides** the CLI flag and `settings.json`.

### Configure the sandbox image (Docker/Podman)

- **CLI flag**: `--sandbox-image <image>`
- **Environment variable**: `TURBOSPARK_SANDBOX_IMAGE=<image>`
- **Settings file**: `tools.sandboxImage` in your `settings.json` (e.g., `{"tools": {"sandboxImage": "ghcr.io/turbospark/turbospark:0.14.1"}}`)

Priority order (highest to lowest):

1. `--sandbox-image`
2. `TURBOSPARK_SANDBOX_IMAGE`
3. `tools.sandboxImage`
4. Built-in default image from the CLI package (for example `ghcr.io/turbospark/turbospark:<version>`)

`settings.env.TURBOSPARK_SANDBOX_IMAGE` also works as a generic env injection mechanism, but `tools.sandboxImage` is the preferred persistent setting.
...
1. Create a file named `.turbospark/sandbox-macos-<profile_name>.sb` in your project.
...
- Set `TURBOSPARK_SANDBOX_PROXY_COMMAND=<command>`
...
On Linux, TURBO SPARK defaults to enabling UID/GID mapping so the sandbox runs as your user (and reuses the mounted `~/.turbospark`). Override with:
...
- Container sandbox: add them via `.turbospark/sandbox.Dockerfile` or `.turbospark/sandbox.bashrc`.
...
The official TURBO SPARK Docker image is intentionally minimal to keep the image small, secure, and fast to pull. Different users require different language runtimes (Java, Python, Node.js, etc.), and bundling all environments into a single image is not practical. Therefore, Java is **not included by default** in the Docker sandbox.

If your workflow requires Java, you can extend the base image by creating a `.turbospark/sandbox.Dockerfile` in your project:

```dockerfile
FROM ghcr.io/turbospark/turbospark:latest
...
```bash
TURBOSPARK_SANDBOX=docker BUILD_SANDBOX=1 qwen -s
```
...
**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect the CLI due to automatic exclusion. Use `.turbospark/.env` files for TURBO SPARK-specific debug settings.
