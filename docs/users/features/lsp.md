...
TURBO SPARK provides native Language Server Protocol (LSP) support, enabling advanced code intelligence features like go-to-definition, find references, diagnostics, and code actions. This integration allows the AI agent to understand your code more deeply and provide more accurate assistance.

## Overview

LSP support in TURBO SPARK works by connecting to language servers that understand your code. Once you configure servers via `.lsp.json` (or extensions), TURBO SPARK can start them and use them to:
...
LSP is an experimental feature in TURBO SPARK. To enable it, use the `--experimental-lsp` command line flag:

```bash
qwen --experimental-lsp
```

LSP servers are configuration-driven. You must define them in `.lsp.json` (or via extensions) for TURBO SPARK to start them.
...
TURBO SPARK exposes LSP functionality through the unified `lsp` tool. Here are the available operations:
...
1. **Verify `--experimental-lsp` flag**: Make sure you're using the flag when starting TURBO SPARK
...
5. **Check logs**: Start TURBO SPARK with `--debug`, then search for LSP-related entries in the debug log (see Debugging section below)
...
LSP does not have a separate debug flag. Use TURBO SPARK's normal debug mode together with the LSP feature flag:
...
rg "LSP|Native LSP|clangd|connection closed" ~/.turbospark/debug/latest
# Or, without ripgrep:
grep -E "LSP|Native LSP|clangd|connection closed" ~/.turbospark/debug/latest

# If TURBOSPARK_RUNTIME_DIR is configured
rg "LSP|Native LSP|clangd|connection closed" "$TURBOSPARK_RUNTIME_DIR/debug/latest"
```
...
Use the `--experimental-lsp` flag when starting TURBO SPARK:
...
Start TURBO SPARK with LSP and debug mode enabled:
...
```bash
# Default runtime directory
rg "LSP|Native LSP|<server-name>" ~/.turbospark/debug/latest
# Or:
grep -E "LSP|Native LSP|<server-name>" ~/.turbospark/debug/latest

# If TURBOSPARK_RUNTIME_DIR is configured
rg "LSP|Native LSP|<server-name>" "$TURBOSPARK_RUNTIME_DIR/debug/latest"
```

LSP uses TURBO SPARK's normal `--debug` mode; there is no separate LSP debug flag.
