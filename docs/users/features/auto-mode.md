...
When you're in Auto Mode and the agent tries to run a tool, TURBO SPARK
...
1. **acceptEdits fast-path** — Edit / Write whose target path is inside
   ...
   **Exception:** writes to TURBO SPARK's own self-modification surfaces
   (`.turbospark/settings*.json`, `TURBOSPARK.md`, `AGENTS.md`, `QWEN.local.md`,
   configured context filenames, `.turbospark/rules/`, `.turbospark/commands/`,
   `.turbospark/agents/`, `.turbospark/skills/`, `.turbospark/hooks/`, `.mcp.json`) and
...
- `permissions.allow` rules with specific specifiers (e.g.
  ...
  into permission to rewrite TURBO SPARK settings, commands, hooks,
...
When you enter Auto Mode, TURBO SPARK temporarily removes these rules from
  ...
  "permissions": {
    "autoMode": {
      "hints": {
        ...
        "softDeny": [
          "Editing TURBO SPARK settings unless I explicitly ask for the exact change",
...
the main agent. If you've configured TURBO SPARK to use a self-hosted
