...
1. **Worktree setup**: TURBO SPARK creates isolated Git worktrees for each agent at `~/.turbospark/arena/<session-id>/worktrees/<model-name>/`. Each worktree mirrors your current working directory state exactly — including staged changes, unstaged changes, and untracked files.
...
{
  "arena": {
    "worktreeBaseDir": "~/.turbospark/arena",
...
| :------------------------ | :--------------------------------- | :-------------- |
| `arena.worktreeBaseDir`   | Base directory for arena worktrees | `~/.turbospark/arena` |
...
- Ensure you have write access to the worktree base directory (`~/.turbospark/arena/` by default)
...
Agent Arena is one of several planned multi-agent modes in TURBO SPARK. **Agent Team** and **Agent Swarm** are not yet implemented — the table below describes their intended design for reference.
...
- **Manual parallel sessions**: Run multiple TURBO SPARK sessions yourself in separate terminals with [Git worktrees](https://git-scm.com/docs/git-worktree) for full manual control
