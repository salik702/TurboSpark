...
Channels let you interact with a TURBO SPARK agent from messaging platforms like Telegram, WeChat, QQ, or DingTalk, instead of the terminal. You send messages from your phone or desktop chat app, and the agent responds just like it would in the CLI.

## How It Works

When you run `qwen channel start`, TURBO SPARK:
...
2. Add the channel configuration to `~/.turbospark/settings.json`
...
Once approved, the user's ID is saved to `~/.turbospark/channels/<name>-allowlist.json` and all future messages go through normally.
...
- Approved users are stored in `~/.turbospark/channels/<name>-allowlist.json` — treat this file as sensitive
...
The channel service uses a PID file (`~/.turbospark/channels/service.pid`) to track the running instance:
...
- Sessions are persisted to `~/.turbospark/channels/sessions.json` while the service is running
