...
You can extend the channel system with custom platform adapters packaged as [extensions](../../extension/introduction). This lets you connect TURBO SPARK to any messaging platform, webhook, or custom transport.

## How It Works

Channel plugins are loaded at startup from active extensions. When `qwen channel start` runs, it:

1. Scans all enabled extensions for `channels` entries in their `turbospark-extension.json`
...
Add a channel entry to `~/.turbospark/settings.json` using the custom type provided by the extension:
