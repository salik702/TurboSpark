...
> Create, manage, and share Skills to extend TURBO SPARK's capabilities.

This guide shows you how to create, use, and manage Agent Skills in **TURBO SPARK**. Skills are modular capabilities that extend the model's effectiveness through organized folders containing instructions (and optionally scripts/resources).

## Prerequisites

- TURBO SPARK (recent version)
- Basic familiarity with TURBO SPARK ([Quickstart](../quickstart.md))
...
- Extend TURBO SPARK for your workflows
...
Personal Skills are available across all your projects. Store them in `~/.turbospark/skills/`:

```bash
mkdir -p ~/.turbospark/skills/my-skill-name
```
...
Project Skills are shared with your team. Store them in `.turbospark/skills/` within your project:
...
## Instructions
Provide clear, step-by-step guidance for TURBO SPARK.
...
TURBO SPARK currently validates that:
...
TURBO SPARK discovers Skills from:

- Personal Skills: `~/.turbospark/skills/`
- Project Skills: `.turbospark/skills/`
...
To see which extensions provide skills, check the extension's `turbospark-extension.json` file for a `skills` field.

To view available Skills, ask TURBO SPARK directly:
...
ls ~/.turbospark/skills/

# List project Skills (if in a project directory)
ls .turbospark/skills/

# View a specific Skill's content
cat ~/.turbospark/skills/my-skill/SKILL.md
```
...
If TURBO SPARK doesn't use your Skill, check these common issues:
...
- Personal Skills: `~/.turbospark/skills/<skill-name>/SKILL.md`
- Project Skills: `.turbospark/skills/<skill-name>/SKILL.md`

```bash
# Personal
ls ~/.turbospark/skills/my-skill/SKILL.md
...
Run TURBO SPARK with debug mode to see Skill loading errors:
...
1. Add the Skill under `.turbospark/skills/`
2. Commit and push
3. Teammates pull the changes

```bash
git add .turbospark/skills/
...
```bash
# Personal Skill
code ~/.turbospark/skills/my-skill/SKILL.md

# Project Skill
code .turbospark/skills/my-skill/SKILL.md
```

Changes take effect the next time you start TURBO SPARK. If TURBO SPARK is already running, restart it to load the updates.
...
```bash
# Personal
rm -rf ~/.turbospark/skills/my-skill
