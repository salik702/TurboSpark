...
TURBO SPARK hooks provide a powerful mechanism for extending and customizing the behavior of the TURBO SPARK application. Hooks allow users to execute custom scripts or programs at specific points in the application lifecycle, such as before tool execution, after tool execution, at session start/end, and during other key events.
...
Hooks are user-defined scripts or programs that are automatically executed by TURBO SPARK at predefined points in the application flow. They allow users to:
...
TURBO SPARK supports four hook executor types:
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$QWEN_PROJECT_DIR/.turbospark/hooks/security-check.sh",
            "name": "security-check",
            "timeout": 10000
          }
        ]
...
4. TURBO SPARK processes the decision and continues or blocks execution accordingly
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "prompt",
            "prompt": "You are evaluating whether TURBO SPARK should stop working. Context: $ARGUMENTS\n\nAnalyze the conversation and determine if:\n1. All user-requested tasks are complete\n2. Any errors need to be addressed\n3. Follow-up work is needed\n\nRespond with JSON: {\"ok\": true} to allow stopping, or {\"ok\": false, \"reason\": \"your explanation\"} to continue working.",
            "timeout": 30
          }
        ]
      }
...
When `ok` is `false`, TURBO SPARK will continue working and use the `reason` as context for the next response.
...
Hooks fire at specific points during a TURBO SPARK session. Different events support different matchers to filter trigger conditions.
...
{
  "decision": "block",
  "reason": "Must be provided when TURBO SPARK is blocked from stopping"
}
```
...
```bash
#!/bin/bash
# ~/.turbospark/hooks/todo-validator.sh
    ...
    "TodoCreated": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$HOME/.turbospark/hooks/todo-validator.sh",
            "name": "todo-validator",
            "timeout": 5000
          }
        ]
...
```bash
#!/bin/bash
# ~/.turbospark/hooks/todo-completion-validator.sh
    ...
    "TodoCompleted": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$HOME/.turbospark/hooks/todo-completion-validator.sh",
...
Hooks are configured in TURBO SPARK settings, typically in `.turbospark/settings.json` or user configuration files:
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$QWEN_PROJECT_DIR/.turbospark/hooks/run-tests-async.sh",
            "async": true,
            "timeout": 300000
          }
        ]
...
Configure in `.turbospark/settings.json`:
