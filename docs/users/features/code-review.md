...
When reviewing a PR, `/review` creates a temporary git worktree (`.turbospark/tmp/review-pr-<number>`) instead of switching your current branch. This means:
...
- Model attribution footer on each comment (e.g., _— qwen3-coder via TURBO SPARK /review_)
...
1. `.turbospark/review-rules.md` (TURBO SPARK native)
...
4. `TURBOSPARK.md` — `## Code Review` section
...
Example `.turbospark/review-rules.md`:
...
Cache is stored in `.turbospark/review-cache/` and tracks both the commit SHA and model ID. Make sure this directory is in your `.gitignore` (a broader rule like `.turbospark/*` also works). If the cached commit was rebased away, it falls back to a full review.

## Review Reports

For same-repo reviews, results are saved as a Markdown file in your project's `.turbospark/reviews/` directory (cross-repo lightweight reviews skip report persistence):

```
.turbospark/reviews/2026-04-06-143022-pr-123.md
.turbospark/reviews/2026-04-06-150510-local.md
```
