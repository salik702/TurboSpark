...
Headless only — works with `turbospark -p`, a positional prompt, or a prompt
piped via stdin.
...
```bash
# Inline JSON literal
turbospark -p "…" --json-schema '{"type":"object", "properties":{…}}'

# Read from a file
turbospark -p "…" --json-schema @./schemas/summary.json
```
...
- The `ToolCallEvent` telemetry path (OTLP exports, TurbosparkLogger,
  ui-telemetry stream, chat-recording UI event mirror).
- The on-disk chat-recording JSONL at
  `~/.turbospark/projects/<sanitized-cwd>/chats/<sessionId>.jsonl` (re-fed
