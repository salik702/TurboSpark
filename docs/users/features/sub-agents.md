...
Subagents are specialized AI assistants that handle specific types of tasks within TURBO SPARK. They allow you to delegate focused work to AI agents that are configured with task-specific prompts, tools, and behaviors.
...
- **Project-level**: `.turbospark/agents/` (highest precedence)
- **User-level**: `~/.turbospark/agents/` (fallback)
...
To see which extensions provide subagents, check the extension's `turbospark-extension.json` file for an `agents` field.
...
- `glm-5`: Use that model ID. TURBO SPARK first checks the main conversation's
...
When the selector resolves to another auth type, TURBO SPARK creates a dedicated
...
TURBO SPARK accepts the Claude Code 2.1.168 frontmatter fields below so you
can drop a CC agent file into `.turbospark/agents/` and have the supported fields
...
TURBO SPARK proactively delegates tasks based on:
