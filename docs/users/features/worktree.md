...
qwen --worktree=#4174
qwen --worktree https://github.com/turbospark/turbospark/pull/4174
...
Alternatively, ask TURBO SPARK in plain language to create a worktree from inside an existing session:

```text
> start a worktree called experiment-a
Worktree experiment-a created on branch worktree-experiment-a
.turbospark/worktrees/experiment-a
```

From this point on, the model routes every file edit and shell command through `.turbospark/worktrees/experiment-a/`. Your original working directory is untouched.
...
Kept worktree experiment-a at .turbospark/worktrees/experiment-a
```
...
Every Qwen-managed worktree is placed under your project's `.turbospark` directory:

```
<repoRoot>/.turbospark/worktrees/<slug>/         # Working directory
...
```json
{
  "worktree": {
    "name": "experiment-a",
    "path": "/path/to/repo/.turbospark/worktrees/experiment-a",
...
The `agent` tool accepts an optional `isolation: "worktree"` parameter. When set, TURBO SPARK creates an ephemeral worktree at `<repoRoot>/.turbospark/worktrees/agent-<7hex>/` before the sub-agent starts, and:
...
- **Has changes** → the worktree is preserved; its path and branch are appended to the agent's result, e.g.
  ```
  …agent output…
  [worktree preserved: /path/to/.turbospark/worktrees/agent-3f2a1b9 (branch worktree-agent-3f2a1b9)]
  ```
...
```jsonc
// ~/.turbospark/settings.json or <repo>/.turbospark/settings.json
{
  "worktree": {
...
- `agents.arena.worktreeBaseDir` — controls **Agent Arena** worktree placement (default `~/.turbospark/arena`). Does not affect general-purpose worktrees, which always live under `<repoRoot>/.turbospark/worktrees/`.
...
- The current working directory is already inside `.turbospark/worktrees/` (no nested worktrees).
