...
Dual Output is a sidecar mode for the interactive TUI: while TURBO SPARK keeps
...
Embed TURBO SPARK inside the IDE. The TUI runs in the editor's integrated
...
A CI job runs TURBO SPARK with a task prompt. The human sees the TUI in the
...
Integration tests spawn TURBO SPARK headlessly, drive it with `--input-file`
...
At first glance `--json-fd` looks sufficient — the caller spawns TURBO SPARK
...
```ts
const child = spawn('turbospark', ['--json-fd', '3'], {
...
Run TURBO SPARK with both channels enabled using regular files:
...
A typical embedding parent process spawns TURBO SPARK with both channels:
...
```jsonc
// ~/.turbospark/settings.json  (user-level)
// or <workspace>/.turbospark/settings.json  (workspace-level)
{
  "dualOutput": {
...
next TURBO SPARK launch, since the bridge is constructed once during
startup.
...
# Ask TURBO SPARK to do something that needs approval, e.g.
...
The most realistic shape: a parent process spawns TURBO SPARK, tails
...
const child = spawn('turbospark', ['--json-file', events, '--input-file', input], {
  stdio: 'inherit',
});
...
# TURBO SPARK TUI opens in the current terminal; the embedder logs
# handshake + turn-end + session_end events to the parent's stdout.
```

### POC 5 — capability handshake feature detection

Older TURBO SPARK versions won't emit `protocol_version`. Treat the field
...
rl.on('line', (line) => {
  ...
  if (ev.type === 'system' && ev.subtype === 'session_start') {
    ...
    if (v < 1) {
      console.error(
        'turbospark dual output is present but protocol < 1; ' +
          'falling back to best-effort behavior',
      );
    } else {
      console.log('turbospark dual output protocol v' + v);
    }
  }
});
```
