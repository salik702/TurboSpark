...
Add a `statusLine` object under the `ui` key in `~/.turbospark/settings.json`:
...
| `qwen-version`         |         | TURBO SPARK version (e.g. `v0.14.1`)                                 |
...
```json
{
  ...
  "worktree": {
    "name": "fix-auth",
    "path": "/home/user/project/.turbospark/worktrees/fix-auth",
    "branch": "fix-auth",
...
| `version`                             | string           | TURBO SPARK version                                                                  |
...
For longer commands, save a script file at `~/.turbospark/statusline-command.sh`:
...
```json
{
  "ui": {
    "statusLine": {
      "type": "command",
      "command": "bash ~/.turbospark/statusline-command.sh"
    }
  }
}
```
...
- **Shell**: Commands run via `/bin/sh` on macOS/Linux. On Windows, `cmd.exe` is used by default — wrap POSIX commands with `bash -c "..."` or point to a bash script (e.g. `bash ~/.turbospark/statusline-command.sh`).
