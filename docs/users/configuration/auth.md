...
TURBO SPARK supports three authentication methods. Pick the one that matches how you want to run the CLI:

- **TURBOSPARK OAuth**: sign in with your `qwen.ai` account in a browser. **Free tier discontinued on 2026-04-15** — switch to another method.
...
## Option 1: TURBOSPARK OAuth (Discontinued)

> [!warning]
>
> The TURBOSPARK OAuth free tier was discontinued on 2026-04-15. Existing cached tokens may continue working briefly, but new requests will be rejected. Please switch to Alibaba Cloud Coding Plan, [OpenRouter](https://openrouter.ai), [Fireworks AI](https://app.fireworks.ai), or another provider. Run `qwen` and use `/auth` to configure.

- **How it works**: on first start, TURBO SPARK opens a browser login page. After you finish, credentials are cached locally so you usually won't need to log in again.
...
- **How it works**: Subscribe to the Coding Plan with a fixed monthly fee, then configure TURBO SPARK to use the dedicated endpoint and your subscription API key.
...
Enter `qwen` in the terminal to launch TURBO SPARK, then run the `/auth` command and select **Alibaba Cloud Coding Plan**. Choose your region, then enter your `sk-sp-xxxxxxxxx` key.
...
If you prefer to skip the interactive `/auth` flow, add the following to `~/.turbospark/settings.json`:
...
The simplest way to get started with API Key authentication is to put everything in a single `~/.turbospark/settings.json` file. Here's a complete, ready-to-use example:
...
| `security.auth.selectedType` | Tells TURBO SPARK which protocol to use on startup (e.g. `openai`, `anthropic`, `gemini`). Without this, you'd need to run `/auth` interactively. |
| `model.name`                 | The default model to activate when TURBO SPARK starts. Must match one of the `id` values in your `modelProviders`.                                |
...
The key concept is **Model Providers** (`modelProviders`): TURBO SPARK supports multiple API protocols, not just OpenAI. You configure which providers and models are available by editing `~/.turbospark/settings.json`, then switch between them at runtime with the `/model` command.
...
#### Step 1: Configure models and providers in `~/.turbospark/settings.json`
...
> [!important]
>
> It is recommended to define `modelProviders` in the user-scope `~/.turbospark/settings.json` to avoid merge conflicts between project and user settings.

Edit `~/.turbospark/settings.json` (create it if it doesn't exist). You can mix multiple protocols in a single file — here is a multi-provider example showing just the `modelProviders` section:
...
TURBO SPARK reads API keys from environment variables (specified by `envKey` in your model config). There are multiple ways to provide them, listed below from **highest to lowest priority**:
...
TURBO SPARK auto-loads the **first** `.env` file it finds (variables are **not merged** across multiple files). Only variables not already present in `process.env` are loaded.

Search order (from the current directory, walking upward toward `/`):

1. `.turbospark/.env` (preferred — keeps TURBO SPARK variables isolated from other tools)
2. `.env`
...
> [!tip]
>
> `.turbospark/.env` is recommended over `.env` to avoid conflicts with other tools. Some variables (like `DEBUG` and `DEBUG_MODE`) are excluded from project-level `.env` files to avoid interfering with TURBO SPARK behavior.

**3. `settings.json` → `env` field (lowest priority)**

You can also define API keys directly in `~/.turbospark/settings.json` under the `env` key. These are loaded as the **lowest-priority fallback** — only applied when a variable is not already set by the system environment or `.env` files.
...
After launching TURBO SPARK, use the `/model` command to switch between all configured models. Models are grouped by protocol:
...
| API-key or custom provider setup | Configure `~/.turbospark/settings.json`, `.env`, or provider-specific environment variables       |
| Check current authentication     | Run `/doctor` inside TURBO SPARK                                                              |
...
- Prefer `.turbospark/.env` for project-local secrets (and keep it out of git).
