...
TURBO SPARK supports a variety of themes to customize its color scheme and appearance. You can change the theme to suit your preferences via the `/theme` command or `"theme":` configuration setting.

## Available Themes

TURBO SPARK comes with a selection of pre-defined themes, which you can list using the `/theme` command within the CLI:
...
1.  Enter `/theme` into TURBO SPARK.
...
Selected themes are saved in TURBO SPARK's [configuration](../configuration/settings) so your preference is remembered across sessions.

---

## Auto Theme Detection

When the theme is set to `"auto"` (or left unset), TURBO SPARK automatically detects whether your terminal uses a dark or light background and selects the matching Qwen theme (`Qwen Dark` or `Qwen Light`).
...
TURBO SPARK uses multiple detection methods in a fallback chain. At startup (async path), the order is:
...
TURBO SPARK allows you to create your own custom color themes by specifying them in your `settings.json` file. This gives you full control over the color palette used in the CLI.
...
- Select your custom theme using the `/theme` command in TURBO SPARK. Your custom theme will appear in the theme selection dialog.
