...
**ACP 消息内容**：action 现有返回内容已包含完整文件路径（如 `Session exported to markdown: turbospark-export-2024-01-01T12-00-00.md`），对 IDE 消费友好，无需修改文本。
...
- `handleSlashCommand('/docs', ...)` 在 non-interactive 模式下返回 `{ type: 'message', content: 包含 'turbospark.github.io' }`
