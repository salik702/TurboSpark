...
`docs/developers/turbospark-serve-protocol.md` § `POST /session/:id/recap`
...
(TURBOSPARK.md / managed auto-memory) as a suffix. The final system prompt is
...
`~/.turbospark/debug/<sessionId>.txt` (`latest.txt` symlinks to the current
...
| `TURBOSPARK_ENABLE_AWAY_SUMMARY` env var          | Claude Code uses it to keep the feature on when telemetry is disabled; TURBO SPARK's current telemetry model doesn't need this.            |
