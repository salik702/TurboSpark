...
1. Cross-provider reliability — OpenAI-compatible endpoints, Gemini, and
   TURBO SPARK's native tool-calling all implement function calling; tag parsing
...
symlink planted in `~/.turbospark/projects/<proj>/chats/` can't redirect a
...
4. `!config.isInteractive()` → skip. Headless `turbospark -p` / CI never spends
...
| Non-interactive auto-titling                | `turbospark -p` / CI scripts throw the session away; fast-model tokens for a title no one will ever resume is pure waste.                         |
