...
> 本文介绍 TURBO SPARK 中 **Managed Auto-Memory**（托管自动记忆）的记忆管理机制、触发时机和实现细节。
...
> **环境变量覆盖**：
>
> - `TURBOSPARK_MEMORY_BASE_DIR`：替换全局基础目录
> - `TURBOSPARK_MEMORY_LOCAL=1`：改用项目内路径 `.turbospark/memory/`
...
**不应该存入记忆的内容**：代码模式/约定、Git 历史、调试方案、临时任务状态、已在 TURBOSPARK.md/AGENTS.md 中记录的内容。
